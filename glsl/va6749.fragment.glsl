...
void main( void ) {
	...
	sph1.xz = vec2 ( 1.5 * cos( time ),  2.5 * sin( time )+2.5); // move sphere around	
	vec3 ro = vec3( 0.0, 1.4, 8.0 ); //ray origin
...
*/if ( id>1.5 ) {				// plane
		...
		float amb = smoothstep( 0.0, sph1.w * 3.0, length(pos.xz - sph1.xz)); // shadows under the sphere
		amb = 1.0- amb;
	col = vec3 (amb * 1.9);
		...
		col.rgb *= vec4(vec3(color*shade) * vec3(0.9*pos.z,.5*pos.z,abs(pos.z)*0.2), 1.0).rgb;
	}		
