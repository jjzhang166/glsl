// by @eddbiddulph - made a new cookie texture
...
float faceMask(vec2 tc)
{
    return step(dot(tc, tc), 0.25);
}
...
float circleMask(float radius, vec2 p, vec2 tc)
{
    tc -= p;
    return step(dot(tc, tc), radius * radius);
}


vec3 faceTexture(vec2 tc)
{
    float dsq = dot(tc, tc);

    // eyes
    float m = max(circleMask(0.08, vec2(-0.2, 0.2), tc), circleMask(0.08, vec2(0.2, 0.2), tc));

    // outline
    m = max(m, step(0.45 * 0.45, dsq));

    // mouth
    m = max(m, min(step(tc.y, -0.05), step(0.28 * 0.28, dsq) - step(0.35 * 0.35, dsq)));

    return mix(vec3(1.0, 1.0, 0.1), vec3(0.0), m);
}

vec3 lightTexture(vec2 tc)
{
  return faceTexture(tc);
  /*
    tc += 0.5; // put centre at {0.0, 0.0}
