...
{
	// this seemed slow compared to using dot functions
	return vec2(
		dot(vec2(c1.x, -c1.y), c2), //c1.x * c2.x - c1.y * c2.y,
		dot(c1, c2.yx)              //c1.x * c2.y + c1.y * c2.x
	);
}
void main( void )
{
	...
	uPos.x -= 1.4 + sin(time/2.11)/20.0;
	uPos.y -= 1.1 + sin(time/1.11)/20.0;
