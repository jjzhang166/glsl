...
void main( void )
{
	...
	vec2 z = surfacePosition*0.05+vec2(0,1.);
	
	z += vec2(sin(time*.02), cos(time*.02))*.2;
	c.x = -.4+(sin(time*.1))*.1;
	c.y = -.16+(sin(time*.1))*.01;
	...
	color = pow(color, 8.);//+sin(time*.005));

	gl_FragColor = fract(vec4(color*1.1e2,color*4.1e2,color*4.1e2,1.0))
	;
