...
void main(  ) {
	...
	float deltaIO = mod(time, interval);
	float deltaM = mod(time+3.0, interval*effects);
	...
	if (e == 0.0) c += clamp(effectMain1(time), 0.0, 1.0);
	if (e == 1.0) c += clamp(effectMain2(time), 0.0, 1.0);
	if (e == 2.0) c += clamp(effectMain3(time), 0.0, 1.0);
	if (e == 3.0) c += clamp(effectMain4(time), 0.0, 1.0);
