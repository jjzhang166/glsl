...
void main(void) {
	...
	vec4 vColor = vec4(5.4, 8.7, 0.5, 0.5);
	
	//adjust padding to keep circle within bounds...
	float padding = 9.0;
	//play with these values
	float smooth1 = 996000.0;
	float smooth2 = 1000.0; 
	
	vec2 c = vec2(resolution.x/3.0, resolution.y/2.0);
	
	//for simplicity, assume we're dealing with square sizes
	float dist = resolution.y/3.0 - distance(gl_FragCoord.xy, c);
