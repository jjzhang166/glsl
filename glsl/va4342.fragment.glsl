// Mike: Removed some text to make it compile with fewer temp registers
#ifdef GL_ES
  ...
  //  if (abs(pos.x) > 0.4 || abs(pos.y) > 0.4) 
    ...
    */
	...
	if (acc < 0.5) {
		...
		float t0 = cos(time*19.0+cursor.y*221.0);
		float t1 = cos(time*67.0+cursor.y*94.7);
		float t2 = sin(time*28.0+cursor.y*169.0);
		float t3 = cos(time*36.0+cursor.y*129.0);
		float t4 = cos(time*46.5+cursor.y*198.7);
		float t5 = sin(time*57.8*cursor.y*91.0);
