...
// just added mouse control to show it off... beautiful work
...
{
    float fixedRadius2 = mouse.x * 8.0;
    float minRadius2 = mouse.y * 0.5;
    float r2 = dot(z,z);
    if (r2<minRadius2) { 
...
void main( void ) {
	...
	vec4 result = vec4( vec3(1.0) - vec3(c, c, c) - vec3(0.025, 0.025, 0.025)*m*1.3, 1.0 );
	result.xyz += vec3(glow*6.0, glow*1.0, glow*1.5);
	gl_FragColor = result;
}
