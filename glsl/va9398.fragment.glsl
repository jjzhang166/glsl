...
vec3 Scene2D(vec2 p) {
	...
	for (int i = 0; i < 30; ++i) {
		color = mix(color, LIGHT_BLUE, Rect(p, vec2(sin(a) * 0.6, cos(a) * 0.6), OUT_BOX, a));
		a += inc;
	}
...
void main()
{
	...
	gl_FragColor *= 0.5;
	gl_FragColor += vec4(Scene2D(coords*1.5)/2.0,1.0);
}
