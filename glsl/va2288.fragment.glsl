...
float getVal(in vec2 position) {
	...
	float t = time + 1.0 / u;
	float val = sin(0.12 * (time * 1.0 + 10.0 * v) );
	return max(-0.5,val);
}

float getMix(in vec2 position, float n) {
	float val1 = getVal(position + vec2(cos(time * 0.95 + n + n), sin(time * 0.1 + n + 10.0)) * 0.1);
	float val2 = getVal(position + vec2(sin(time * 0.21 + n * n), cos(time * 0.02 + n + 4.2)) * 0.1);
	return (val1 * val2 + val1 + val2) * 0.6;
}

vec3 colorMix(in vec2 position, in vec3 color, in float n) {
	float val = getMix(position, n);
	return color * val + (1.0 - val) * vec3(0.9, 0.1, 0.0);
}
