...
//Non penis update --- No longer looks like a weiner on a checkerboard
//also less depth
//patched by someDouche
...
vec2 obj1(in vec3 p){
  return vec2(length(max(abs(p)-vec3(1.0,1.0,1.0),3.0))-1.25,1);
}

//RoundBox with simple solid color
vec3 obj1_c(in vec3 p){
	return vec3(0.0,2.5,0.2);
}
...
vec2 inObj(in vec3 p){
	...
	r = ObjUnion(r, capsule(q, 1., 1.));
	//r = ObjUnion(r, vec2(length(q+vec3(-.5, 1.5, 0.)) - 0.75, 1.));
	//r = ObjUnion(r, vec2(length(q+vec3(.5, 1.5, 0.)) - 0., 1.));
	//r = ObjUnion(r, vec2(length(q+vec3(0, -1.5, 0.)) - 0.2, 1.));
