// @rotwang, mod* colors, checker, cage
...
float cageDistance( in vec3 p, const in vec3 origin, const in float fSphereRadius)
{
...
float GetDistanceBumpyFloor( const in vec3 vPos, const in float fHeight )
{
	float da = 3.0;
	float dd = -0.05;
	return vPos.y - fHeight + (sin((vPos.x + timed4) * da) * dd + dd)
		+ (cos((vPos.z) * da) * dd + dd);
}
...
vec2 GetDistanceScene( const in vec3 vPos )
{
	...
	vec2 vDistSphere1 = vec2(GetDistanceSphere( vPos, vec3( 1.75 + sin(timed4), -0.25, 5.0 + cos(timed4)), 0.75 ), 2.0);
	vec2 vDistSphere2 = vec2(cageDistance( vPos, vec3(-1.75 - sin(timed4), -0.25, 5.0 - cos(timed4)), 0.75 ), 3.0);
...
vec3 GetObjectDiffuse( in float fObjId, in vec3 vPos )
{
	...
	vec3 checker_a = vec3(0.2,0.2, 0.2);
	vec3 checker_b = vec3(0.8,0.8, 0.8);
	...
	if(fObjId < 1.5) // checker
	{
		float fBlend = mod(floor(fract(vPos.x + time/8.0) *2.0) + floor(fract(vPos.z) * 2.0), 2.0);
		return checker_a * fBlend + checker_b * (1.0 - fBlend);
	}
...
vec3 GetSkyGradient( const in vec3 vDir )
{
	...
	return vec3(0.0, 0.0, 1.0) * fBlend + vec3(0.2, 0.6, 0.8) * (1.0 - fBlend);
...
vec3 GetLightPos()
{
	return vec3(0.0 + sin(timed4), 1.0 + cos(timed4 * 1.231), 5.0);
}
