...
vec2 obj0(in vec3 p){
  ...
  p.z=p.z-32.0;
	  p.xy=rot(p.xy,time);
  p.yz=rotsim(p.yz,1.0);

  p.z=p.z-3.0;
  p.xy=rotsim(p.xy,8.0);
  float c1=length(max(abs(p)-vec3(0.2,0.2,0.2),0.0));
  float c3=length(max(abs(p)-vec3(0.2,4.0,0.1),0.0));
...
vec2 obj1(vec3 p){
  ...
  float c1=length(max(abs(p)-vec3(0.1,0.2,2.0),0.0));
  float c3=length(max(abs(p)-vec3(8.0,8.0,0.1),0.0));
