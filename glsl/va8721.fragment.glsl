...
void main( void ) {
	...
	f += trans (bounce (vec2 (0.5413479757, 0.4512601603), vec2 (0.9359430479, 0.5225455910), t)) + f;
	f += trans (bounce (vec2 (0.9943942183, 0.0935004216), vec2 (0.3106910990, 0.1622248976), t)) + f;
	f += trans (bounce (vec2 (0.2429376209, 0.1883747749), vec2 (0.4109957732, 0.6861682279), t)) + f;
	...
	f = f / (f - 0.5);
	f = clamp(.991, .637, tan(utils_sigmoid (f)));
	float fluid = sin(.0006*mix (color2, color1, tan(color1/f))).r;
	gl_FragColor = f*fluid + dot(9.+f, fluid) + vec4(fluid-0.4, fluid-0.3, fluid, 0.);
}