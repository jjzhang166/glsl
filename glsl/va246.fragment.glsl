...
void main( void ) {
	vec2 position = (( gl_FragCoord.xy / resolution.xy ) - 0.5)  * 2.0 + (mouse - 0.5) * 2.0;
	vec2 zero = vec2( cos(time), sin(time) );
	int n = julia( position, zero );
	
	float color = float(n) / float(divergent);
	gl_FragColor = vec4( vec3( color, cos(color+time), sin(color+time) ), 1.0 );
}