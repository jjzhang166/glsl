...
mat2 m = mat2( 0.50,  0.50, -0.50,  0.50 );
...
float grid(vec2 loc) {
	float dist=0.8; // distance to closest grid cell
	...
	for (float dy=-1.0;dy<=1.0;dy++)
		for (float dx=-1.0;dx<=1.0;dx++)
		{
			...
			if (gridshift.x>.5) gridshift.x = 3.0;else gridshift.x=0.0; // pick some weird grid offsets
			if (gridshift.y>.5) gridshift.y = 0.0;else gridshift.y=3.0; // pick some weird grid offsets
			vec2 center=thiscorner+gridshift;
			vec2 delta = loc - center;
			float d = sqrt(delta.x/sin(time))-pow(delta.y, sin(time));
			dist=min(d,dist);
		}
	return dist;
}
