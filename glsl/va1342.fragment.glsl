...
float q(vec2 p) {
	...
	float c = smoothstep(0.9, 1.0, 1.0 - abs(p.y - s) * perlin().z);  
	return c; 
}

vec3 aurora(vec2 p, float time) {
	vec3 c1 = q( vec2(p.y, p.x / 0.4) + vec2(sin(time), -0.5)) * vec3(15.0, 1.0, .9); 	
	vec3 c2 = q( vec2(p.y, p.x) + vec2(time, -0.2)) * vec3(.1, .1, .8); 	
	vec3 c3 = q( vec2(p.y, p.x / 0.2) + vec2(time / 1.0, -.6)) * vec3(.90, 1.0, .90); 
