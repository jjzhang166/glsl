...
const float origin_z = 12.0;
const float plane_z = 41.0;
const float far_z = 641.0;

const float astep = (far_z - plane_z) / float(iters) * 0.025;

const float color_bound = 0.0;
const float upper_bound = 11.0;

const float scale = 312.0;

const float disp = 10.25;
...
void main(void)
{
	...
	float scale = sin(0.1 * time) * 2.0 + 2.5;

	vec2 position = vec2((gl_FragCoord.x / resolution.x) - 0.5, (gl_FragCoord.y / resolution.y) - 0.5) * scale + (mouse * 10.0) / scale;

	vec2 rotated_position;
	rotated_position.x = ((position.x - 0.5) * cos(sin(0.125 * time) * 2.5) - (position.y - 0.5) * sin(sin(0.125 * time) * 2.5)) * scale;
	rotated_position.y = ((position.y - 0.5) * cos(sin(0.125 * time) * 2.5) + (position.x - 0.5) * sin(sin(0.125 * time) * 2.5)) * scale;
