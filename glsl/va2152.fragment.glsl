...
vec2 distanceField(in vec3 p){
	...
	//return ObjUnion(obj_floor(p),obj_manyBoxes(p)); // infinite boxes
...
vec2 obj_clusterBuildings(in vec3 p){
	...
	for(int i = 0; i < 3; i++){
		for(int j = 0; j < 5; j++){
			building = obj_simpleBuilding(q+vec3(i*5,0,j*5), 3.0);//rand(vec2(i,j))*3.0);
...
void main(void){
	...
	E=vec3(-sin(time/10.0)*10.0,5,cos(time/10.0)*10.0); //spinning scene
		...
		E=vec3(-sin(1.0)*10.0, 6.0, cos(1.0)*10.0);//Change camera path position here
	}
