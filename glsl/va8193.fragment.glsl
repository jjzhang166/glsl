...
float rand( vec2 n ) {
	  ...
	  return fract(sin(dot(n.xy, vec2(312.9898, 784.233)))* 432758.4453);
}

void main( void ) {

	vec2 position = ( gl_FragCoord.xy / resolution.xy );
	//gl_FragColor = vec4( vec3(rand(position)), 1.0 ); return;
	float offset = sin(time/2.0+0.1)*0.2;
	position += vec2(rand(position)*offset-offset/2.0, rand(1.0-position)*offset-offset/2.0);
	position.x = position.x/0.6-0.3;
