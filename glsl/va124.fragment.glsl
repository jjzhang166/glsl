...
void main( void ) {
	...
	float wave1 = sin(d1*waveLength+(time*5.))*0.9 + 0.5 * (((d1 - 0.5) * 2.) + 0.5);
	float wave2 = sin(d2*waveLength+(time*2.))*0.5+0.5 * (((d1 - 0.2) * 2.) + 0.6);
	float c = d1 > 0.11 || d2 > 0.99 ? 1. : 0.;
	c + wave1*wave2;
	gl_FragColor = vec4(c + wave1*wave2,c,c,2.);
