...
void main( void )
{
	...
	if( mouse.x == 0.0 || time < 1.) {
		...
		float t = distance(length((1.5-sin(cos(time*3.)+time * 2.)) * 256.*du*dv+(position-mouse)) * color.xyz + rand.y, color.xyz + rand.x);
		
		t -= .99*t;
		
		gl_FragColor = .05*abs(sin(time*1.))+vec4(t * length(position-mouse)) + abs(sin(rand.x * 32. * time) * vec4(.111,.115, .039, 1.)); //sphinx
	}//todo - cleanup - raytrace
}//sphinx