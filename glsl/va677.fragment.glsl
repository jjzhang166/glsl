...
void main( void ) {
 	...
 	float zoom = mouse.x + 0.5;
	vec2 position = ((( gl_FragCoord.xy / resolution.x ) - 0.5) + mouse * 2.0) * 32.0 * zoom * zoom;
	...
	float facets = smoothstep(0.0, 0.15, abs(F.y-F.x));
	float facetsX = smoothstep(0.0, 0.15, abs(Fx.y-Fx.x));
	float facetsY = smoothstep(0.0, 0.15, abs(Fy.y-Fy.x));
	...
	float light = clamp(1.-length(uv - mouse + d),0.,1.) + 0.1;
