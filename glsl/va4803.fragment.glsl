...
float logscale2(float v) {
    return pow(2.0, floor(log(v) / LOG2));
}

float logscale(float v, float b) {
    return pow(b, floor(log(v) / log(b)));
}
...
float position_to_distance(vec2 o) {
	float m = 3.0;
	float xt = floor(o.x / m);
	o.x = o.x - xt;
	float n = o.x / m;
	float shape1 = hexatiles(o) - 0.5;
	float shape2 = hexatiles(o + vec2(1.0,0.0)) - 0.5;
	return mix(shape1, shape2, 1.0 - n);
}
...
float position_to_distance(vec3 o) {
	...
	//float scene = max(position_to_distance(o.xz), o.y);
	
	vec3 n = o;
	o.x = abs(o.x);
	
	float b = 1.618;
	float cellid = logscale(abs(o.x), b);
	
	o.x = o.x - cellid;
	
	float c = 0.22;
	float sphere1 = length(o) - cellid*c;
	float sphere2 = length(o - vec3(cellid*(b - 1.0),0.0,0.0)) - (cellid*b)*c;
	
	float scene = min(sphere1, sphere2);
	scene = min(scene, max(o.y, ringslice(rotate(n.xz, time), vec2(4.0, 0.2), radians(45.0))));
	
	return min(scene, o.y + 1.0);
}
...
void render_3d() {
  ...
  vec3 prp=vec3(sin(mouse.x*3.0)*8.0,4,cos(mouse.x*3.0)*8.0); //Change camera path position here
...
vec3 lp = prp + vec3(0.0, 5.0, 0.0);	
  ...
  for(int i=0;i<64;i++){
	...
	c=vec3(.5,0.5,0.5);
    	...
    	float b=dot(n,normalize(lp-p1));
    	vec4 tex=vec4(vec3(b),1.0);//vec4((b*c+pow(b,8.0))*(1.0-f*.01),1.0);
	vec4 background=vec4(0,0,0,1);
	
	vec4 Color=(f<maxd)?tex:background;
	Color.rgb *= 1.0 - steps;
	Color.r = smoothstep(0.0, 1.0, Color.r);
	Color.g = max(smoothstep(-0.1, 0.95, Color.b), 0.0);
