...
//============================== UTILS ====================================//
vec2 distanceField(in vec3 p){
	vec2 test = obj_infiniteBuildings(p);
...
vec2 obj_infiniteBuildings(in vec3 p){
	...
	vec2 pos = vec2(ceil(abs(p.x)/c.x), ceil(abs(p.z)/c.z));
	
	float height = rand(pos)* 10.0 - 4.0;
	float width1 = rand(pos + 1150.0) * 1.5 + 0.5;
	float width2 = rand(pos + 100.0) * 1.5 + 0.5;
	
	//building height
	vec3 k = vec3(max(0.0, width1), max(0.0, height), max(0.0, width2));
	return obj_sdBox(q,k);	
}
