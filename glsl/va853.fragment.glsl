...
void main( void ) {
	...
	vec2 p2 = (vec2(sin(time+3.142), cos(time+3.142))*orbitDistance)+.25;

	float d1 = 0.1-length(uv +p1);
	float d2 = 0.1-length(uv -p2);

	float wave1 = sin(d1*waveLength+(time*5.))*0.5 + 0.5 * (((d1 - 0.5) * 2.) + 2.5);
	float wave2 = sin(d2*waveLength+(time*5.))*0.5 + 0.5 * (((d1 - 0.5) * 2.) + .5);
	...
	gl_FragColor.y = gl_FragColor.z*.5 + gl_FragColor.x*0.1;
