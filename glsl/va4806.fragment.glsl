...
float position_to_distance(vec2 o) {
	float r1 = tri(rect(mod(o,vec2(3.0,2.0)) - vec2(1.5,1.0), vec2(0.5, 0.25)))-0.5;
	float r2 = tri(rect(mod(o,vec2(2.4,1.6)) - vec2(1.2,0.8), vec2(0.1, 0.4)))-0.95;
	vec2 p = vec2((o.x + o.y) * 0.707, (o.x - o.y)*0.707);
	
	float r3 = tri(rect(mod(p,vec2(4.6,1.1)) - vec2(2.3,0.55), vec2(2.1, 0.4)))-0.9;
	
	float g = min(-r3, max(r1, r2));
	float ht = hexatiles(o) - 0.8;
	return ht;
}
