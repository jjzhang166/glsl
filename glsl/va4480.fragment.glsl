...
float udRoundBoxRep( vec3 p, vec3 c )
{
    vec3 q = mod(p,c)-0.5*c;
    return udRoundBox( q, vec3(0.1), 0.01 );
}
...
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
	...
	float op = udRoundBoxRep(p, vec3(0.7));
//        return min(op, min(min(blend(sphere1, torus), blend(sphere2, torus)), plane));
        return min(min(min(blend(sphere1, torus), blend(sphere2, torus)), op), plane);
    } else {
        return plane;
    }
}
