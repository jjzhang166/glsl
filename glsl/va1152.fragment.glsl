...
{	
	...
	ray.vDir = normalize( vec3(vViewCoord, 1.3) );
}
...
float GetBumpyFloorHeight(const in vec3 vPos, const in int level)
{
	float base = pow(2.0, float(level));
	float lambda = base * 3.0;
	float amp = 0.1 / base;
	return (sin(lambda * (vPos.x + time)) * amp) + (sin(lambda * vPos.z) * amp);
}

float GetBumpyFloorHeight(const in vec3 vPos)
{
	return GetBumpyFloorHeight(vPos, 0) + GetBumpyFloorHeight(vPos, 1) + GetBumpyFloorHeight(vPos, 2) + GetBumpyFloorHeight(vPos, 3) + GetBumpyFloorHeight(vPos, 4);
}
...
void main( void ) 
{
	...
	cFinal = cFinal / (1.0 + cFinal);
        // [avoozl] bit more realistic gamma control here (makes the light look a bit more real)
	gl_FragColor = vec4( pow(cFinal.xyz,vec3(1.8,1.8,1.8)), 1.0 );
}
