...
void main( void ) {
	...
	float f0 = fract(time * time *0.05)+1e-5;
	...
	float as = (cos(time*.2)*.4);	// there was originally an outer sinh for as and bs but difference is just about 1 percent which doesn't really matter for the camera path
	float ac = sqrt(as*as+1.);
	float bs = (sin(time*.6)*.4);
	...
	for (int i = 0; i < 13; i++) {
		...
		color = color * mod(length(hpos) * (tDist * 3.0), 1.0) * 2.0;
