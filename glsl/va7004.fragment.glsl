...
#endif
//By ninjapretzel
//Just playing around and learning how to use this thing
//Looked at some other functions, and adapted some things into a perlin noise function
//There are probably better ways to create a 'light' and rays, but it does look pretty cool.
...
float random(in float a, in float b) { return fract((cos(dot(vec2(a,b) ,vec2(2.9898,22.233))) * 43758.5453)); }
...
void main(void) {
	...
	float r = combineNoise(pos.x, pos.y)*.5;
	float g = combineNoise(pos.x*1.5, pos.y*1.5) * .4;
	float b = combineNoise(pos.x*.5, pos.y*.5) * .5;
	
	vec3 col = vec3(r, g, b) * 1.;
