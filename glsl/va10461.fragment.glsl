...
void main( void ) {
	...
	v[6] = vec2(1050.0, 500.0);
	v[7] = vec2(1150.0, 500.0);
	v[8] = vec2(1100.0, 600.0);
	...
	for (int i = 0; i < NUM_VERTS-1; ++i) {
		...
		if ((abs(A[i]) <= 400.0) && (D[i] < 0.0)) {
			gl_FragColor = (R[i+1]*C[i] + R[i]*C[i+1]) / (R[i] + R[i+1]);
			return;
		}
	}
	...
	if ((abs(A[LAST_VERT]) <= 400.0) && (D[LAST_VERT] < 0.0)) {
		gl_FragColor = (R[0]*C[LAST_VERT] + R[LAST_VERT]*C[0]) / (R[LAST_VERT] + R[0]);
		return;
	}
