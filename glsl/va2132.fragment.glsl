...
// INFINITE BOXES
vec2 obj_manyBoxes(in vec3 p){
	vec3 offset = vec3(0.,3,5.); //makes cubes touch the ground
	vec3 c = vec3(5,10,5); // how close cubes are to each other
	vec3 q = p;
	q.y += (sin(p.x * 0.5) + cos(p.z * 0.5))*.5;
	p += vec3(sin(time + p.x + p.y), 1., sin(time+p.z + p.y)*0.5) * vec3(1., 0., 1.);
	//vec3 q = mod(p-offset,c)-0.5*c;
	q.x = mod(p.x-offset.x,c.x)-0.5*c.x;
	q.z = mod(p.z-offset.z,c.z)-0.5*c.z;
	q.y += (cos(p.x * 0.5) + sin(p.z * 0.5))*.5;
	
	return obj_sdBox(q,vec3(1.5,4.0,1.5));	
}

// SIMPLE BUILDING (white)
vec2 obj_simpleBuilding(in vec3 p, in float height){
	vec3 offset = vec3(sin(p.y), (-height), 0.0);
