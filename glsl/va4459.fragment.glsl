...
void main( void ) {
	...
	float d1 = rect(pos - vec2(-1.0,0.0), vec2(0.1,0.75), uflash * 0.5); 
	vec3 clr1 = vec3(0.2,0.6,1.0) *d1; 
	
	float d2 = rect(pos - vec2(0.0,0.0), vec2(0.1,0.5), uflash * 0.3); 
	vec3 clr2 = vec3(0.6,0.99,0.2) *d2; 

	float d3 = rect(pos - vec2(1.0,0.0), vec2(0.1,0.25), uflash*0.5); 
