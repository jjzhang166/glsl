...
float cast_sky(vec3 p) {
  ...
  float t2 = 1.0;// dot(p, normalize(vec3(0.0, 1.0, 0.0)));
  float t3 = 1.0;// length(max(abs(d).xz - 0.1, 0.0)) - 0.05;
  float t4 = 1.0;// length(abs(mod(p.xz,2.0)) - 1.0) - 0.9;
  float t5 = 1.0;// length(abs(mod(p.zy,0.2)) - 0.1) - 0.05;
