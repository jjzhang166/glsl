...
vec3 floorColor( in vec3 pos, in vec3 nor, out vec2 spe )
{
    ...
    float f = fbm( 4.0*pos*vec3(6.0,10,0.5) );
    ...
    // frekles
    f = smoothstep( 0.0, 2.0, fbm(pos*111.0) );
    f = smoothstep( 0.4,0.9,f);
    col = mix( col, vec3(0.2), f*0.75 );

    // fake ao
    f = smoothstep( 0.1, 1.1, length(pos.xz) );
    col *= f*f*01.4;
    col.x += 0.1*(0.10-f);
    return col;
}
...
void main(void)
{
    ...
    if( tmat.y>0.5 )
    {
        ...
        // lights
        col  = 0.10*con*vec3(0.80,0.90,0.00);
        col += 0.70*dif*vec3(0.00,0.0,0.0)*vec3(sh, (sh+sh*sh)*0.5, sh*sh );
        col += 0.15*bac*vec3(0.00,0.00,0.0);
        col += 0.20*amb*vec3(0.00,0.0,0.00);
