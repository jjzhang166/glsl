...
vec2 obj0(in vec3 p)
{
  ...
  p.z=p.z-0.5;
  p.xy=rotsim(p.xy,12.0);
	
vec3 ap = abs(p);
 // float c0=length(max(abs(p)-vec3(0.1,0.1,0.1),0.0));
  float c1=length(max(ap-vec3(0.1,0.3,0.1),0.0));
  float c2=length(max(ap-vec3(0.1,0.125,2.5),0.0));
float c = _union(c1,c2);
