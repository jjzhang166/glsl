...
const vec3 diffuse = 1.0-vec3( .5, .3, 0.2 );
...
float c( vec3 p )
{
	...
	q = sin(abs(mod(q,2.0)-1.0));
	float b = q.x + q.y + q.z - fract(min(min(sin(q.x), cos(q.y)), q.z)) - max(max(q.x, q.y), q.z);
	return min(a,b);
}
