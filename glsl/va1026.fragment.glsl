...
{	
	float dif = res * 40.0 - abs( left - right );
	if ( dif > 0.0 ) {
		float percent = dif / (res * 40.0);
		vec3 newColor = color * percent + vec3(1.0, 1.0, 1.0) * (1.0 - percent);
		//gl_FragColor = vec4( color, percent ); //This would work but transparency doesn't for some reason	
		gl_FragColor = vec4( newColor, 0.9 );	
	}
}
...
void main( void )
{
	...
	// y = x^3
	drawFormula( y, x*x*x, res*(1.0+abs(y))/2.0, vec3( 0.0, 1.0, 0.0 ) );
	// y = sqrt( x )
