...
vec3 f2(vec3 c)
{
	...
	float w=length(c);
	c=mix(c*vec3(1.0,1.2,1.6),vec3(w,w,w)*vec3(7.4,1.2,1.0),w*1.1-0.2);
	return c;
}
...
void main( void ) {
	...
	float qx= cloud(vec3(coord+vec2(0.16,1.0),time*0.0222));
	...
	//d+=0.1;
	d*=0.3;
	d=mix(vec3(1.0,1.0,pow(cos(atan(qy)), sin(qx)))*0.1+d*7.0,vec3(1.0,qy,1.0)*0.9,1.0-pow(q,(pow(qy,4.03)))*1.1);
    d*=8.0;
    //d+=cos(time*0.01-0.5);
    ...
    vec3 col =mix(vec3(sin(time*0.1-3.6)-2.0/(position.y*2.5+0.2),sin(time*0.1+0.5)/(position.y*9.5),0.2)+gradient*cos(time*0.1+sin(time*0.1-1.0))*cos(time*0.1),d,clamp(0.0,q*2.9,1.0));
    
    //vec3 col =mix(vec3(0.1,0.5,0.8)/(position.y*0.5+0.7)*gradient*0.1,d,q);
    color = vec3(0, gradient, 1);
	col += (color*0.5);
