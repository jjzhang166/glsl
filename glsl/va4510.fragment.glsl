...
void main( void ) {
	...
	vec2 p = position*.5*mouse.x*2.1;
	float atan = atan(p.y,p.x);
	float dist = length(p);
	
	float dist2 = dist - (cos(atan*3.+time*2.7)*.5*cos(time)+cos(atan*5.+time*.2)*.25*sin(time)+1.);
	
	float mask = max(1.-abs(dist2)*3.,0.);
	
	
	color = color * (1.-mask) + (cos(time+dist2*vec4(20.,15.,30.,0.))*.5+.5)*mask;
