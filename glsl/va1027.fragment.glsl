...
const float rsteps = 64.0;

const float light_x = 0.0;
const float light_y = 0.0;

const float light_power = 0.9;
const float light_length = 0.5;
...
vec4 calc_all_effects(vec2 dis)
{
	float scale = 2.0; //sin(0.3 * time) * 8.0 + 8.25;

	vec2 position = vec2((gl_FragCoord.x / resolution.x) - 0.5, (gl_FragCoord.y / resolution.y) - 0.5) * scale + mouse * 32.0 + dis;
...
//	position = rotated_position;
	...
	else if (effect_number==1.0)
	{
		...
		gradient = (color.r + color.g + color.b) / 3.0;
	}
...
if (gradient <= 0.1) gradient = length(centered_coord) * 0.25;
	...
	return vec4(color,gradient);
}

void main(void)
{
	vec2 screen_position = vec2(gl_FragCoord.x / resolution.x, gl_FragCoord.y / resolution.y) - vec2(0.5);
		
	float dis = 0.005;
	
	vec4 col00 = calc_all_effects(vec2(0.0));
	vec4 col10 = calc_all_effects(vec2(dis,0.0));
	vec4 col01 = calc_all_effects(vec2(0.0,dis));
	...
	vec2 light_coord = vec2(0.0);
	vec2 disp_coord = screen_position - vec2(dx, dy);
	float gradient = 0.02 / pow(length(disp_coord - light_coord), 2.0);

// ==================================

	vec2 tex_coord = vec2(gl_FragCoord.x / resolution.x, gl_FragCoord.y / resolution.y);
	vec2 centered_tex_coord = tex_coord - vec2(0.5);
	vec2 lighta_coord = vec2(light_x * light_length, light_y * light_length);

	vec4 rgb0 = col00 * gradient;

	vec4 sum = vec4(0.0);
	for (float i=0.0; i<=rsteps; i++)
	{
		vec2 sample_tex_coord = vec2(mix(centered_tex_coord, (centered_tex_coord - lighta_coord) * (1.0 - light_length), i / rsteps));
		vec2 displacement = sample_tex_coord - centered_tex_coord;
		sum += texture2D(backbuffer, tex_coord + displacement) * light_power;
	}

	gl_FragColor = mix((vec4(vec3(sum / rsteps), 1.0)),rgb0, 0.5);
