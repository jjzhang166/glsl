...
vec4 wave(vec2 pos)
{
	...
	if(a>0.5) return vec4(0.7,0.72,0.7,1.0) * grid(4.0*pos, 2.0*a) + vec4(0.585,0.52,0.5,1.0)*grid(7.5*pos.yy, 0.25*a);
	else return vec4(0.3,0.33,0.3,1.0) * grid(4.0*pos, 2.0*a);
}
