...
float makePoint(float x,float y,float fx,float fy,float sx,float sy,float t){return 1./sqrt((x+sin(t/2.*fx)*sx)*(x+sin(t/2.*fx)*sx)+(y+cos(t/2.*fy)*sy)*(y+cos(t/2.*fy)*sy));}
...
void main( void ) {
   ...
   gl_FragColor = mix(vec4(d.x,d.y,d.z,1.0), texture2D( backbuffer, gl_FragCoord.xy/resolution.xy), 0.95) - 0.012;
}