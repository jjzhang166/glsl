...
float supershape(vec2 p, float m, float n1, float n2, float n3, float a, float b, float s, float r) {
	...
	return 1. - step(smoothstep(.0,2.5*sin(time*1.5)+3., (v)+.5) * s * resolution.y, length(p * resolution)); 
...
void main( void ) {
	...
	//gl_FragColor.r *= rand(p * cos(time));
	//gl_FragColor.g *= rand(p * sin(time));
	//gl_FragColor.b *= rand(p * tan(time));
}
