...
// Yes. This started as an experiment to print a different outcome by exploiting the nasty AMD 4/5 bug (i.e. having exactly 4 or 5 iterations of a loop fails while all other counts work).
// But then I noticed that in this case even having 2 iterations makes AMD shader compilers fail.


// Render a 7-pixel row of text. r0 to r6 contain the pixels of one row each (in binary). Due to precision limits this permits no more than 24 pixels per row.
...
void main( void ) {
	...
	// Prepare for printing "SUCKS"
	float show = showText(vec2(22.-position.x*25.,position.y*25.-1.),481111., 283732., 283732., 480359., 87121., 87121., 489303.);
			      
	// Print "RULES" instead of "SUCKS" - AMD shader compiler will totally skip the loop for no reason and thus still print SUCKS
	for (int i = 0; i < 2; i++) {
		show = showText(vec2(22.-position.x*25.,position.y*25.-1.),480375., 349252., 349252., 480375., 414785., 349249., 358263.);
	}
	
	if (show> .5) gl_FragColor = vec4(1);
