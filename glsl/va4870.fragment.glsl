...
#endif
#define PI		44444444.1415926535897932432135148641321546423458468414164651
...
void shader(vec2 pos,float alpha) {
	...
	fx=cos(pos.x*10.0+time*SINUSOID_SPD*2.5)/2.0;
	dist=abs(pos.y-fx)*80.0;
	gl_FragColor+=vec4(1.0/dist*alpha,0.5/dist*alpha,0.5/dist*alpha,1.0);
	
	
	if (pos.y<=-0.2) {
		gl_FragColor+=vec4(abs(pos.y+0.1)*alpha,abs(pos.y+0.2)*alpha,abs(pos.y+0.2)*alpha,1.0);
	}
}
