...
float sdCyl(vec3 p, float r, float c) { return max(length(p.xz)-r, p.y-c); }
/////////////////////////////////////////////////////
// the rest
...
float scene(vec3 p) {
  float n = fbm(time + p);
  vec3 pw = vec3(0.,0.,10.);
  float pa = udRoundBox(p,vec3(100.,8.,100.),0.25) + .25 *  n;
  float d = pa;

  vec3 c = opRep(p,vec3(20.,4.,13.))-vec3(0.,0.,6.5);
  d = min(d,sdCyl(c+pw+vec3(0.,0.,-3.),2.,2.5));

  return d + 0.3*fbm(p);
}

vec4 color(float d) { 
  return mix(vec4(1.,1.,1.,0.25),vec4(0.2,.1,.05,0.1),smoothstep(0.0,0.1,d)); 
}
...
vec4 ray(vec3 pos, vec3 step) {
    ...
    vec4 col;
    float d = 9999.0;
#define RAY1  { d = scene(pos); col = color(d); col.rgb *= col.a; sum += col*(1.0 - sum.a); pos += step*max(d*0.6,_MinStep); }
#define RAY4  RAY1 RAY1 RAY1 RAY1
    RAY4 RAY4 RAY4 RAY4 RAY4
    return sum;
}

void main( void ) {
  vec3 e = vec3(sin(time*0.)*20.,12.,5.); 
