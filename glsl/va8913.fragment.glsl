// by @jimhejl
// 
// added gamma transform & filmic tonemap
//
...
void main( void ) 
{
    ...
    vec2 t = mix(vec2(0.3),vec2(1.0),vec2(sin(time),cos(time)));
    ...
    float d2  = sdCapsule(vec3(pos.xy,0.),vec3(0.8,-0.5,0.0), vec3(0.8,0.3,0.0),.2);
    vec3 clr3 = c1 * flare(d2,6.3,0.8) + flare(d2,3.7,0.08);  

    vec3 vOut = clr1+clr2+clr3;
    vOut *= vOut;
	
    gl_FragColor = vec4(ToneMapFilmicALU_HEJL(vOut),1.);
}