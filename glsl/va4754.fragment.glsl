...
void main( void ) {
	...
	float closestDist = 0.02;
	int closestId = -1;
	
	for(int i = 0; i < 6; i++)
	{
		vec2 delta = points[i]- position;
		float dist = sqrt(dot(delta, delta))/5.;
		if(i == 4) dist *= 0.5;
		if(i == 2) dist = (abs(delta.x) + abs(delta.y))*0.05;
		if(i == 5) dist = max(abs(delta.x), abs(delta.y))*0.08;
		if(i == 0) dist = sqrt(dot(delta, delta))/10.+0.009;
		if(closestDist > dist)
		{
			closestDist = dist;
			closestId = i;
