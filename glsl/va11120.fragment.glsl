...
float scene(vec3 p)
{
	float d = flr(p, -5.0);
	d = min(d, flr(p, 5.0));
	d = min(d, sph(p, vec4( 0,-2, 15. * sin(time), 1.5)));
	d = min(d, sph(p, vec4(-8, 0, 20, 2.0)));
	d = min(d, sph(p, vec4(-5, 4, 15. * sin(time), 0.5)));
	d = min(d, sph(p, vec4(-1, 3, 15, 2.0)));
	d = min(d, sph(p, vec4( 2,-3, 15. * sin(time), 0.5)));
	d = min(d, cly(p, vec4(10, 0, 20, 1.0)));
	d = min(d, cly(p, vec4( 4, 0, 15, 1.0)));
	d = min(d, cly(p, vec4( 0, 0, 20. * sin(time * .1), 1.0)));
	d = min(d, cly(p, vec4(-2, 0, 25, 1.0)));
	d = min(d, cly(p, vec4(-6, 0, 30. * sin(time / 2.), 1.0)));
...
void main()
{
	...
	vec3 lp[3],lc[3];
	lp[0] = vec3(-4,0.0,4.0);
	lp[1] = vec3(2.0 * sin(time),.0,8.0);
	lp[2] = vec3(4,-2,24.0);
	lc[0] = vec3(1.0,0.5,0.4);
	lc[1] = vec3(0.4 * sin(time) ,0.5,1.0 * sin(time));
