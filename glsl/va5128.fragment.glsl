...
int countNeighbors(vec2 p) {
    ...
    for (int x=-KERNEL_R; x<=KERNEL_R; ++x) {
        ...
        if ( texture2D(backbuffer, uv+spoint).r > 0.0 )
            ++count;
    }
...
// int countNeighbors(vec2 p) {
    // int count = 0;
    // vec2 spoint = vec2(0.0);
    
    // spoint = uvUnit*vec2(1.0, 0.0);
    // if ( texture2D(backbuffer, uv+spoint).r > 0.0 ) ++count;
    
    // spoint = uvUnit*vec2(-1.0, 0.0);
    // if ( texture2D(backbuffer, uv+spoint).r > 0.0 ) ++count;
    
    // spoint = uvUnit*vec2(0.0, 1.0);
    // if ( texture2D(backbuffer, uv+spoint).r > 0.0 ) ++count;
    
    // spoint = uvUnit*vec2(0.0, -1.0);
    // if ( texture2D(backbuffer, uv+spoint).r > 0.0 ) ++count;
    
    // spoint = uvUnit*vec2(1.0, 1.0);
    // if ( texture2D(backbuffer, uv+spoint).r > 0.0 ) ++count;
...
float gameStep() {
    isLive = texture2D(backbuffer, uv).r > 0.0;
