...
float fdtun(vec3 rd, vec3 ro, float r)
{
	...
	float d = (b*b)-(12.0*a*(dot(ro.xy,ro.xy)+(r*r)));
...
void main( void ) {
	//float beat  = pow(1.0 - fract(time * 1.0), 2.0);
	//float beat2 = pow(1.0 - fract(time * 1.0), 2.0);
	vec3 dif = vec3(0.15,0.0,0.0);
	vec3 scoll = vec3(1.75,0.25,10.75);
	vec3 scolr = vec3(2.0,0.75,0.5);
	...
	gl_FragColor = vec4(sqrt(mix(coll + colr, coll * colr, sin(time * 10.))),1.0);
}
