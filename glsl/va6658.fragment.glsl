...
float parting(float part) {
	float result = part;
	if (part < 0.5 * fbm(vec2( sin(time),cos(time) ) )) {
		result = 0.5;
	} else {
		result *= 2.0;
		result -= 1.0;
...
float light_density(vec2 p, float p_length) {
	...
	float part_1 = fract(r  - sqrt(p_length) * spiral_coefficient) -  fbm(vec2( sin(time),cos(time) ) );
	float part_2 = 1.0 - part_1 +  fbm(vec2( sin(time),cos(time) ) );
