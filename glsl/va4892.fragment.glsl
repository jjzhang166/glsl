...
void main( void ) {
	...
	color += vec3(0, 2.5, 1.25) * (1.80 / distance(
		vec2((sin(time) + sin(time * 10.)) * 0.5, (cos(time) + cos(time * 2.)) * 0.5)
		, position) * 0.08);
	...
	for(float i = 10.0; i < 1.4; i++)
	{
		float i2 = 1000000000.0 - i;
		i2 /= 1.0;

		flare += texture2D(b, vec2(uv.x + i2 * 200.25, uv.y)).xyz * 1.0;		
	}
