...
#define skyColor vec3(0.6,0.8,1.0)
...
void applyFog( in float distance, inout vec3 rgb ){
    ...
    float fogAmount = (1.0 - clamp(distance*0.015,0.0,1.0) );
...
{
	float t = time/16.0;
	float h = 15.0 + cos(t)*10.0;
    float d =  SphereDistance(position, vec3(0.0, h, 1.0), 5.0);
	return d;
}
...
{
    screenPos.y -= 0.3;
    screenPos *= vec2(PI*1.0,PI*1.0/ratio)/fovy;
    ...
    position = vec3(0.0, 5.0, time/4.0);
...
void main(void)
{
    ...
    {
	    float t = time/4.0;
        vec3 lightpos = vec3(50.0 * sin(t*0.01), 10.0 + 40.0 * abs(cos(t*0.01)), (t) + 100.0 );
		...
		float m = clamp(hitPosition.y/5.0, 0.0, 1.0);
          mtlColor = mix(shadowColor*3.0, mtlColor*0.5, 1.0-m);
        }
        hitColor = mix(shadowColor, mtlColor, 0.4+shadow*0.6);
        vec3 hitNormal = ComputeNormal(hitPosition, 0);
        float AO = AmbientOcclusion(hitPosition, hitNormal, 0.55, 5.0);
