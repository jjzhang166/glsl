...
//CBS - Added perspective
//CBS - Included rotation and coloring from previous version
...
vec3 galaxy(vec2 p) {
	...
	float c2 = max(1.-length(p * vec2(.25, 3.))*2., 0.);
	vec3 core = c*c * vec3(2., 1., 1.) + c2*c2 * vec3(.2, .5, 2.);
	
	return star_distribution(result, p_length)+core;
}
...
void main( void ) {
	...
	position.y+=mouse.y;
	position.x+=mouse.x;
	vec2 coord= vec2((position.x-0.5)/position.y,1.0/(position.y+0.2))-vec2(0.5, position.y);
	
	//Rotation
	float fc = cos(time);
	float fs = sin(time);
	vec2 pp = vec2(fc * coord.x - fs * coord.y, fc * coord.y + fs * coord.x);
