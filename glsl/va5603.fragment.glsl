...
#define F2 0.35403
#define G2 .29324865
#define K .000343902439 // 1/41

// Permutation polynomial
float permute(float x) {
  return mod((300.0 * x + 1.0)*x, 23.0);
}
...
float srdnoise(in vec2 P, in float rot, out vec2 grad) {
  ...
  vec3 t = max(0.85 - vec3(dot(v0,v0), dot(v1,v1), dot(v2,v2)), 0.0);
  vec3 t2 = t*t;
  vec3 t4 = t2*t2;
...
vec2 torus_mirror(vec2 uv){
	return vec2(1.)-abs(fract(uv*.35)*2.4-1.0);
}
...
void main( void ) {
	...
	vec2 mousePolar = vec2((mouseW), (mouseW));
	vec2 offset = 0.25 + complex_mul((sin(time/100.)-0.5)*vec2(-1.,1.)*aspect, mousePolar)*aspect*8. ;
