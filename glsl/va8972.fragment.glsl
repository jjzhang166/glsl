...
vec3 getSurfacePosition(vec2 coord) {
	...
	for (int k=0; k<15; k++) {
		index += 1.13;
		Sphere_center = vec3(screen_center.x + mov_radius * sin(2.0*time*(1.0/index)),
				     screen_center.y + mov_radius * cos(2.0*time*(1.0-(1.0/index))*1.013),
				     0.0);
