...
void main( void ) {
	...
	float t3 = t2*sin(cos(position.x*15.13+position.y*42.0)*cos(position.y*20.23+position.x*3.0)*6.0*t+time) * 0.5 + 0.5;
	
	gl_FragColor.g = t;
	
	
	gl_FragColor.g *= 1.0 - t * t2;
	gl_FragColor.g *= 1.0 - t * (1.0-t2);
	gl_FragColor.b = t*t;
	gl_FragColor.r = t3*t3*t3*t3*t3*t;
}