...
// power value animation + zoom added by @ccliffe :)
...
float PI=3.14159265;
#define Power (8.0+cos(time*0.5)*6.0-sin(time*0.3)*4.0)
#define Bailout 4.0
...
void main(void){
  vec2 vPos=-1.0+2.0*gl_FragCoord.xy/resolution.xy*0.5+0.5;

  //Camera animation
  vec3 vuv=vec3(0,1,0);//Change camere up vector here
  vec3 vrp=vec3(0.0,0.0,0.0); //Change camere view here
...
// vignette
gl_FragColor = vec4( mix( gl_FragColor.rgb, vec3( 0.0 ), 0.5*dot( position - 0.5, aspect * (position - 0.5)) ), gl_FragColor.a );
