...
float dots(vec2 pos)
{
	return step(0.1, fract(pos.x * resolution.x / 20.0)) + step(0.8, fract(pos.y * resolution.y / 100.0));
}
...
void main( void )
{
	...
	gl_FragColor = vec4( vec3(dots(distort1(distort1(distort1(position.xy))))), 1.0 );
}