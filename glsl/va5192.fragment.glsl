...
void main() {
	...
	float filter = smoothcircle(uv_distorted, 0.2, 128.);
	uv_distorted = complex_mul(((uv_distorted - 0.5)*mix(16., 4., filter)), mousePolar) + offset;

	vec2 p = torus_mirror(uv_distorted);
