	...
	vec4 subScatterFS(in Intersection isect)
	{
		...
		float eps  = 0.0001;
		float RimScalar = 0.15;
		float MaterialThickness = 0.5;
		vec3 ExtinctionCoefficient = vec3(0.1,0.3,0.1);
		...
		finalCol.rgb += vec3(blinnPhongSpecular(wNorm,lVec,10.0) * attenuation * SpecColor * finalCol.a * 0.05);
