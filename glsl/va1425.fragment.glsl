...
void main( void ) {
	...
	vec2 screenPosNorm = gl_FragCoord.xy / resolution.xy+.005*vec2((sin(time*.05),cos(time*.05)));//some movement to the feedback._gtoledo3
	vec2 position = screenPosNorm + mouse / 4.0;
	vec4 prevColor = texture2D( backbuffer, screenPosNorm );
	
	const float num_iter = 3.0;
	const float cursor_size = 23.0;//gtoledo3
	...
	perlinval  += max(0., .99999 * (1. - (length((screenPosNorm - mouse) ) * cursor_size)));//feedback write._gtoledo3
	perlinval  += max(0., .9999 * (1. - (length((screenPosNorm - vec2(sin(time*.2)*.1+.5,cos(time*.2)*.1+.7)) ) * cursor_size+.2)));//feedback write._gtoledo3
	perlinval  += max(0., .9999 * (1. - (length((screenPosNorm - vec2(sin(time*.5)*.1+.5,cos(time*.5)*.1+.3)) ) * cursor_size+.2)));//feedback write._gtoledo3
	vec3 colorLight = vec3(255.0, 255.0, 169 ) / 255.0;
	vec3 colorDark  = vec3(40.0, 64, 84. ) / 255.0;
	
	float vignette = 1.5-length(screenPosNorm*2.0-1.0);
	
	vec3 curr_color = vec3(mix(colorDark, colorLight, pow( perlinval,5.0) )) * vignette;//let's make a weird snow blackhole look._gtoledo3
	gl_FragColor = vec4( mix(prevColor.xyz, curr_color, 0.09), 1.0 );
