...
void main( void ) {
	...
	float noise = length(fractnoise((time+time*1.1)*block + vec2(6.001, 5.0), 725., 1.4));
	noise = fract(3.25*noise*modulus);	//block
	...
	gl_FragColor = 2.*result - buffer - .0019 * signal.z;
