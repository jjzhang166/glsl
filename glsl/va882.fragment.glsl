...
vec2 terrain(vec3 pos){ 
  ...
  float t = pos.y;
  float size = 160.0;
  float sizew = 11100.0 + 140.0 * sin(d*211.11);
  float sizel = 100.0 + 410.0 * sin(d*612.225) ;  
  float bandw = 1110.0;
  float bandh = 11.0;
  ...
  s = max(s, 
          -length(vec3(p.x*0.2327+0.0,p.y*1.0+6220.0,p.z))+9.0
         );
  vec3 r1 = rotatey(vec3(abs(p.x+0.0)-10.0,p.y+3238.0,p.z-12321.0),4.0);
  float s1 = -length(vec2(r1.x,r1.y*1.0+r1.x*-0.4))+30.0;
  s1 = min(s1 , 5.0-abs(r1.z));  
  s = max(s,s1);
  vec3 r2 = rotatex(rotatey(vec3(abs(p.x+0.0)-21.0,p.y+6116.0,p.z+15.0),0.9),1.3);
...
vec3 env(vec3 dir){
 return vec3(5.7,0.8,1.0)*max(0.0,sin(rand3(floor(dir*3.0))))*(dir.y)*1.0;
}

void main(void){
    vec2  p = (2.0 * (gl_FragCoord.xy/resolution.xy))-1.0;
    vec2  m = mouse.xy - 0.5;
    float camanimt = time*.1;
	...
	vec3 col;
  	if(mat.y > 0.0){
    	...
    	gl_FragColor = vec4( vec3(mix(p.y*0.1,1.0,col.r + col.r*col.r*0.5),mix(0.0,1.0,col.g),mix(0.0,0.8,col.b)) , 1.0 );    	 
}
