...
// ptB must be between A and C
bool areCollinear( vec2 ptA, vec2 ptB, vec2 ptC )
{
	highp float AB = distance( ptA, ptB );
	highp float BC = distance( ptB, ptC );
	highp float AC = distance( ptA, ptC );
	//return abs(AB + AC + BC - 2.0 * max( max( AB, AC ), BC )) <= .0; // This line breaks everything, and simultaneously makes it beautiful. I wish I knew why.
	//return abs( AB + AC - AC ) < .110;
	return abs( distance( ptA, ptB ) + distance( ptB, ptC ) - distance( ptA, ptC ) ) < .110; 
}
