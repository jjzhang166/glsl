...
vec3 intersectPlane(vec3 ro, vec3 rd, vec4 plane, out bool hit)
{
...
void main(void)
{
    ...
    float zoom = 1.0; // 5.0*surfaceSize.y;
    ...
    //ro += rd*20.0;
    bool hit = true;
    vec3 p1 = intersectPlane(ro, rd, vec4(0.0, -1.0, 0.0, 3.0), hit);
    vec3 p2 = intersectPlane(ro, rd, vec4(0.0, -1.0, 0.0, 1.0), hit);
	
    ro = p1;
    float stepSize = length(p2 - p1) / float(_VolumeSteps);
    rd = -rd;
