...
void main( void ) {
	...
	for(int n=0; n<2; n++)
	{
		vec2 localPos = resolution / 12.0;
		localPos.y = resolution.y/2.0*sin(time+float(n))/72.0+resolution.y/1.5+float(n)-radius*float(n)*2.0;
		localPos.y *= (cos(gl_FragCoord.x/3350.0*(float(n)+12.0))/120.0);
		localPos.y += resolution.y/1.52 + float(n)*radius;
