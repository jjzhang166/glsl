...
float f(vec3 o)
{
    float a=(sin(o.x)+o.y*.980 )*.2 ;
    o=vec3(cos(a)*o.x-sin(a)*o.y,sin(11.0*a)*o.x+cos(a)*o.y,o.z);
    return dot(cos(o)*cos(o),vec3(1)) + 0.1*sin(5.0*time) - 0.3;
}
...
vec3 s(vec3 o,vec3 d)
{
    ...
    vec3 p=o+d*t;
    vec3 n=-normalize(vec3(f(p+e),f(p+e.yxy),f(p+e.yyx))+vec3((sin(p*2.)))*.01);

    return vec3( mix( ((max(-dot(n,vec3(.577)),0.) + 0.125*max(-dot(n,vec3(-.707,-.707,0)),0.)))*(mod

(length(p.xy)*10.,1.)<1.0?vec3(.71,.85,.25):vec3(.79,.93,.4))
...
void main()
{
    vec2 p = -1.0 + 2.0 * gl_FragCoord.xy / resolution.xy - 4.0*(mouse-vec2(0.5,0.5));
