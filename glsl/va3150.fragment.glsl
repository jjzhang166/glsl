...
//mo improv.-gt
float f(vec3 o){	
	float a=(cos(o.x-o.y))*.3;
	o=vec3(sin(a)*o.x,cos(a)*o.y,o.z+(sin(time*.1)+1.5));
	return dot(cos(o)*cos(o),vec3(1.0))-1.;
}
...
vec3 s(vec3 o,vec3 d){
	...
	for(int i=0;i<450;i++){
		if(f(o+d*t)<0.01){
			...
			vec3 e=vec3(.1,.1,.3),p=o+d*t,n=-normalize(vec3(f(p+e),f(p+e.yxy),f(p+e.yyx))+vec3((sin(p*3.14)))*.01);
			return vec3(mix(((max(dot(n,vec3(.3)),-1.) + 0.125*max(dot(n,vec3(.1,.1,0)),0.)))*(mod(length(p.xy)*1.2,1.)<1.0?vec3(.1,.1,.5):vec3(.79,.3,.4)),vec3(1.3,.1,.1),vec3(pow(t/83.,1.))));
		}
		t+=.23
	;
	}
	return vec3(1.1,1.1,0.0);
}
...
void main(){
	...
	gl_FragColor=vec4(s(vec3((cos(time*.1)*.5)-lum,(sin(time)*.10)+lum,time+lum), normalize(vec3((gl_FragCoord.xy-vec2(resolution/2.))/vec2(resolution.x),.23))),1.);
}