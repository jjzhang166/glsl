...
// Normals + more color for tiles // Light needs to be reprojected for tiles-- Original Proc-Tiles author

// Don't mind me, just added some obviously flawed parallax mapping and slightly tweaked (still faked) lighting (fernozzle)
...
void main( void ) {
	...
	vec3 light = normalize(vec3(1.,0.1,1.))*light_intensity;

	vec2 uv = ( gl_FragCoord.xy / resolution.xy );  // this is the pixel coords
	sph1.xz = vec2 ( 0.5 * cos( time ),  0.5 * sin( time )+2.5); // move sphere around	
	vec3 ro = vec3( 0.0, 1.8, 8.0 ); //ray origin
	vec3 rd = normalize( vec3( -1.0 + 1.0*uv* vec2(resolution.x/resolution.y, 1.0), -1.0 ) ); // ray destination
	float t;
        vec3 col = vec3(.3);
	...
	if ( id>0.5 && id<1.5 ) {			// sphere		
		...
		float offset = voronoi(p);
		
		p.y += pow(offset, 1.7)*1.4;
		float color = voronoi(p);
		vec3 vnorm = normal(p);
		float shade = dot(light,vnorm)+0.5;
		col.rgb *= vec4(vec3(color*shade) * tile_color, 1.0).rgb;
		float darkfac = 1. - length(col);
		darkfac = pow(darkfac, 10.);
		darkfac *= 0.1;
		col += vec3(0.1, 0.15, 0.24) * darkfac;
	}		
