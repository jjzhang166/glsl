...
vec2 map( vec3 p ){
	...
	float tor1 = Torus(p1, vec2(0.02, 0.001));
	vec3 p2 = p;
	p2 = rotXaxis(p2,time );
	float tor2 = Torus(p2, vec2(0.02, 0.001));
	
	vec3 p3 = p;
	p3.x -= 0.0;
	p3 = rotZaxis(p3, p.z * 30. + time*.2);
	p3 = rotYaxis(p3, time * .2);
	p3 = rotZaxis(p3, time * .4);
	float box1 = Hexagon(p3, vec2(0.02,0.03));
	box1 = max(box1, -tor2+0.004);
	box1 = max(box1, -tor1+0.004);
	box1 += abs(sin((p3.x+p3.z+p3.y)*1610.)*0.0001);
