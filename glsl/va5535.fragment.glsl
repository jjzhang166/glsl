...
float GetDistanceToScene( vec3 P)
{
	...
	for (int i = 0; i < 9; i++)
	{
		float f2 = pow(float(i+3),.2);
		O = f2 * O;
		f = f2 * f;
		O.xz *= -1.;
		O = sopPolarRepeatY(O,float(i+3));

		O = sopRotateZ(O, fract(time*.05)*4.);

		O.x -= -0.515;
		O.y -= 1.0;

	}
	return (length(O)-1.345)/f; // Tweak: /f added. (If you scale space you should scale distances as well. This works much better than the previously used approximation.)
}
...
void main(void){
  ...
  // General parameter setup
   ...
   //Camera setup
	Camera cam;										// Make a struct holding all camera parameters
  	cam.lookAt = vec3(0,0.7,0);								// The point the camera is looking at
	cam.position = vec3(sin(t)*2.7, 2, cos(t)*2.7);						// The position of the camera
  ...
  //Raymarching
  	const float MIN_DISTANCE = 0.001;								// Distance to scene that we will count as a hit
	const float MAX_DEPTH=64.0;								// Distance from camera that we will count as a miss
	const int MAX_STEPS = 35;								// Maxmimum amount of ray marching steps before counting the ray as a miss
	
	float distanceFromCamera = 0.0;								// The ray starts at the camera, so 0
	
  	vec3 raymarchPosition = cam.position+cam.rayDir*distanceFromCamera;			// Variable holding the current position along the camera ray
  ...
  //Lighting
	...
	const vec3 e=vec3(0.0001,0,0);								// Epsilon value, used to calculate the hit normal
	
  	if (distanceFromCamera<MAX_DEPTH){							// If this is true, it means we hit something
    		diffuseColor = vec3(0.5, 0.8, 0.2);						// Set diffuse color for cube to red
