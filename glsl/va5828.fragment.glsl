...
float GetDistanceToScene( vec3 p){
  ...
  float a2 = asin(p.y/(distance(vec3(p),vec3(0.0))));
	 
  float d = length(p.xz);
  float indnt = min(tri(a*8.),tri(a2*8.));
	indnt = 1.-indnt;
	indnt = clamp(indnt,0.9,1.);
  float sphere = length(p)-2.-indnt;
...
void main(void){
  ...
  // General parameter setup
   ...
   //Camera setup
	vec2 ca = vec2(surfacePosition/4.);
	float cz = surfaceSize.x+1.5;
	Camera cam;										// Make a struct holding all camera parameters
  	cam.lookAt = vec3(0,0,0);								// The point the camera is looking at
	cam.position = vec3(cos((ca.x))*cz, sin((ca.y+1.))*cz, sin((ca.x))*cz);						// The position of the camera
