...
#define SPINNING_CAMERA 01
#define SPINNING_CAMERA_W_MOUSE 21
#define TEST_CAMERA 2

const int SHADING_MODE = RAYMARCH_SHADING; 
...
// Fog
vec3 applyFog (in vec3 currColor, in vec3 ray){
...
vec2 obj_manyBoxes(in vec3 p){
	...
	vec3 c = vec3(10,0,10); // how close cubes are to each other
	vec3 q = p;
...
vec3 color_floor(in vec3 p)
{
return vec3(0.5,0.5,0.5);
}

// White color
vec3 color_block(in vec3 p){
...
void main(void){
	...
	//Colors
	const vec4 skyColor = vec4(0.1, 0.1, 0.2, 1.0);
	...
	if (totalDist<MAX_DEPTH){
		...
		else if(dist.y==1.0) // building color
		{
			...
			float hue =   (fract(p.y*4.0) );
			c = hsv2rgb(0.6+hue, 0.3*v, v*v);
		}
