...
vec4 encodeColor(vec3 n) {
	return vec4(n, 1.0) / max(max(1.0, n.x), max(n.y, n.z));
}
...
void main() {
	...
	if (samples < kSPP) {
		...
		color = (samplesNext < kSPP) ? encodeColor(color.xyz) : vec4(pow(color.xyz, vec3(1.0 / kGamma)), 1.0);
	}
