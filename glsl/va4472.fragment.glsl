...
// 4472.0:  Added nice back buffer effect http://glsl.heroku.com/e#4427.20
// 4472.9:  Added mouse control for extra fun
...
void main( void ) {
	...
	vec3 curr_color = (tc*0.2) + (tc2*0.3);
	
	
	//Let's draw some stars
	
	float scale = sin(0.3 * time*10.0) + 15.0;
	vec2 position2 = (((gl_FragCoord.xy / resolution) - mouse) * scale);
	float gradient = 0.0;
	vec3 color = vec3(0.0);
	float fade = 0.0;
	float z = 0.0;
 	vec2 centered_coord = position2;
	
	for (float i=1.0; i<=120.0; i++)
	{
		vec2 star_pos = vec2(sin(i) * 250.0, sin(i*i*i) * 250.0);
		float z = mod(i*i - 10.0*time*10.0, 256.0);
	...
	curr_color += gradient;
	curr_color += texture2D(bb, position).xyz * 0.85;
