...
void main(void){
  ...
  float f=1.0,g=1.0;
  for(int i=0;i<500;i++){
    if (abs(s.x)<.007||f>MAX_DEPTH) break;
    f+=s.x;
    g+=l.x;
    p=E+scp*f;
    m=E+scp*g;
...
n=normalize(
  ...
  float t=1.0;
  vec3 u;vec2 y = vec2(0.1,0.0);
  for(int Z=0;Z<300;Z++){
    if (abs(y.x)<.007||t>MAX_DEPTH) break;
    t+=y.x;
    u=E+scp*t;
    y=inObj(u);
  }
  ...
  if (f<MAX_DEPTH){
    ...
    if (s.y==0.0)
      ...
      c=obj_roundBox_c(p) *1.0 + (obj_floor_c(m+n)*(1.0-g*.03)*(g>MAX_DEPTH?0.0:1.0)) * 0.5 + 1.0 * ((y.y==0.0?obj_floor_c(u):obj_roundBox_c(u))*(1.0-t*.03)*(t>MAX_DEPTH?0.0:1.0));
    float b=dot(n,normalize(E-p));
    gl_FragColor=vec4((b*c+pow(b,300.0))*(1.0-f*.03),1.0);//simple phong LightPosition=CameraPosition
    if(s.y==1.0)gl_FragColor -= 1.0 - ao(p, n, 0.1);
    //if(s.y==1.0) gl_FragColor *= vec4(ao(p, n, -0.4));
    //gl_FragColor -= 1.0 - shadow(p, n);
    if(s.y==0.0)gl_FragColor *= softshadow(p, n, 16.0);
  }
