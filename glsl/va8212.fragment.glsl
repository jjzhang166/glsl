...
void main()
{
	...
	l += 0.8 * vec3(1.1, 1.1, 0.8) *
		torch(cp, cv * -0.2 + cu * 0.1, rotateY(cos(time * 2.0) * 0.03, normalize(cw + cu * -0.2)), rp, nearest, torch_o, torch_d);
	...
	gl_FragColor.rgb = 1.5 * mix(fog, col, exp(-nearest * 6.0e-1)) * v;
	
	gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(1.0, 1.0, 0.9) * (clamp(ti.x * 0.3, 0.0, 1.0) +
																	torchFlicker() * torchOn() * pow(clamp(ti.x - 0.2, 0.0, 1.0) * 2.0, 30.0)), thit);
	
	gl_FragColor.rgb *= 1.2;
	
	gl_FragColor.rgb += vec3(N(gl_FragCoord.xy) * 0.005);
	gl_FragColor.a = 1.0;
}