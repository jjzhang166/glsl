...
void main() {
	...
	vec2 uv = 0.5 + (gl_FragCoord.xy * vec2(1./resolution.x,1./resolution.y) - 0.5)*resolution;
	float mouseW = atan((mouse.y - 0.5)*aspect.y, (mouse.x - 0.5)*aspect.x);
	vec2 mousePolar = vec2(sin(mouseW), cos(mouseW));
	vec2 offset = (mouse - 0.5)*2.*aspect*1.;
	//offset =  - complex_mul(offset, mousePolar) +time*0.0;
	vec2 uv_distorted = uv;
	
	float filter = smoothcircle( uv - offset*resolution*aspect.yx, resolution.x/8., 0.1 + sin(time*8.*asin(1.))*0.09);
	uv_distorted = complex_mul(((uv_distorted - 0.5)*mix(4., 1./128., filter)/resolution), mousePolar) + offset;
