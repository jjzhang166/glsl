...
void main( void ) {
	...
	vec2 distanceVector = gl_FragCoord.xy;
	
	float d = 135.0;
	
	float newtime = time * sign(mod(distanceVector.x, d * 4.0) - d) * sign(mod(distanceVector.y, d * 2.0) - d);
	
	distanceVector = mod(distanceVector, d) - d / 4.0;	
	...
	cogMin += 20.0 * clamp(1.0 * (angleFoo + 0.2), 0.0, 0.915); // add 0.2 to make teeth a bit thinner, the 0.015 clamp controls the thickness of the cog ring
