...
vec3 Forest(vec3 p, float spacing) {
	...
	vec3 forest = mod(p, spacing)-spacing*.5;								// WHY DO I ONLY GET HALF A TREE?
												//- well, because the tree is centered at the origin and mod only gives values >= 0, thus discarding the parts of the tree < 0
...
void main(void){
  ...
  //Raymarching
	...
	const float MAX_DEPTH=146.0;								// Distance from camera that we will count as a miss
	const int MAX_STEPS = 60;								// Maxmimum amount of ray marching steps before counting the ray as a miss
