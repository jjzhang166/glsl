...
vec2 GetPolar(vec2 pos)
{
	...
	float ang= atan(relPos.x * time * time * time, relPos.y * time) / TwoPI + 0.5;
...
void main( void ) {
	...
	float ang= polCoord0.x + polCoord1.x * 80.0;
	float rad= polCoord0.y + polCoord1.y;

	rad= rad+sin(rad*10.0+time*1.7*time)*300.3*
		 sin(rad*1.0+time*0.9*time)*0.3;
...
//		c= 1.0;
	...
	gl_FragColor = vec4(2.*c,2.*c,1.-c,1); 
