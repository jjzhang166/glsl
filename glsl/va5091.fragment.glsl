...
vec2 DistCombineUnionSphere( const in vec2 v1, const in vec2 v2 )
{
	//if(v1.x < v2.x)
	//            return v1;
	//else
...
float GetBumpyFloorHeight(const in vec3 vPos)
{
	...
	return 0.2 * snoise(temp); //+ 0.1 * snoise(temp * 0.5);
}
...
{             
	...
	vec3 vSphere1Pos = vec3( 1.0 + sin(time), -0.3, cos(time));
        ...
        vec2 vResult = DistCombineUnionSphere(vDistSphere1, vDistSphere2);
