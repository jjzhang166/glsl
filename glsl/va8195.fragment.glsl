	...
	if((position.x <= 0.5 && position.x >= 0.0) && 
	   (position.y <= 0.5 && position.y >= 0.0)){
		if(position.x < position.y)
			...
			col = vec4(1.0, position.x * cos(time * 1.5) * 1.5, position.y * sin(time * 2.0) * 1.5, 1.0).bgra;
	}
	
	float radius = 0.3;
	vec2 center = vec2(-0.5, -0.5);
	if(length(position.xy - center) <= radius){
		float r = length(position.xy - center) / radius;
		col = r * vec4(0.0, 1.0, 0.0, 1.0) +  (1.0 - r) * vec4(1.0, 0.0, 1.0, 1.0);
