...
void main(void){
  ...
  vec3 U=vec3(0,1,0);//Camera Up Vector
	...
	float speed = time*3.0 + sin(time * 0.7) * 2.0;
	float my =  sin(time/8.0); // mouse.y*4.0;
	float camy = -0.3+my + sin(time * 0.04) * 0.2; 
	float tary = -1.0+my; 
  vec3 E=vec3(speed, camy, abs(speed) * 2.0);//vec3(-sin(time/4.0)*8.0,4,cos(time/4.0)*8.0); //Camera location; Change camera path position here
  ...
  float ang = sin(time * 0.1);
  scp.xy = vec2(cos(ang) * scp.x - sin(ang) * scp.y, sin(ang) * scp.x + cos(ang) * scp.y);
...
vec3 cc = vec3(0.7, c.g, c.b);
	...
	float m = smoothstep(3.0, 20.0, f);
	 c = mix(c,cc, 1.0-m);
c = vec3(abs(p.z - speed * 2. - 4.) < 0.1 ? 1. : 0., mod(p.y, 0.25) * 4. > 0.9 ? 1. : 0., 0.);
    ...
    gl_FragColor=vec4(b*c,1.0);//simple phong LightPosition=CameraPosition
  }
	else {
		float invy = 1.0-vPos.y;
		float r = invy*0.8;
		float g = 0.2 + invy*0.7;
		vec3 sky_clr = vec3(0.7,0.2+invy*0.3, invy*0.8);
		gl_FragColor=vec4( 0., 0., 0.,1.0); //background color
	}
  gl_FragColor = (0.5 * gl_FragColor + texture2D(backbuffer, gl_FragCoord.xy/resolution.xy) * 0.5);
}