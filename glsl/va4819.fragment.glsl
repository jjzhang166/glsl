...
{
    vec3 fp = floor(p), col = mix(vec3(sin(time*2.0)*0.5+0.5, 0.2, 0.2), vec3(1.0, 0.6, 0.6),
...
void main(void)
{
    ...
    for(int i = 0; i < sum_num; ++i)
    {
        ...
        vec3 ro = 0.0*normalize(vec3(cos(roty), cos(rotx), sin(roty)));//vec3(rand1(gl_FragCoord.xy + fi), rand2(gl_FragCoord.xy + fi), 0.0) * 0.06;
