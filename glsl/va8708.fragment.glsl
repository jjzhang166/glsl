...
float makePoint(float x,float y,float fx,float fy,float sx,float sy,float t){
   float anglex = tan(t*fx)*sy;
   float angley = tan(t*fy)*sx;
   float xx=x+angley;
   float yy=y-anglex;
   float xx0=x*2.0+anglex;
   float yy0=y*2.0-angley;
   float a=0.5/sqrt(abs(abs(y*xx)+abs(y*yy)));
   float b=0.5/sqrt(abs(x*xx+y*yy));
   float c=(xx0*sign(anglex)-yy0*sign(angley));
   float f = 1.0-smoothstep(0.0, 0.5, c*c);
   float color = mix(a*b, 0.0, f*c*c);
   return color;
}
