...
vec2 distanceField(in vec3 p){
  return ObjUnion(obj_floor(p),obj_unsignedBox(p));
}
...
// UNSIGNED BOX
vec2 obj_unsignedBox(in vec3 p){
	float height = rand(p.xz);
	return vec2(length(max(abs(p)-vec3(1,height,1),0.0)),1); //trying to make box heigh random...
}
...
void main(void){
  ...
  //vec3 E=vec3(-sin(spin)*4.0, 8.0 * mouse.y, cos(spin)*4.0);//Change camera path position here
	vec3 E=vec3(-sin(time/10.0)*8.0,4,cos(time/10.0)*8.0); //spinning scene
  ...
  vec3 dir=normalize(P-E); //normalized direction vector from Eye to point on screen
  ...
  float totalDist=0.0;
  vec3 c,p,n; //c=color, p=point, n=normal at any point on the surface
   
  int steps = 0;
  for(int i=0;i<MAX_STEPS;i++){
	  steps++;
    totalDist+=dist.x;
    p=E+dir*totalDist; // p = eye + total_t*dir
  ...
  if (totalDist<MAX_DEPTH){
	...
	// Choose between shading methods
	int PHONG_SHADING = 0;
	...
	if(PHONG_SHADING==1){
		...
		gl_FragColor=vec4((b*c+pow(b,8.0))*(1.0-totalDist*.01),1.0);
	}
	else{
