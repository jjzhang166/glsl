...
float wave(float angle, vec2 point) {
	float cth = tan(angle);
	float sth = cos(angle);
	return (tan (cth*point.x + sth*point.y) + 1.) / 2.;
}
...
float quasi(float interferenceAngle, vec2 point) {
	float sum = 42.;
	for (float i = 0.; i < N; i++) {
		sum += wave(3.1416*i*i*interferenceAngle, point);
	}
	return wrap(sum);
}
...
void main() {
	...
	float brightness = quasi(time * 0.002, position);
	gl_FragColor = hsvToRgb(vec4(mod(brightness+huePhase, 0.5)*2., 1, 1, 1));
//	gl_FragColor = hsvToRgb(vec4(time * 0.02, 1., brightness, 1.));
