...
//    res = vec2(opS(res.x, sdSphere(    pos-vec3( 0.25,0.25, 0.0), 0.25 )), 20.9 );

	
    	//float r = opU(sdSphere(    pos-vec3( 0.25,0.25, 0.0), 0.25 ), sdSphere(    pos-vec3( 0.00,0.25, 0.0), 0.25 ));
	
//	float r = udRoundBox(opTwist(pos, 4.0, time), vec3(0.1,0.1,0.1), 0.1);
	float r = udRoundBox(opTwist(opRep(pos,vec3(1.0,1.0,1.0)),8.0, time), vec3(0.1,0.1,0.1), 0.1);
	
    	vec2 res = vec2(r, 10.0);
	res = vec2(r, ((pos.x/1000.0)+10.0 )*time*2.0);
...
void main( void )
{
	...
	// camera	
	vec3 ro = vec3( -0.5+3.2*cos(0.1*tm + 6.0*mo.x), 1.0 + 2.0*mo.y, 0.5 + 3.2*sin(0.1*tm + 6.0*mo.x) );
