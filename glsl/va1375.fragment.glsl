...
vec3 rock(vec2 p) {
  ...
  const vec3 SkyColor = vec3( .3, .3, .3);
  const vec3 CloudColor = vec3(1., 1., 1.);
...
vec3 grass(vec2 p) {
  ...
  const vec3 SkyColor = vec3( .3, .7, .3);
  const vec3 CloudColor = vec3(.6, .9, .5);
...
vec3 snow(vec2 p) {
  ...
  const float sharpness = .2;
  const vec3 SkyColor = vec3( .8, .8, .8);
  const vec3 CloudColor = vec3(1., 1., 1.);
...
void main( void ) {
    ...
    float shading = pow((float(steps)/128.0) * 1.0, 3.0);
		    
    if (o.y != -1. && t<max_depth) {
        gl_FragColor = vec4(mix(terrainColor(ro, rd, t, p.y-o.x, o.y), vec3(0.0, 0.0, 0.0), shading), 1.);
        return;
    }
    ...
    gl_FragColor = vec4(mix(sky(pos), vec3(0.0, 0.0, 0.0), shading), 1.);
}