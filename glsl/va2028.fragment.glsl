...
void main( void ) {
	...
	r = ((r*2.0) + mod(gl_FragCoord.y * gl_FragCoord.x, (sin(time*0.01)*0.1)+0.3));
	
	gl_FragColor = vec4( r - refract(rd.y,normalize(rd.y)*1.2,1.0), r * refract(rd.y,normalize(rd.y)*1.2,1.0), r - refract(rd.y,normalize(rd.y)*1.2,1.0), 1.0 );
