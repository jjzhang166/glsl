...
float surface3 ( vec3 coord ) {
	...
	n += 0.01625    * abs( cnoise( coord * frequency * 64.0 ) );
...
void main( void ) {
	...
	float r = surface3(vec3(surfacePosition, time * 0.1));
	float g = surface3(vec3(surfacePosition, time * 0.2));
	float b = surface3(vec3(surfacePosition, time * 0.3));	

	if (g < .3) {
		gl_FragColor = vec4(r, g, b, 1.0);
	}  else {
		
		gl_FragColor = vec4(r+g+b/3., r+g+b/1., r+g+b/3., 1.0);
	}
}
