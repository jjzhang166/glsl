...
float r = resolution.x/(200.0+sin(time)*100.0);
float mbsize[5];
    mbsize[0]=r*100.;
	mbsize[1]=r*(200.0+sin(time*2.0)*100.0);
	mbsize[2]=r*(300.0+cos(time)*100.0);
	mbsize[3]=r*(400.0+sin(time/2.0)*200.0);
...
mbpos[4] = vec2(-cos(time * 0.33) * r*3. + sw, cos(time * 1.5) * r*2. + sh);
 ...
 for(int i=0; i<5; i++)
  ...
  gl_FragColor = vec4(dist/2.0,0,dist/1.8,0.2);
