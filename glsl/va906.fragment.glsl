...
void main(void)
{
	...
	for (float i=1.0; i<=528.0; i++)
	{
		...
		float z = mod(i*i - 32.0*time, 256.0);
		float fade = (256.0 - z) / 256.0;
		vec2 blob_coord = star_pos / z;
		float gradient = ((fade / 384.0) / pow(length(centered_coord - blob_coord), 1.5)) * (fade * fade);
		color += vec3(gradient * mod(i, 61.0)/61.0, gradient * mod(i, 27.0)/27.0, gradient * mod(i, 7.0)/7.0);
	}
