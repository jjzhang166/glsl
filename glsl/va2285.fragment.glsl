...
bool letterB(in vec2 pos) {
	return pos.x>.0 && pos.x<.6 && pos.y>.0 && pos.y<1.
		&& rect(pos, vec2(.0), vec2(.8, .1))
		//top horizontal b
		||rect(pos, vec2(.0, .9), vec2(.6, 1.))
		||rect(pos, vec2(.5, .5), vec2(.6, 1.))
		||rect(pos, vec2(.0, .5), vec2(.5, .6))
	
		||rect(pos, vec2(.0, .0), vec2(.1, 1.));
...
void main()
{
	...
	for (float i = .0; i < 10.; i += 1.) {
		...
		if (letterH(vec2(p.x+1.,p.y))
		    ...
		    || letterB(vec2(p.x-1.4,p.y))) { 
			color += alpha;
		}
	}
	gl_FragColor = vec4(mix(color, texture2D(backbuffer, gl_FragCoord.xy / resolution.xy).rgb, 0.7), 1.);
}