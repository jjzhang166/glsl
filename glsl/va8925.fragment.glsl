...
void main( void ) {
	...
	for( int i=0; i<21; i++) {
	  ...
	  tmp = radius * 3.0/length(sferePos - gl_FragCoord.xy);
	  tmp *= tmp;
	  accum += tmp * 0.000075;
	}

	tmp = radius * 13.0/length(mouse*resolution.xy - gl_FragCoord.xy);
	tmp *= tmp;
	accum -= tmp * 0.000075;

		
	gl_FragColor = vec4(
		accum<0.35?0.0:1.0-accum*1.0+0.2,
		accum<0.47?0.0:1.0-accum*0.5+0.05,
		accum<0.70?0.0:1.0-accum*0.00175+0.2, 1.0 );
}