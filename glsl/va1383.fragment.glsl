...
//Yay for smoke! by uitham
//added perlin func didn't compile on my mac, changed some stuff._gtoledo
...
void main( void ) {
	...
	vec2 position = gl_FragCoord.xy / resolution.xy*2.;//<more swirly!-gt
	//-mouse, bc I like it when the mouse follows (not being a smartass!)_gt
	position.x += snoise(vec3(vec2(position.x, position.y) - mouse, .5*time)) + (perlin().y)*1.5;//<--someone contributed this but left it as "+ perlin()". Doesn't compile on mac. Added extra operand too._gt
