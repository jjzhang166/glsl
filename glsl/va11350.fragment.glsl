...
//4d manifolds = more better  //sphinx
...
float map(vec3 p){	
	...
	p-=o;
	p = (p4*mat).xyz;
	float s0 = cube(p, vec3(.5), p4.xyz);
	float s1 = cube(p, vec3( 1024., .5, .5), p4.xyz);
	float s2 = cube(p, vec3(.5,  1024., .5), p4.xyz);
	float s3 = cube(p, vec3(.5, .5,  1024.), p4.xyz);
	
	return max(s0, -min(min(s1, s2), min(s2, s3)))+.001;
}	
