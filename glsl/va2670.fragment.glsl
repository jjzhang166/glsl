...
//Floor
vec2 obj0(in vec3 p){
  return vec2(p.y+3.0,1);
}
//Floor Color (checkerboard)
vec3 obj0_c(in vec3 p){
	vec3 pt = p;
	float twist = time*.4+p.y*.5;
	pt.x = p.x*cos(twist)+p.z*sin(twist);
	pt.z = p.x*sin(twist)-p.z*cos(twist);
	
	vec3 pr = pt;
	pr.x = pt.x*cos(time*.4)+pt.y*sin(time*.4);
	pr.y = pt.x*sin(time*.4)-pt.y*cos(time*.4);
...
void main(void){
  ...
  if (f<maxd){
    ...
    gl_FragColor=vec4((b*c+pow(b,8.0))*(1.0-f*.01),0.9);//simple phong LightPosition=CameraPosition
  }
