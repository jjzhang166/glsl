...
void main(void)
{
	...
	vec2 q = sin(rotate*(position+4.0*position*sin(time+position.yx*2.0)+tan(2.0*position*rotate*rotate))*3.0*(1.5+sin(time*0.5)/r));
	q=sqrt(abs(q)-0.1);
	...
	color.b *= abs(p2.x);
	color.r *= abs(p3.x);
	color = reflect(q.xyy,color);
	color.xy *= rotate;
	color.yz *= rotate;
	color = sqrt(abs(reflect(color,color.zxy)));
