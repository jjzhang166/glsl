...
{  
	return length(max(abs(point)-size, 0.0))-0.9;
}
...
{    
	return min(      
		CubeRepetition(position, vec3(0, 0.0, 20.0))      , 
		CubeRepetition(position+vec3(350.0,-2.0,0.0), 
					   vec3(23.0, 0.0, 23.0))    );
}
...
float Softshadow( in vec3 landPoint, in vec3 lightVector, float mint, float maxt, float iterations )
{
	...
	vec3 sphereNormal;    
	float t = mint; 
	//(mint + rand(gl_FragCoord.xy) * 0.01);    
	for( int s = 0; s < 100; ++s )    
	{        
		if(t > maxt) break;        
		float nextDist = min(BuildingsDistance(landPoint + lightVector * t ), 
...
{
	int dummy ;    
	return normalize(vec3(DistanceField( vec3(pos.x + epsilon, pos.y, pos.z), dummy ) - 
						  DistanceField( vec3(pos.x - epsilon, pos.y, pos.z), dummy ) , 
						  DistanceField( vec3(pos.x, pos.y + epsilon, pos.z), dummy ) - 
						  DistanceField( vec3(pos.x, pos.y - epsilon, pos.z), dummy ) , 
						  DistanceField( vec3(pos.x, pos.y, pos.z + epsilon), dummy ) - 
						  DistanceField( vec3(pos.x, pos.y, pos.z - epsilon), dummy )        
						 )
					);
}
...
void FishEyeCamera( vec2 screenPos, float ratio, float fovy, mat4 transform, out vec3 position, out vec3 direction )
{
	...
	direction = vec3(sin(screenPos.y+PI*0.5)*sin(screenPos.x)
					 , -cos(screenPos.y+PI*0.5)        
					 , sin(screenPos.y+PI*0.5)*cos(screenPos.x) );
	position = vec3(5.0*sin(time*0.21), 8.0, 1.0*time);
}
...
void main(void)
{
	float ratio = resolution.x / resolution.y; // position on the screen    
	vec2 screenPos;    
	...
	{
		vec3 lightpos = vec3(50.0 * sin(time*1.0), 
							 10.0 + 40.0 * abs(cos(time*0.01)), 
							 (time) + 100.0 );        
		...
		//material color        
		vec3 whtColor = vec3(1.0,1.0,1.0);          
		vec3 mtlColor = MaterialColor(material);          
		if(material == BUILDINGS_MTL)
		{
			whtColor = mix(shadowColor, whtColor, clamp(hitPosition.y/7.0, 0.0, 1.0));  
		}
		hitColor = mix(shadowColor, whtColor, 0.4+shadow*0.6);    
