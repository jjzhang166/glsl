...
void main( void ) {
	...
	sunpos.x += cos(time*0.5)*cos(time*0.1)*0.3;
	sunpos.y += sin(time*0.5)*cos(time*0.1)*0.3;
	...
	float blade = clamp(pow(sin(time*0.7+atan(t.x, t.y)*(BLADES+2.0))+BIAS, SHARPNESS), 0.0, 1.0);

	float dist = .2 / distance(sunpos, position) * 0.075;
	float dist2 = 2.5+0.5*cos(time*0.3);// - distance(vec2(0.0), sunpos);
	dist2 = pow(dist2, 3.5);
