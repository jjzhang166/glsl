...
void main( void ) {
	...
	else
	{
		color = (texture2D(backbuffer, screen_pos/resolution).x +
			1.2*texture2D(backbuffer, (screen_pos + vec2( 0,-5))/resolution).x +
			texture2D(backbuffer, (screen_pos + vec2(-1, 0))/resolution).x +
			texture2D(backbuffer, (screen_pos + vec2( 1, 0))/resolution).x)/4. - 0.031;
	}
	gl_FragColor = vec4( color*1.01, color*1.2 - .5, 0., 1.0 );
