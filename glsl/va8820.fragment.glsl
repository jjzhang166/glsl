...
float BlendSoftLightf(float base, float blend)
{
	...
	if (blend < 0.5)
	{ 
		fResult = (2.0 * base * blend + base * base * (1.0 - 2.0 * blend));
	}
	else
	{
		fResult = sqrt(base) * (2.0 * blend - 1.0) + 2.0 * base * (1.0 - blend);
	}
	return (fResult);
}
...
vec4 Shade(float distance)
{
	...
	a.rgb = BlendSoftLight(a.rgb,Color2.rgb);
	b.rgb = BlendSoftLight(b.rgb,Color4.rgb);
	
	a = mix(a,b,c2);
	b = mix(a,b,c3);
	
	return 	vec4(mix(a.rgb,b.rgb,0.5).xyz,1.0);
}
