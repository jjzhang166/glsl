...
void main( void ) {
	...
	// draw clouds using noisy() function
  	float perspective =  1.0 / (position.y + 0.2);
	float clouds = noisy(((position.x - 0.8) * perspective) + time * 0.03, position.y * perspective * resolution.y / resolution.x * 3.0);
  	fog += pow(1.0 - clouds, 1.87) * 0.2;
	me.rgb = me.rgb * (1.0 - clouds) + fog * clouds;

	if (position.y > 0.5) {
		float plane = mod(time * 0.25, 37.0) + 3.6;                                                   // Z position of plane
