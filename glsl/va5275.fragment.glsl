...
void main( void ) {
	...
	{
		vec2 np = p;
		color = texture2D(bb, vec2(uv.x + sin(rand(vec2(time,time*time)))*0.01, uv.y + sin(rand(vec2(time,time*time)))*0.01 + 0.01)).x;
	}

	gl_FragColor = vec4( vec3( color - 0.015, color - 0.5, color), 1.0 );
