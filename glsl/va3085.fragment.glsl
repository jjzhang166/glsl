...
void main( void ) {
	...
	float cogMin = 0.01; // 'min' is actually the outer edge because I'm using 1/distance... for reasons I can't remember :(
	float cogMax = 0.04; // 'max' is the inner edge, so this controls the size of the hole in the middle - the higher the value the smaller the hole
	...
	float angleFoo = sin(angle*111.0 - pow(mouse.x,0.0) * 4.0) - toothAdjust;
	...
	float hue = 0.5;
	float sat = 0.8;
	float lum = isSolid * length(bipos*0.25)*2.0;
