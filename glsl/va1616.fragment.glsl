...
void main( void ) {
	...
	float f0 = fract(time*0.405)+1e-5;
	float f = fract(f0*2.);
	float fs = sign(f-.5);
	float fs0 = sign(f0-.5);
	...
	float as = (cos(time*.1)*.13);	// there was originally an outer sinh for as and bs but difference is just about 1 percent which doesn't really matter for the camera path
	float ac = sqrt(as*as+1.);
	float bs = (sin(time*.12)*.3);
	...
	vec4 fb = vec4(129.507160029822894,32.10711839885068,35.17128234256937,26.70192179035261);
