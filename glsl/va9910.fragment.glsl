...
{
	float d = 0.;
	vec3 col = vec3(0.1, 0.1, 0.3*(.5+sin(gl_FragCoord.y*3.1456+time*2.0)));
	...
	space(); space(); space();
	d += T(r()); add(); d += O(r()); add(); d += K(r()); add(); d += Y(r()); add(); d += O(r());space();
	newline();
	space(); space(); space();
	d += D(r()); add(); d += E(r()); add(); d += M(r()); add(); d += O(r()); space();
	newline();
	space(); space(); space();
	d += F(r()); add(); d += E(r()); add(); d += S(r()); add(); d += T(r()); space();
