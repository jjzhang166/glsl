...
// Click 'hide code' then drag the right mouse to go forward/back.
...
float halfpi = asin(1.0); // AMD/OSX fix
///float cos(float v){
//	return sin( v + halfpi);
//}
...
void main( void ) 
{
	...
	float time2 = time * 0.7 + log(surfaceSize.y) * 50.0;
	GetCameraRay(vec3(getTunnelCoords(time2),time2), vec3(mouse.x * -40.0 + 20.0, mouse.y * 40.0 - 15.0, time2*1.+17.), currentRay);
