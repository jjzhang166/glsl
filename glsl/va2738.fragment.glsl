...
{
	float fFlagCount = 7.0; // +1 to add new flag
	
	index = mod(index, fFlagCount * sin(time / 100.0));
	if(index < 0.5)
	{
		return Flag1(p);
	}
...
void main( void ) 
{
	...
	//vCol = GetFlagCol( p, 5.0 ); // uncomment to show flag full screen
