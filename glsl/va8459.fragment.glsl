//magnifying cube by SiliconLife
...
void main(){
  ...
  mat3 rot=genRotMat(sin(time/40.13)*360.0,1.0,0.0,0.0);
  rot=rot*genRotMat(sin(time/40.64)*360.0,0.0,1.0,0.0);
  rot=rot*genRotMat(sin(time/40.24)*360.0,0.0,0.0,1.0);
  vec2 p=vec2(0.0,0.0);
  ...
  for(int i=0;i<250;i++){
    ...
    else tempc.y=1.0/tempc.y*sin(time*0.0842);
    ...
    if(val>thres && refracted){
      rayDir=vec3(2.0*spread*p.xy*delta,delta);
      refracted=false;
    }
