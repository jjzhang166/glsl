...
// @mod+ fog, vignette
// CBS: @mod+ Stars
// @psonice_cw fixed camera height, UFO follows terrain
...
vec2 scene(in vec3 p, in vec3 up){
    
    vec2 hit = mindist( terrain(p), ufo(up) );
...
void main(void){
    ...
    vec3 p,n;
    vec3 clr;
    vec3 ufoP = vec3(0.) -viewDir;
	ufoP.z -= 10.0 + sin(time*0.25)*5.0;
	ufoP.x += sin(time/4.0)*10.0;
	ufoP.y -= (terrain(ufoP).x + terrain(ufoP + vec3(0., 0., 0.1)).x + terrain(ufoP + vec3(0., 0., -0.1)).x) / 3.;
	ufoP.y += 7.5;
    ...
    if (f<MAX_DEPTH){
        ...
        n=normalize(
                    vec3(s.x-scene(p-e.xyy, p-e.xyy + ufoP).x,
                         s.x-scene(p-e.yxy, p-e.yxy + ufoP).x,
                         s.x-scene(p-e.yyx, p-e.yyx + ufoP).x));
