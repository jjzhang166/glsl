...
float position_to_distance(vec2 p) {
    ...
    float v = abs(fx-p.y) / sqrt(1.0 + pow(dfx / dx,2.0));
    
	return v - 0.1;
	
    p = repeat(p, vec2(1.0));
    
    float r0 = length(p - vec2(-0.5,-0.5));
    float r1 = length(p - vec2(0.5,-0.5));
    float r2 = length(p - vec2(-0.5,0.5));
    float r3 = length(p - vec2(0.5,0.5));
    //float r1 = 
    
    float d = min(min(r2,r3),min(r0, r1)) - 0.4;
    return d;
}
...
vec3 distance_to_color(float d) {
    ...
    return vec3(inside*outside, inside, inside*outside)+border;
}
...
void main( void ) {
	...
	float pod = position_to_distance(position);
	vec3 podclr =distance_to_color(pod);

	
	vec2 pon = position_to_normal(position);
	vec3 ponclr =normal_to_color(pon);
		
    vec3 distance_color = clamp(distance_to_color(position_to_distance(position)), 0.0, 1.0);
    vec3 normal_color = ponclr;
