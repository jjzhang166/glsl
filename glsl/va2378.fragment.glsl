...
void main( void ) {
	...
	vec3 uv = vec3(th + d, th - d, th + sin(d)) + mouse.xyx;
	...
	vec3 color = mix(vec3(1.0, 0.8, 1.0-f), vec3(0.5*f, 0, 0), pow(a, 0.2)) * 3.0 + mouse.xyx / 2.0;
	color += mix(vec3(0.8, 0.9, 1.0), vec3(0.1, 0.1, 0.2),  pow(b, 0.1)) * 0.75;
	color += mix(vec3(0.9, 0.8, 1.0), vec3(0.1, 0.2, 0.2),  pow(c, 0.1)) * 0.75;
	
	gl_FragColor = vec4( max( color * clamp(dd, 0.0, 1.0) , color ) , 1.0);
}