...
uniform float time;
float water = 0.1 * sin(time);//water level (floodland'd)
const float scale = .2;//height of the rock 
float offTime = time*2.0+97.0;  // Some cool place in time. (makes the animation a little faster
...
{
    float fogAmount = clamp((3.15 * exp(-rayOri.y*.25) * (1.0-exp( -dis*rayDir.y*.25 ))/rayDir.y), 0.0, 1.0);
