...
vec2 obj_roundBox(in vec3 p){
  return vec2(length(max(abs(p)-vec3(1.5,1.5,1.5),0.0))-0.25,1);
}


vec2 obj_sphere(in vec3 p){
  return vec2(length(p)-sin(time)-2.0);
}

vec2 bla(in vec3 p) {
  //return min(obj_roundBox(p), obj_sphere(p));	
  vec2 muh, meh;
  muh = obj_roundBox(p)* obj_sphere(p);
  meh = obj_roundBox(p)+ obj_sphere(p);
  return max(muh,meh);
  //return muh;
}
...
vec2 inObj(in vec3 p){
  return ObjUnion(obj_floor(p),bla(p));
}
...
void main(void){
  ...
  vec3 E=vec3(-sin(time/10.0)*8.0,4,cos(time)*8.0); //Camera location; Change camera path position here
