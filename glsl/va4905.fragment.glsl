...
{
    float d1 = sdSphere(p + vec3(cos(time * 0.2 + PI) * 0.45,0,0), 0.25);
    float d2 = rand(abs(p) / 100.);//0.0;//abs(snoise((gl_FragCoord.xy)/(400. + time/100.)));
    return d1+d2;
}
...
{ //  ray_dir is used only for some optimizations
    ...
    if (ray_dir.z <= 0.0 || p.z < 1.0) 
    { // Optimization: try not to compute blobby object distance when possible
        float sphere1 = opDisplace(p);
