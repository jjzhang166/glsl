...
vec3 Texture(vec2 p,float ts)
{
	p=mod(p*ts,4.)-2.;
	//p=floor(p*p*p);
	float d=length(p*p*p*p*p*p);
	return vec3(1,1.0,0)*d*0.025;
}
void main( void ) {
	...
	float tscale=86.1;
	// shabby tri planar mapping experiment
	n=abs(n);
	vec3 nc=Texture(p.xy,tscale)*n.z;	    
	nc+=Texture(p.xz,tscale)*n.y;	    
	nc+=Texture(p.yz,tscale)*n.x;	    	    
        color = vec4(nc * max(diffuse1+ diffuse2, ambient), 1);
    } else {
        color = vec4(0,0,0,0);
    }
    gl_FragColor = color;
