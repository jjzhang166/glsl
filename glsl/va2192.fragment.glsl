...
void main( void ) {
	...
	float mv = sin(time * 0.9);
	uv = mix(uv, q, 0.0);
	
	float color = 0.0;
	color = cos(uv.x*TAU) * cos(uv.y*TAU + time*5.7);
	color = pow(abs(cos(color*TAU)), 0.3);
	
	float color2 = 0.0;
	color2 = cos(uv.x*TAU*3.0);
	color2 -= 0.9;
		
	float shadow = 10.0/(z*z);
