...
float heightmap(vec2 position)
{
	...
	float height = 0.0;
	float f = .01;		// seems to control range of dot size
...
void main( void ) {
	...
	float sd = sin(time)*0.5+0.5;
	float cd = cos(time)*0.5+0.5;
	
	n1 = heightmap(vec2(pos.x+cd,pos.y-sd));
	n2 = heightmap(vec2(pos.x-sd,pos.y+cd));
	n3 = heightmap(vec2(pos.x+sd,pos.y-cd));
	n4 = heightmap(vec2(pos.x-cd,pos.y+sd));
