...
// red mod by @emackey, alien blood maybe.  The math here is dark magic I think, Kabuto can you post how you created this?
...
float scale = mod(time * 0.0005, 0.2) + 1.3;
mat3 dottransform = mat3(
...
void main( void ) {
	...
	vec3 pos = vec3(0,0,mod(time * 0.4, 60.0) + 400.0);
	...
	gl_FragColor = vec4(max(0.,1.-h)*dist*dist*dist*vec3(pow(c,1.),pow(c,5.5),pow(c,4.)),1.);
}