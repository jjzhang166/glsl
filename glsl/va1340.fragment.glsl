...
void main( void ) {
	...
	position.x += snoise(vec3(vec2(position.x, position.y) + mouse, 0.1*time)) + perlin();
	
	float r = distance(vec2(0.5, 0.5), vec2(position.x, position.y));
	
	float v = sin(r * PI * 2.0);
	//log func_g_toledo_ I have no idea what I'm doing, it just looked cool to me. I probably screwed with other stuff too.
	v = 1.-log(2.8-v);	
