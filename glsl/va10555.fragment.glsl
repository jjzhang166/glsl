...
void main() 
{
	...
	vec2 px =(2.0*gl_FragCoord.xy-resolution.xy)/max(resolution.x,resolution.y);
	...
	for(int i=1;i<zoom;i++) {
       ...
       vec3 col2 = vec3(0.5*sin(3.0*p.x)+0.5,0.5*sin(3.0*p.y)+0.5,sin(p.x+p.y));
	...
	{
		c = c + vec3(smoothstep(0.0, 1.0, -s/0.9));
	}
	...
	vec2 res = Running(uv + vec2(1.0, 0.1));

	if (res.x < 0.005)
	{
		col = mix(col2 * col2 + col, vec3(1.0), smoothstep(0.0, .0005,res.x) );
	}
