...
void main( void ) {
	...
	vec2 rnd = pixel*(vec2(rnd3, rnd4)-0.5)*1.33; // critter code copied from http://glsl.heroku.com/91/0
	...
	gl_FragColor += (vec4(rnd4,rnd3,rnd2,rnd1)-0.5)*0.02 - 0.01; // error diffusion
	gl_FragColor.xyz += 0.05*(gl_FragColor.yzx-gl_FragColor.zxy); // http://en.wikipedia.org/wiki/Belousov-Zhabotinsky_reaction
//	gl_FragColor = vec4(0.);
}