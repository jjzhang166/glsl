...
{
	float grey = dot(gl_FragColor.rgb, vec3(0.299, 0.587, 0.114));
	gl_FragColor.rgb = vec3(grey * vec3(1.2, 1.0, 0.8));
}
...
void tvNoise(void)
{
	...
	float w,x,y,z=0.0;
	vec3 c;
	u+=vec2(20.,11.+fract(time));
	y=fract(u.x*u.x*u.y*u.y*50.);
	c=vec3(y);
	gl_FragColor.rgb -= vec3(c);

}

void getFilter(float index)
{
	float filterCount = 5.0;
	index = mod(index, filterCount);
	if (index < 1.0)
	{
		black_white();
	}
