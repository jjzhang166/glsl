...
void main( void )
{
	...
	float res = 4.0/(resolution.x+resolution.y);
	
	// White background
	gl_FragColor = vec4( 1.0, 1.0, 1.0, 1.0 );
	
	// y = x^3
	drawFormula( y, x*x*x, res, vec3( 0.0, 1.0, 0.0 ) );
	// y = sqrt( x )
	...
	// Axes
	if ( abs( y ) < res*10.0 ) gl_FragColor = vec4( 1.0, 0.0, 0.0, 1.0 );
	if ( abs( x ) < res*10.0 ) gl_FragColor = vec4( 1.0, 0.0, 0.0, 1.0 );
}