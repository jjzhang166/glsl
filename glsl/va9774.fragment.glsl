...
// thank you // sphinx // added light // tweaks
#ifdef GL_ES
...
void main(){
	...
	for ( int i = 0; i < M; i++ ){
		...
		if ( abs(d) < 0.00001 ){
			...
			vec3 lp = vec3(3. + mouse.x, 1. + mouse.y, -2.);
			vec3 lc = vec3(.9, .6, .2);
			...
			float s = pow(96., a * 0.5 * ndh) * 0.00744;
	...
	gl_FragColor = vec4(length(c)*vec3(0.1, 0.215, .317), 1.);
