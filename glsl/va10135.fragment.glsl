...
//port from "Rainbow Slides"
//https://www.shadertoy.com/view/XdsGD4 (original author = fizzer?)
//tigrou (IND) 2013.07.23

//notes : i had to decrease "num_spheres" const a little bit.
//initially it was 32, which hangs up chrome browser and sometimes machine.
...
void main()
{
	...
	vec2 t = (uv * 2.0 - vec2(1.0))*0.5;
	t.x *= resolution.x / resolution.y;
	
	
	vec3 ro = vec3(-0.3, sin(time * 2.0) * 0.05, 0.7), rd = rotateX(1.1, rotateY(0.5, normalize(vec3(t.xy, -0.8))));
	float f = 0.0;
	vec3 rp, n;
	...
	if(length(rp) > 1000.0) rp = vec3(1.0);
	n = sceneNorm(rp); 
