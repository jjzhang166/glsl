...
//Floor
vec2 obj0(in vec3 p){
  return vec2(p.y+4.0 + 0.5*(p.x) + sin(p.x),0);
}
...
//  p.y = p.y*1.2-2.0;
//  p.z = p.z;
  vec2 q = vec2(length(p.xz)-4.5,p.y);
  return vec2(length(q)-1.0, 1);
	
  return vec2(length(p)-2.5, 1);
  return vec2(length(max(abs(p)-vec3(1,1,1),0.0))-0.25,1);
}

//RoundBox with simple solid color
vec3 obj1_c(in vec3 p){
	return vec3(1.0,0.0,0.2);
}
