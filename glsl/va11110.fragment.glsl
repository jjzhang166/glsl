...
float scene(vec3 p)
{
	float d = flr(p, -2.0);
	d = min(d, flr(p, 2.0));
	d = min(d, sph(p, vec4( 0,-2, 15, 2.0)));
	d = min(d, sph(p, vec4(-8, 0, 20, 2.5)));
	d = min(d, sph(p, vec4(-5, 4, 15, 1.0)));
	d = min(d, sph(p, vec4(-1, 3, 15, 2.5)));
	d = min(d, sph(p, vec4( 2,-3, 15, 1.0)));
	d = min(d, cly(p, vec4(10, 0, 20, 1.5)));
	d = min(d, cly(p, vec4( 4, 0, 9, 1.5)));
	d = min(d, cly(p, vec4( 0, 0, 20, 1.5)));
	d = min(d, cly(p, vec4(-2, 0, 25, 1.5)));
	d = min(d, cly(p, vec4(-6, 0, 30, 1.5)));
	d = min(d, cly(p, vec4(-12,0, 35, 1.5)));
	return d;
}
...
void main()
{
	...
	vec3 dir = (vec3(position.x*resolution.x/resolution.y, position.y, 1.1)) + mouz; //ray
...
//-------------------------------------------------------------------------------
	//Tests
	gl_FragColor = vec4(s*a,1)*fg*fg*fg*fg;
	//gl_FragColor = vec4(position,100.0,100.0);
	//gl_FragColor = vec4(dir,-0.5);
}
