...
#endif
//forked up by gtoledo3. Added some stuff floating around here. Best viewed at 1 res.
// moded by seb.cc
...
void main( void ) {  
	...
	float time=time*.2;
	float n = surface3(vec3(uPos, time));	

	float vertColor = 0.0;
	//*
	for(float i=0.0; i<LINES; i++){
		float t = time*(i*0.1+1.)/3.0 + (i*0.1+0.5); 
		uPos.y += sin(t+uPos.x*2.0)*0.55 ;
		uPos.x += sin(-t+uPos.y*3.0)*0.55 ;
	...
	color += 1.-(color.g+color.g+color.b);//+sin(time)+.25;
	gl_FragColor = (vec4(color+(n), 1.0));
	
	
	
	float dx = texture2D(backbuffer, uPos + vec2(-1.,0.)).x - texture2D(backbuffer, uPos + vec2(1.,0.)).x ;
	float dy = texture2D(backbuffer, uPos+ vec2(0.,-1.)).x - texture2D(backbuffer, uPos + vec2(0.,1.)).x ;
	
	vec2 d = vec2(dx,dy)*2.;
	
	gl_FragColor.z = pow(clamp(1.7-2.*length(uPos - vec2(mouse.x,mouse.y) + d),0.0,1.),1.);
