...
float ring(vec2 pos, float ra, float ry, float smooth) {
	
	float len = length( pos );
	float a = smoothstep( ra-smooth, ra+smooth, len );
	float b = smoothstep( ry-smooth, ry+smooth, len );
	return b-a;
}
...
void main( void ) {
	...
	vec2 pos = unipos*0.5-1.0;
	pos.x *=aspect;
	
	float sint = sin(time);
	float usint = sint*0.6+0.11;


	vec3 clr = ring01(pos);
    
    gl_FragColor = vec4(clr,12.0);
