...
void main(void){
	...
	p.z=-1.0;
	p.xy*=3.0;
	float t=cos(time/2.0)*1.00001;
	for(int i=0;i<ITERZ;i++){
		pRot=(vec4(p,1.0)
		      *rotationMatrix(vec3(1.0,0.0,0.0),-3.14*(mouse.y*2.0-1.5))
		      *rotationMatrix(vec3(0.0,0.0,1.0),-3.14*(mouse.x*2.0-1.5))		      
		     ).xyz;
		c+=julia(pRot.xy,vec2(pRot.z,t));
		p.z+=step;
	}
	gl_FragColor=smoothstep(0.0,0.75,c/float(ITERZ));
}