...
*/
     vec3 q = abs(p.xyz);
 	vec2 b = vec2(max(q.z-.06,max(q.x+q.y*0.57735,q.y)-.1));
  vec2 s = vec2(length(p) - .5);
  return vec2(mix(b,s,.1));
 }
...
void main(void)
{
  ...
  vec3 c,p,n;
	...
	if (f<22.)
	{
		...
		gl_FragColor=vec4((b/c+pow(b,32.))-(1.0-f*.05),0.8);//simple phong LightPosition=CameraPosition
	}
	
  else gl_FragColor=vec4(1.,1.,1.,1.0); //background color
}