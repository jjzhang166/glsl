...
vec2 f(vec2 z)
{
  ...
  vec2 p = vec2(sin(time),mouse.x * cos(time));
  p = mul(p,z);
...
vec2 newtonfractal(vec2 z)
{
  ...
  if (length(d) < 0.2) {
	      //return vec2(.2,0.8);
      return vec2(atan(z.y,z.x)/3.141592653/2., float(r)*0.025); // Hue based on the direction of z.
      //return vec2(z.x+z.y, float(r)*0.025); // Ad hoc coloring based on z, the original by @hintz, optimized for z^3-1.	
  }
  
  return vec2(1.,200./length(d));
}
