// 'Landscape' by Dave Hoskins (2012)
#ifdef GL_ES
...
{    
    //fisheye
    //vec2 uv=(-1.0+2.0*((gl_FragCoord.xy)/resolution.xy)) * 0.3;	
    vec2 vPos=barrelDistortion(-1.0+2.0*((gl_FragCoord.xy)/resolution.xy)) * 0.3;
    //Camera animation
    vec3 vuv=vec3(stime*0.001,.8,1.*sin(time*.1)-.5);     //view up vector //some rotation w time -gt
	...
	gl_FragColor.rgb *= smoothstep(3.95, .01, d)*1.5;//vignette and blare out -gt
	
	gl_FragColor.rg += mod( gl_FragCoord.y, .3 );
	//gl_FragColor.b += mod( gl_FragCoord.y, .31 );	//lines after vignetting -gt
