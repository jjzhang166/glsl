...
void main( void ) {
    ...
    x+=cos(x.yx*sqrt(vec2(13,9)))/5.;
    c.rgb=lungth(fract(x*sqrt(vec2(33,43))),vec3(5,6,7)*Saturation);
    x+=sin(x.yx*sqrt(vec2(73,53)))/5.;
    c.rgb=2.*lungth(sin(time+x*sqrt(vec2(33.,23.))),c.rgb/9.);
    x+=exp(x.yx*sqrt(vec2(93,73)))/2.;
    c.rgb=lungth(sin(x*inversesqrt(vec2(13.,1.))),c.rgb/2.0);
