...
void main( void )
{
	...
	vec2 min = vec2(-floor( resolution.x / 640.0 ), -floor( resolution.y / 64.0 ));
	vec2 max = vec2(floor( resolution.x / 6400.0 ), floor( resolution.y / 64.0 ));
	...
	drawFormula( cos(x - time * 1.0), y + sin( time ), res*5.0, vec3( 0.2, 0.7, 1.0 ) );
}