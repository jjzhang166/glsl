// graph-plotting @pixelmager
// see http://blog.hvidtfeldts.net/index.php/2011/07/plotting-high-frequency-functions-using-a-gpu/
...
float stochEval( vec2 uv ) {
  ...
  vec2 maxdist = 0.05 * vec2( aspect, 1.0 );
...
float distAvgEval( vec2 uv ) {
  const int samples = 255; //note: on AMD requires 255+ samples, should be ~50
  const float fsamples = float(samples);
  vec2 maxdist = 0.025 * vec2( aspect, 1.0 );
...
float proxyEval( vec2 uv ) {
  const int samples = 255; //note: on AMD requires 255+ samples, should be ~50
  const float fsamples = float(samples);
  vec2 maxdist = vec2(0.0125) * vec2( aspect, 1.0 );
