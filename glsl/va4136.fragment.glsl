...
/*
 * Metaballs 102.001, sames as last but with colors.
...
void main( void ) {
	...
	// Same with color.
	vec3 color1 = vec3( 1.0, 0.5 + 0.5 * sin( time ), 0.5 + 0.5 * cos( time ) );
	vec3 color2 = vec3( 1.0, 1.0, 1.0 );
	vec3 color = normalize( ( color1 * sphereContribution1 + color2 * sphereContribution2 ) / totalContribution );
	...
	// Out!
	gl_FragColor = vec4( proj * color, 1.0 );
