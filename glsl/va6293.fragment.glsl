...
float color(vec2 p) {
	float v = 0.0;
	v += 1.-abs(pow(noise(p)-0.5,0.75))*1.1;
	return v;
}

void main( void ) {
	vec2 p = surfacePosition*.05+.5;
	float c1 = color(p*.3+time*.00015);
	float c2 = color(p*.3-time*.00015);
	
	float c3 = color(p*.2-time*.00015);
	float c4 = color(p*.2+time*.00015);
	
	float cf = pow(c1*c2*c3*c4+0.6,15.);
