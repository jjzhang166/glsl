...
{
	return (vec3(pow((fract(dir.x*10.0) * (1.0 - fract(dir.x*10.0))),9.0)*pow((fract(dir.y*10.0) * (1.0 - fract(dir.y*10.0))),9.0))*1000000000000.0 + 
	       vec3(0.5,0.5,0.7) / (min(length(dir.xy+vec2(0.0,-0.4)) * 0.6+0.6, 1.0))) * 0.4;
}
