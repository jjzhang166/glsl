...
float scene(vec3 p) { 
  ...
  float n = fbm(p*3.0);  	
  return min(e,n*0.12+d); // 'texture'
}

vec4 color(float d) { 
	float opa = smoothstep(1.0,0.0,0.5+d*5.0);
	return vec4(opa*opa,opa*(1.0-0.3*opa),opa,opa);
 // return mix(vec4(1.,1.,1.,0.25),vec4(0.2,0.2,0.,0.01),smoothstep(0.,0.2,d)); 
}
...
void main( void ) {
  vec3 e = vec3(sin(time*0.2)*20.,12.,-20.); 
  vec3 p = vec3((gl_FragCoord.xy / resolution.xy) * 2. -1., 1.);
  p.x *= resolution.x/resolution.y;
  p += e;
  gl_FragColor = ray(p, normalize(p-e)*0.55);
}