...
void main( void ) {
	...
	//check out this too : http://glsl.heroku.com/e#8461.4
	
	vec2 position = ( gl_FragCoord.xy / resolution.xy );
	
	float f = length(vec3(position-mouse,0.0)); //I do so love disturbing patterns 
	position = ( gl_FragCoord.xy / resolution.xy * f); //run your mouse along the bottom and look at the dual boxes
