...
void main( void )
{
	...
	const float fAmpModifier = 0.03;	// The height of the wave.
	const float fWaveSpeed = 1.5; 		// the speed at which it bounces up and down
	const float fSpeed = 0.3;		// the speed of the beam heading towards the center.
	const float k = 7.;  			// the number of iterations (waves within waves) i.e. 9 
	
	vec3 color = vec3(0.0, 0.0, 0.12); 	// background colour
	...
	pos.x = floor(float(pos.x) / 0.01) * 0.01;
	pos.y = floor(float(pos.y) / 0.01) * 0.01;
