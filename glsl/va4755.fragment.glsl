...
// Glass mod: structures filled with colored glass. Only the glass volume itself is colored, not its surface.
  // - That would make it more like fog, wouldn't it?

const float refractionIndex = 1.05;	// 1 = just fog, 1.2 = lightweight glass (most interesting value IMO), 1.4 = glass, 2.4 = diamond, 100 = apparently static texture
const float maxrad2 = 0.8;		// radius of innermost spheres
const float maxrad3 = 0.55;		// radius of outermost spheres
...
void main( void ) {
	...
	for (int i = 0; i < 20; i++) {
		...
		// vec3 fogcolor = vec3(dot(fract(fa),fract(fb)),dot(fract(fb),fract(fc)),dot(fract(fc),fract(fa)));
		vec3 fogcolor = vec3(cremainold,pd,kDist).yzx;
		//fogcolor *= fogcolor;
		fogcolor = pow(fogcolor.zxy,vec3(1.8));
	...
	gl_FragColor = vec4(pow(color,vec3(1.0/1.8)), 1.)*4.;
