...
void main(void) 
{
	...
	float power = 1.;
	float color = 3.0 - (3.4*length(3.*p));
	
	vec3 coord = vec3(atan(p.x,p.y)/Tau+.9, d*.2, .5);
	coord += vec3(0.,-time*.01, time*.002);
	
	for(int i = 0; i < 7; i++)
	{
		power *= 4.;
		color += (.99 / power) * snoise(coord, power*12.);
	}

	gl_FragColor = vec4( color, pow(max(color,0.),1.)*1.9, pow(max(color,0.),1.)*4. , pow(max(color,0.),1.)*4.)*a;
}