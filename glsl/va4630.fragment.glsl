  ...
  if (f<maxd){
    ...
    vec3 finalColor = vec3( b*c+pow(b,8.0))*(1.0-f*.02) ;  //simple phong LightPosition=CameraPosition
     ...
     bgColor = 
      ...
      vec3(s.x-inObj(p-e.xyy).x,
           s.x-inObj(p-e.yxy).x,
           s.x-inObj(p-e.yyx*10.).x));
	  ;
	  
	  //vPos += vec2(sin(f*0.005), cos(f*.005));
	  vPos += n.xy*.05;
	  float ch = mod(vPos.x * 10., 1.) > .5 ? 1. : 0.;
	  ch = mod(ch + (mod(vPos.y * 10., 1.) > .5 ? 1. : 0.), 2.);
	  bgColor = vec3(ch);

	  //bgColor = mod(vec3(length(vPos) * f), 1.);
	  //bgColor = bgColor *f/n * 0.00001 ;
	  gl_FragColor=vec4(bgColor,1.)*1.;
