...
void main( void ) {
  ...
  vec4 memState = texture2D(backbuffer, vec2(pp.x*10. ,0.0));
  vec4 memPos = texture2D(backbuffer, vec2(pp.x*15., 0.0));
  vec4 memDir = texture2D(backbuffer, vec2(pp.x*20., 0.0));
  vec4 color = black;
  ...
  if (memState.r == 1.0 &&
    ...
    color = white;
  } else if (statePixel(pos, 10.0, 0.0)) {
    if (memState.r == 0.0) {
      memState.r = 1.0;
      memState.a = 1.0;
      memState.g = 0.0;
    ...
    color = memState;
  } else if (statePixel(pos, 15.0, 0.0)) {
    ...
    color = memPos;
  } else if (statePixel(pos, 20.0, 0.0)) {
