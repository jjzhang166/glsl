...
const int kSamplesPerPixel = 5000;
const int kSamplesPerFrame = 20;

const float kTwoPI = 2.0 * 3.14159;
const float kInf = 1e20;

const int kDiffuse = 0;
const int kSpecular = 0;
...
vec4 encodeColor(vec3 n) {
	...
	return vec4(min(n.xyz, vec3(1.0)) / d, d);
}
...
void main() {
	...
	if (samples < kLoop) {
		...
		color = (samplesNext < kLoop) ? encodeColor(color.xyz) : vec4(pow(color.xyz, vec3(1.0 / 2.2)), 1.0);
	}
