...
const int MAX_RAYMARCH_ITER = 64;
...
float map(vec3 p, vec3 ray_dir) {
    ...
    if (ray_dir.z <= 0.0 || p.z < 1.0) { // Optimization: try not to compute blobby object distance when possible
        float sphere1 = sdSphere(p + vec3(cos(time * 0.33 + PI) * 0.15,0,0), 0.4);
       
        float torus = sdTorus(vec3(p.y + sin(tt) *0.51, p.z + cos(tt) * 0.1, p.x), vec2(0.3, 0.08));
	    
       
	 return min(blend(sphere1, torus), plane);    
    } else {
        return plane;
    }
}
...
void main( void ) {
    ...
    float angleX = (start.y -0.5) * 0.5;
    float angleY = (start.x -0.5);
    float angleZ = (start.x -0.5) * 0.5;
    ...
    ray_start.x = -start.x *3.0+1.5;
    ray_start.y = start.y * 1.0 - 0.5;

	float light_time = time*0.25;
	float light_x = sin(light_time*0.5)*4.0;
	float light_z = cos(light_time*0.5)*4.0;
	
    vec3 light1_pos = vec3(light_x, 1.0,  light_z);
  //  vec3 light2_pos = vec3(sin(time * 1.9 + 2.0) * 0.6, sin(time * 1.8) + 5.0, -0.5 + sin(time * 1.6) * 0.5);
    ...
    if (raymarch(ray_start, ray_dir, dist, p, iterations)) {
        ...
        float ambient = (0.09 - ambientOcclusion(p, n)) / (dist * dist * 0.07);
        vec3 diffuse1 = vec3(0,0,0.0);
	    
        if (!raymarch_to_light(p + light1_dir * 0.1, light1_dir, light1_dist, d2, p2, i2, light_intensity)) {
            diffuse1 = vec3(1.0, 0.7, 0.2) * max(0.0, dot(normalize(light1_pos - p), n) * light_intensity * 3.0 / (dist * dist));
        }
        vec3 diffuse2 = vec3(0,0,0);
  
        color = vec4(vec3(0.9,0.5,0.4) * max(diffuse1 - diffuse2, ambient), 1);
    } else {
