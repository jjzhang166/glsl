...
//added some torus type deform, now I'm giving it a little noise, fattening up the dots. major weirdedelia.
//-2012, George Toledo
//uniform float size;
  ...
  void main(void)
 {
	...
	uv+=+(lum*.55);
	 ...
	 //dots
  	...
  	//smoothstep em.
     	gl_FragColor = mix(vec4(.85, .58, .59, 1.0), vec4(0.0, 0.0, 0.0, 1.0),
                        smoothstep(1.0, resolution.x*4., dist_squared));
	//gl_FragColor.rgb *=(lum,lum,lum);
	 gl_FragColor.rgb +=(lum,lum,lum)-.95;
