...
{ 
  ...
  return vec2(mix(c1,c2,cf),1.0);*/
	...
	p.y -= 1.3*pow(dot(p.xz,p.xz)+0.3,0.42);
	p.z /= 0.55;
   vec2 d1 = vec2( length(p) - 4.0, .0 );
   return d1;
}
...
void main(void){
  ...
  //Camera animation
  float t = time*0.1; // slower
  vec3 vuv=vec3(0,1,0);
  vec3 vrp=vec3(12.0,4.0,2.0);
  float mx=sin(t*0.1)*PI*4.0+t*0.7;
  float my=sin(t*0.4*PI/1.01)*0.2+0.5; 
  vec3 prp=vrp+vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*12.0; //Trackball style camera pos
  float vpd=0.8;
  vec3 light=prp+vec3(9.0,5.0,5.0);
  ...
  const vec3 backc=vec3(0.99,0.7,0.2);
  const float spec=64.0;
  const vec3 ambi=vec3(0.5,0.5,0.0);
  
  vec3 c1=render(prp,scp,128,maxe,startf,60.0,backc,light,spec,ambi,n,p,f,o);
  c1=c1*max(1.0-f*.016,0.0);
  vec3 c2=backc;
  if (o>0.6){
    scp=reflect(scp,n);
    c2=render(p+scp*0.05,scp,32,maxe,startf,30.0,backc,light,spec,ambi,n,p,f,o);
  }
  c2=c2*max(1.0-f*.026,0.0);
  gl_FragColor=vec4(c1.xyz*0.7+c2.xyz*0.35, 1.0);
