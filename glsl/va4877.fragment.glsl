...
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
    ...
    if (ray_dir.z <= 1.0 || p.z < 1.0) { // Optimization: try not to compute blobby object distance when possible
...
bool raymarch(vec3 ray_start, vec3 ray_dir, out float dist, out vec3 p, out int iterations) {
    ...
    for (int i = 1; i <= MAX_RAYMARCH_ITER; i++) {
        ...
        minStep += 0.0000002 * ifloat * ifloat;
    }
    return false;
}
...
bool raymarch_to_light(vec3 ray_start, vec3 ray_dir, float maxDist, out float dist, out vec3 p, out int iterations, out float light_intensity) {
    ...
    for (int i = 1; i <= MAX_RAYMARCH_ITER; i++) {
        ...
        float ifloat = float(i);
        minStep += 0.000008 * ifloat * ifloat;
    }
    return false;
}
...
void main( void ) {
    ...
    vec3 light1_pos = vec3(-0.5 + sin(time), 1.0, -1.0 + cos(time * 0.2) * 1.0);
    vec3 light2_pos = vec3(sin(time * 0.9 + 2.0) * 0.6, sin(time * 1.0) + 1.0, -0.1 + sin(time * 0.2) * 0.1);
    vec4 color;
    ...
    if (raymarch(ray_start, ray_dir, dist, p, iterations)) {
        ...
        float ambient = (0.116 - ambientOcclusion(p, n)) / (dist * dist * 0.17);
        vec3 diffuse1 = vec3(0,0,0);
        if (!raymarch_to_light(p + light1_dir * 110.1, light1_dir, light1_dist, d2, p2, i2, light_intensity)) {
            diffuse1 = vec3(.0, 0.8, 0.6) * max(0.0, dot(normalize(light1_pos - p), n) * light_intensity * 5.0 / (dist * dist));
        }
        vec3 diffuse2 = vec3(0,0,0);
        if (!raymarch_to_light(p + light2_dir * 0.1, light2_dir, light2_dist, d2, p2, i2, light_intensity)) {
            diffuse2 = vec3(0.0, 0.8, 1.0) * max(0.1, dot(normalize(light2_pos - p), n) * light_intensity * 11.0 / (dist * dist));
        }
