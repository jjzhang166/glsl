...
//
//	Adding these static points, so somethings onscreen for the galery when teh screen shot is taken
bool AtStaticBurnPoint(vec2 pos, vec2 burnPoint)
{
	float radius = 0.008;
	float pulseSpeed = 4.0;
	float orbit		 = 0.05 * ((sin(time * pulseSpeed) + 1.5) * 0.5);
	...
	vec2 toOrbitOne 	= pos - (burnPoint + vec2(sin(newTime) * orbit, cos(newTime) * orbit));
	vec2 toOrbitTwo 	= pos - (burnPoint + vec2(sin(newTime + k13PI) * orbitTwo, cos(newTime + k13PI) * orbitTwo));
	vec2 toOrbitThree   = pos - (burnPoint + vec2(sin(newTime + k23PI) * orbit, cos(newTime + k23PI) * orbit));
	vec2 toOrbitFour   = pos - (burnPoint + vec2(sin(newTime + k33PI) * orbitTwo, cos(newTime + k33PI) * orbitTwo));
	vec2 toOrbitFive   = pos - (burnPoint + vec2(sin(newTime + k43PI) * orbit, cos(newTime + k43PI) * orbit));
	vec2 toOrbitSix   = pos - (burnPoint + vec2(sin(newTime + k53PI) * orbitTwo, cos(newTime + k53PI) * orbitTwo));
...
void main( void ) 
{
	...
	if(AtMouse(position) || 
		AtStaticBurnPoint(position, vec2(0.25, 0.1)) ||
		AtStaticBurnPoint(position, vec2(0.5, 0.1)) ||
		 AtStaticBurnPoint(position, vec2(0.75, 0.1)))
	{
		newPixel	= flameMin;
	}
	else
