...
void main( void ) {
	...
	float a = 0.5 + cos(uv.x * 3.1415926*0.2) * 0.3;
	...
	float scale = sin(0.1 * time) * 0.222 + 5.0;
	float distortion = resolution.y / resolution.x;

	vec2 position2 = (((gl_FragCoord.xy * .1 / resolution) ) * scale);
	...
	{
		vec2 star_pos = vec2(sin(i) * 200.0, sin(i*i*i) * 500.0);
		float z = mod(i*i - 100.0*time, 512.0);
		float fade = (456.0 - z) /300.0;
		vec2 blob_coord = star_pos / z;
		gradient += ((fade / 50.0) / pow(length(centered_coord - blob_coord ), 1.8)) * ( fade);
	}
