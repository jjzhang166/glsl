...
{          
	...
	vec3 vSphereDomain = DomainRepeatXZ(vPos, vec2(3.0, 3.0));
       
	vec4 vDistSphere = vec4( length(vSphereDomain + vec3(0.0, sin(vPos.x / 4.0)*0.5 + sin(vPos.z / 4.0)*0.5, 0.0)) - 0.5, 2.0, vSphereDomain.xy);
...
void main( void )
{
	...
	vec3 vCameraPos = OrbitPoint(-mouse.x * 7.0, (mouse.y + 0.1) * kPI * 0.25) * 7.0 - vec3(0.0, 0.9, 0.0);
	#ifdef ENABLE_MONTE_CARLO             
	float fDepthOfField = 0.0;
