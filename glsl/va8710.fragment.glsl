...
void main( void ) 
{
    ...
    vec3 clr1 = c0 * flare(d0,6.3,.8) + flare(d0,3.3,0.08); 

    float d1  = sdCapsule(vec3(pos.xy,0.),vec3(-.8,mix(-.3,.3,t.x*.84),0.0), vec3(0.8,mix(0.3,-0.3,t.y),0.0), 0.12); 
    vec3 clr2 = mix(c0,c1,unipos.x)  * (flare(d1,7.3,.77) + flare(d1,4.3*t.x,0.09)) * .75; 

    float d2  = sdCapsule(vec3(pos.xy,0.),vec3(0.8,-0.3,0.0), vec3(0.8,0.3,0.0),.2);
    vec3 clr3 = c1 * flare(d2,6.3,0.8) + flare(d2,3.3,0.08);  

    gl_FragColor = vec4(clr1+clr2+clr3,1.);
}