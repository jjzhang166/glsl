//star tunnel, ... messed around with, and not for the better. See parent() for orig.
...
void main(void)
{
	...
	float gradient		= 0.0;
	float speed		= 128.0 * sin( time); //negate to reverse
	...
	for (float i = 1.0; i < stars; i++)
	{
		...
		gradient 	+= ((fade / depth) / pow(length(pos_center - blob_coord), 2.5));
	}
	float c = abs(sin(time * 1.13));

	gl_FragColor = vec4(gradient * c, gradient * c, gradient * c, 1.0);
}