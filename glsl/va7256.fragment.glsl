...
void main(void)
{
	...
	vec3 lightColor = vec3(0.3, 0.45, 0.85);
	
	//will be better as
	for (int i=0; i<15; i++) {
		//direction of light
		float zr = sin(time*0.15*float(i))*0.5 - PI/3.0;
		vec3 dir = vec3(cos(zr), sin(zr), sin(0.15*time) + 0.25);
		...
		float cosOuterCone = cos(radians( float(i*5)));
		float cosInnerCone = cos(radians( float(i*6)));
