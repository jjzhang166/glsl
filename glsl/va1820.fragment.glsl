...
R(d.xy, -time);
   ...
   for (float i=0.; (i<1.); i+=1./64.) {
	...
	d = normalize(d-l*fieldDeriv);
      ...
      vec3 amiga = normalize(p);
	vec3 camiga = step(0.,sin(atan(amiga.x,amiga.y)*8.)*sin(atan(amiga.z, sqrt(1.-amiga.z*amiga.z))*8.))*vec3(0,1,1)+vec3(1,0,0);
   gl_FragColor = vec4(camiga*(1.-td)*step(length(p),1.)+  tc+(1.-td)*step(mass*mass*2.,dot(p,p))*((fpn(d*10.)+.5)*vec3(.3,0.,1.)+(fpn(d*100.)+.5)*vec3(.2,.1,.3)), 1.0);
}