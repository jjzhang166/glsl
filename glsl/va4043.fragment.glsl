...
float find_intersection(vec3 dir,vec3 pos,out vec3 normal)
{
	...
	intersect_first_plane(dir,pos,normalize(vec3(0.0,1.0,0.0)),0.4,near,far,normal);
	if(far<0.0 || near>far) return -1.0;
	
	const float faces = 	4.0;
	float bigness = 1.0+sin(time);
	for(float i = 0.0;i < faces;i++)
	{
		float a =((360.0/faces)*i)*RAD;	
		
		intersect_plane(dir,pos,normalize(vec3(cos(a), 1.5 * cos(time), sin(a))),bigness,near,far,normal);
		if(far<0.0 || near>far) return -1.0;
		intersect_plane(dir,pos,normalize(vec3(cos(a), -sin(time), sin(a))),bigness,near,far,normal);
		if(far<0.0 || near>far) return -1.0;
	}
...
void main()
{
	...
	if(t>=0.0)
	{
		...
		vec3 reflected=dir-2.0*dot(dir,normal)*normal;
