...
vec2 obj_roundBox(in vec3 p){
  return vec2(length(max(abs(p)-vec3(1,1,1),0.0))-0.25,0.5);
}

//RoundBox with simple solid color
vec3 obj_roundBox_c(in vec3 p){
	return vec3(sin(time),0.5,0.2);
}

//Objects union
vec2 inObj(in vec3 p){
  return obj_roundBox(p);
}
...
void main(void){
  ...
  vec3 viewDir=vec3(2,0,0); //Change camere view vector here
  vec3 E=vec3(-sin(time/3.0)*8.0,4,cos(time/3.0)*8.0); //Camera location; Change camera path position here
