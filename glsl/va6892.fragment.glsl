...
void main( void )
{
	...
	for( float i = 0.0; i < 4.0; ++i )
	{
		...
		uPos.y += sin( uPos.y/1.0+uPos.x/1.0*(i/0.5+1.0)+t ) * 5.8; //3.1
		float fTemp = abs(152.0 / uPos.y/0.09 / 260.0);
		vertColor += fTemp;
		color += vec3( fTemp*i/111.0, fTemp*i/16.0, pow(fTemp,i/2.0)/16.0 ) * 0.05;
	}
...
//	gl_FragColor += vec4( color.z,color.z,color.z, 1.0 );
	...
	const float lineTickness = 0.5;
	const float lumaCount = 20.0;
	const float hueCount = 80.0;
	
	position =  gl_FragCoord.xy - (resolution.xy/2.0);
	
	float d = length(position);
	
	float v = mod(d + time/140.0*resolution.x + resolution.x/(lumaCount*2.0), resolution.x/lumaCount);
	v = abs(v - resolution.x/(lumaCount*2.0)) - lineTickness;
	
	float angle = acos(position.x/d*2.0) / pi_2_f;
	...
	float dr = 1.2+(sin(time*1.1)+1.0)/8.0;
	float dg = 1.2+(sin(time*1.2)+1.0)/8.0;
	float db = 1.2+(sin(time*1.4)+1.0)/8.0;
