...
void main(void){
  ...
  // General parameter setup
   ...
   //Camera setup
	...
	cam.position = vec3(sin(cos(time)-mouse.x)*2.0,sin(time)* 2., cos(mouse.y));						// The position of the camera
	cam.up = vec3(0,(sin(time))-.5,0);									// The up vector, change to make the camera roll, in world space
	...
	const vec3 e=vec3(0.01,0,0);								// Epsilon value, used to calculate the hit normal
  	...
  	if (distanceFromCamera<MAX_DEPTH){							// If this is true, it means we hit something
		...
		float lambert = dot(normal*1.2, -cam.rayDir);					// This is equivalent to having a diffuse light at the camera position
    		...
    		finalColor = lambert*diffuseColor + vec3(cos(distanceFromCamera), sin(2.*time)+0.03, sin( distanceFromCamera) - sin(time));						// Modulate lighting term with color of the material (red)
  	}
