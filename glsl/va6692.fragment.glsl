...
//fork, hexagon with torus out of the middle. @gt
#ifdef GL_ES
...
float sdHexagon( vec3 vPos, vec2 h )
{
...
vec4 map( in vec3 p )
{
   ...
   float d1 = sdHexagon(p, vec2(.75,.5));
   float d2 = sdTorus(vec3(p), vec2(.1,.6));
   float d = max(-d2,d1);
...
void main(void)
{
    ...
    float ctime = time;
    // camera
    vec3 ro = vec3(0.0,1.5*cos(time*.1)/2.-.5,1.+sin(time*.1))/1.55;
    ...
    if( tmat.x>0.0 )
    {
        ...
        vec3 matcol = vec3(
            0.8+0.4*cos(5.0+6.2831*tmat.z),
            0.5+0.4*cos(5.4+6.2831*tmat.z),
            0.5+0.4*cos(5.7+6.2831*tmat.z) );
        col *= matcol;
        col *= 1.2*exp(-0.5*tmat.x);
    }
