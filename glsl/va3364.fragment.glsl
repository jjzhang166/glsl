...
float shape344b_aktenzeichen_n12_strich_19(vec2 p)
{
...
float shape351b_aktenzeichen_n12_strich_19(vec2 p)
{
	p *=0.5;
	p.x = abs(p.x*2.0);
	p.y *= 4.0;
...
float shape352b_aktenzeichen_n12_strich_19(vec2 p)
{
	p *=4.0;
	p.x = abs(p.x*0.5);
	p.y /= 4.0;
...
float Krysler_003(vec2 p)
{
	float a= shape352b_aktenzeichen_n12_strich_19(p);
	return  max(a, 0.0);
