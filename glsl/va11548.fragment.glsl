...
void main( void ) {
	...
	for(float j = 0.0; j < 1.0; j++){
		glow = (pow(20.0 * sin(mouse.y / 0.069), 0.7));
		...
		o += c2 * n; 
		vec4( n,0.0,0.0, 0.5);
		if(near(position.y,o2_1 + resolution.y*0.5 ,3.0)){ o += c2; }
		
		//o3
		n = nearf(position.y, o3 + resolution.y* 0.5,(glow + f * modulator))*0.5;
		o += c3 * n; 
		vec4( n,0.0,0.0, 0.5);
		if(near(position.y,o3_1 + resolution.y*0.5 ,3.0)){ o += c3; }
		...
		o += c4 * n; 
		vec4( n,0.0,0.0, 0.5);
		if(near(position.y,o4_1 + resolution.y*0.5 ,3.0)){ o += c4; }
