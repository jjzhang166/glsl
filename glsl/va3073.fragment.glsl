...
// @rotwang:
// @mod* flatter shapes and bigger gaps
// @mod- removed size change
// @mod* replaced checker with stripes
// @mod* blueish coloring
...
{ 
  ...
  float c1=length(max(abs(p)-vec3(0.33,0.1,0.33),0.0))-0.15;
...
vec3 obj_c(vec3 p){
  vec2 fp=sim2d(p.xz,2.0);

fp.x = smoothstep(fp.y,-0.125,0.125);
	
  if (fp.x > 0.0)
	  return vec3(0.4,0.5,0.6)+fp.x;
	
    else return vec3(0.85,0.9,0.9);   
}
...
void main(void){
  ...
  vec3 c1=render(prp,scp,256,maxe,startf,30.0,backc,light,spec,ambi,n,p,f,o);
