...
// rotwang: @mod*, some floor testing,colored_checker
#ifdef GL_ES
...
vec3 colored_checker(in vec3 p){
	
	float sx = floor( abs( mod(p.x,4.0)));
	float sz = floor( abs( mod(p.z,4.0)));
	vec3 clr = vec3(0,0,0);
 if (fract(p.x*.5)>.5)
   if (fract(p.z*.5)>.5)
     clr = vec3(sx,0,sz);
...
vec2 inObj(in vec3 p){
  return obj0(p); // ObjUnion(obj0(p),obj1(p));
}
