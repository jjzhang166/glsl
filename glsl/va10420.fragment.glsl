...
/*
 * Changelog(Please write changes here):
 * .1 - uploaded code
 * .2 - Some optimalisation
 * .3 - Variation in movement speed.
*/
...
void main( void ) {
	...
	{
		vec2 pos = vec2(
			sin(((time*0.02*sin(float(i)/13.37))+100.0)*hash(float(i)*3.5))*0.9, 
			cos(((time*0.03*sin(float(i)/13.37))+100.0)*hash(float(i)*5.0))*0.9);
		float greyscale = checkCircle(pos, ((vec2(gl_FragCoord.x, gl_FragCoord.y)/resolution)*vec2(2))-vec2(1), ((sin(float(i))+1.0)/1.9)+0.7);
		if(greyscale>0.001)
		{
			color = color + vec3(greyscale)*colorize(pos);
		}
	}
