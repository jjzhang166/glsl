...
#define FracMapScale 32.0
#define FracMapLineWidth 0.05
#define FracMapJitter 0.76 //0.0 is a grid, >0.8 is bad edges
...
void main( void ) {
	...
	vec2 position = (( gl_FragCoord.xy / resolution.x ) + mouse * 2.0) * FracMapScale;
	vec2 F = cellular(position);
	float lines = floor(smoothstep(0.0, FracMapLineWidth, abs(F.y-F.x)));
