...
float svin(float p) { return sin(p*6.28) * 0.5; }
...
void main( void ) {
	...
	float t = svin((time+svin(time))*0.1);
	float x = 1.0-smoothstep(0.1, 1.0, r(pc) * 1.3 + svin(a2(pc) * 6.0 + t * 3.0) - svin(a2(pc) * 4.0 + 7.0) + t*1.1);
