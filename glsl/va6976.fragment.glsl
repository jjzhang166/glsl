...
vec3 getTerrain(vec3 scrCoord, vec3 rd, float dis) {
	...
	if (pos.y <= water) {
		...
		//Panning texture
		//col.x = .6 + .40 * terrainLow(pos.x*14.0+offTime, pos.z*14.0+offTime*4.1, .01, .1);
		//col.y = .1 +  .5 * terrainLow(pos.x*44.0+offTime, pos.z*44.0+offTime*4.1, .01, .1);
		
		col.x = .6 + .40 * NoiseNonLinear(pos.x*14.0+offTime, pos.z*14.0+offTime*4.1);
		col.y = .1 +  .5 * NoiseNonLinear(pos.x*44.0+offTime, pos.z*44.0+offTime*4.1);
...
void main(void)
{
    ...
    vec3 vuv=vec3(-.3+stime*0.15,.4 ,0);     //view up vector
