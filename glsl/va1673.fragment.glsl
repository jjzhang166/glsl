...
// messed up by @blurspline for fun and learning purposes
// any votes to stream audio volume in as a aidditional uniform? ;)
#ifdef GL_ES
...
float f(vec3 p)
{
    return length(max(vec3(0.0, 0.0, 0.0), abs(mod(p, 1.0) - 0.5) - 0.15 + sin(time*100.0)*0.005 )) - 0.1; // 0.1 is curve. change it to 0.4 and see magic!
}
...
{
    vec3 fp = floor(p), col = mix(vec3(0.4, 0.6, 0.8), vec3(0.7, 0.4, 0.6),
...
void main(void)
{
    ...
    for(int i = 0; i < sum_num; ++i)
    {
        ...
        vec3 ro = vec3( rand1(gl_FragCoord.xy + fi), rand2(gl_FragCoord.xy + fi), 0.0) * 0.04; // last variable is "circle of confusion"
	...
	rd = rotateY(rotateX(rd, ax), ay);
	 ...
	 // go dizzy with this   
	 //rd = rotateY(rotateX(rd, ax+cos(t * 0.2)+ sin(t * 0.2)), ay +cos(t * 0.2)+ sin(t * 0.2));
