// simple Mandlebulb by Xavier de Boysson
...
{
	float rdt = dot(p, d);
	float rdr = dot(p, p) - 1.41315501; // sqrt(PI/2)
...
void main( void )
{
	...
	vec3 camOrigin = vec3(cos(mouse.x)*cos(mouse.y), sin(mouse.y), cos(mouse.y)*sin(mouse.x))*1.0;
	vec3 camTarget = vec3(1.0, 0.0, 0.0);
	vec3 camDir = normalize(camTarget - camOrigin);
	vec3 camUp  = normalize(vec3(5.0, 1.0, 2.0));
