...
vec2 obj0(in vec3 p){
  ...
  p.z=p.z-32.0;
  p.yz=rotsim(p.yz,cos(time*.8345)*2.0 + 4.0); 
  p.z=p.z-1.0;
  p.xy=rotsim(p.xy,sin(time)*4.0+5.0);
  float c1=length(max(abs(p)-vec3(0.1,0.05,0.3),0.0));
  float c3=length(max(abs(p)-vec3(0.05,4.0,0.05),0.0));
...
void main(void){
  ...
  vec3 vrp=vec3(sin(time*0.1)*32.0,1.5,cos(time*0.1)*32.0);
  vec3 prp=vec3(sin(time*0.1-0.1)*32.0,1.5,cos(time*0.1-0.1)*32.0);
  float vpd=1.5;  
