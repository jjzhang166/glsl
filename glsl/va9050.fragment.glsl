		...
		vec4 col(float tick, float v) {
			float r = 0.166 * PI + tick * 0.05;
			float g = 0.5*PI + tick * 0.05;
			float b = 0.5*PI + tick * 0.1;
		...
		void main( void ) {
			...
			float sum = sin(dist(p,p1)) + sin(dist(p,p3)) + sin(dist(p,p4));
			
			gl_FragColor = col(tick,sum*30.0);
		}
