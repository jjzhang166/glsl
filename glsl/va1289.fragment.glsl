...
vec3 hairColor0 = rgb(0, 200, 234);
vec3 hairColor1 = rgb(0, 135, 192);
vec3 hairColor2 = rgb(0, 103, 151);
...
bool layer2(out vec3 color) {
	...
	float sx1 = sin(px * 0.175 + time) * 0.25 + sin(py * 0.015 + 2.0 * time) * 0.75 + 0.75;
	float sx2 = sin(px * 0.125 + time) * 0.5 + sin(py * 0.15 + 2.0 * time) * 0.5;
	float sx3 = sin(px * 0.04 + py * 0.018 + 1.5 * time) * 0.5;
	color = vec3(sx1 + sx2, sx2 + sx3, sx1 + sx3);
