...
vec4 GetRayColor(Ray ray)
{
	...
	vec4 clra = vec4(0.99, 0.6, 0.2, 1.0);
	vec4 clrb = vec4(0.2, 0.6, 0.99, 1.0)*1.2;
	vec4 clrc = vec4(0.4, 0.4, 0.45, 1.0)*0.5;
...
void main( void ) 
{
	Ray currentRay;
 float rt = time*0.25;
	GetCameraRay(vec3(10.1*sin(rt), 15.1, 10.1*cos(rt)), vec3(0.0, 0.0, 0.0), currentRay);
