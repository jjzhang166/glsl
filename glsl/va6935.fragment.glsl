...
float linear_a=2.51; 	//slider[0.1,2,1];
float linear_b=0.5; 	//slider[0.1,2,10];
float wrap=0.15; 		//slider[0.1,0.5,10]
float radius=1.0; 	//slider[0.1,1.7,10];
float scale=2.; 	//slider[6,2,10];
float angle=90.5; 	//slider[0,180,360];
const int iters=11;
float bailout=13.;	// slider[1,13,1000];
...
void main( void ) 
{	angle += sin(1.1*time)*10.250;
	vec2 pos = ( gl_FragCoord.xy / resolution.xy )-0.5 ;
	pos=surfacePosition*8.;
	vec3 col=vec3(0);
	col=mandelx(pos);
