...
vec4 averageForRadius(vec2 co, float radius) {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        for (int k = 0; k < 50; k++) {
            ...
            average += subPixel(backbuffer, co + spoint);
            n++;
...
void main( void ) 
{
    ...
    vec3 fillHSL = vec3(0);
    vec3 fill = brushRGB;
    fill = HSLToRGB(vec3(brushHSL.r, 1, 0.5));
	...
	//fill = HSLToRGB(fillHSL);
	    ...
	    brightness = max((brightness * 0.5), 0.01);
	    float minC = min(min(originalFill.r, originalFill.g), originalFill.b);  
	    fill = vec3(
		    originalFill.r,
		    originalFill.g,
		    originalFill.b
	           ) * vec3(brightness / minC);
    }
