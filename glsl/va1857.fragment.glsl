...
void main( void ) {
	vec2 dpos = sin(gl_FragCoord.xy*6.283)/100000.0;
	vec4 cPrev = (texture2D(backbuffer, gl_FragCoord.xy) * 2.
...
//	float rnd = fract(fract(time*2.15461234)*fract(time*6.634512)*fract(gl_FragCoord.x*.17673+2.1454)*fract(gl_FragCoord.y*.72435+.1672)*10000.);
	...
	l = min(l*1.01,.5);
	float a = atan(cPrev.x,-cPrev.z)*0.05;
	a = cPrev.w < .5 ? fract(gl_FragCoord.y*.007)*6. : min(a, a*a*7.);
