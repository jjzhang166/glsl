...
void main( void ) {
	...
	float d1 = rect(centeredMouse, vec2(0.01,0.01)); 
	...
	float d2 = rect(pos - vec2(0.0,0.0), vec2(0.1,0.1), 0.1); 
	...
	float d3 = rect(pos - vec2(1.0,0.0), vec2(0.1,0.1), uflash*0.2); 
	...
	float d4 = sphere(vec3(0.0,.0,0.0),  0.01);
	vec3 clr4= vec3(0.2,0.39,0.9) *d4;
