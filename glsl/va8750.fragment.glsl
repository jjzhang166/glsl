...
float wobbly (float x) {
    return sin(x/1.5)/1.3 + sin(x/2.)/1.5 + sin(x/3.)/6. + sin(x/1.0)*5.;
}

void main(void) {
    vec3 skycolor = vec3(0, 1, 1.0);
    ...
    if(u >= road_u-road_width-0.1 && u <= road_u+road_width+0.1) { 
        if(mod(v, 0.4) >= 0.2) {
	          col = vec3(0.5);	    // white stripes
        } else {
	          col = vec3(0, 0, 1); // red stripes
        }
    }
...
#ifdef TRIP_OUT
    col *= vec3(0.6+0.4*t/10., 0.6+0.4*t/5., 0.6+0.4*-t/20.);
#endif
