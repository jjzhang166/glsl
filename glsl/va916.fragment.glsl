// by @301z
// 
// Stochastic sampling anti-aliasing by @paulofalcao
//

//ok, I'm making a copy of this, only to point out that this isn't stochastic, and it drives me nuts when people misuse this word. THIS is entirely non-random. So....whatever. Anony_gt, out.

//Hi Anony_gt
//i know vec2 rand(vec3 co); is not a true random function, but returns good enough irregular values.
//I used the term Stochastic Sampling anti-aliasing because this term is widely used in CG literature when describing this technique.
//
//Some references:
//
...
//
//http://www.cs.virginia.edu/~gfx/Courses/2003/ImageSynthesis/papers/Sampling/Antialiasing%20through%20Stochastic%20Sampling.pdf
//
//@paulofalcao
...
//Stochastic sampling anti-aliasing stuff start
uniform float time;
...
vec2 position() {
  ...
  return vec2(((gl_FragCoord.x+rnd.x) - resolution.x), (gl_FragCoord.y+rnd.y)) / resolution.y;
}
//Stochastic sampling anti-aliasing stuff end
...
vec3 mouseColor0 = rgb(78, 53, 40);
vec3 mouseColor1 = rgb(152, 78, 69);
vec3 mouseColor2 = rgb(170, 92, 83);
...
bool mouse(out vec3 color) {
  ...
  if (inside(ellipse(vec2(-1.01, -0.04), vec2(-1.095, 0.02), 1.4))
    ...
    color = inside(ellipse(vec2(-1.0, -0.09), vec2(-1.092, 0.002), 1.34)) ? mouseColor2 : mouseColor1;
    return true;
  }
  color = mouseColor0;
  return true;
...
void main() {
  ...
  //More Stochastic sampling anti-aliasing stuff start

  vec2 position = ( gl_FragCoord.xy / resolution.xy );
  vec4 backpixel = texture2D(backbuffer, position);
  gl_FragColor = vec4(filter(ikachan(color) ? color : backgroundColor), 1.0)*(1.0/maxaa)+backpixel*((maxaa-1.0)/maxaa);

  //More Stochastic sampling anti-aliasing stuff end
