...
{	
	...
	ray.vDir = normalize( vec3(vViewCoord, 0.7) );
}
...
float GetBumpyFloorHeight(const in vec3 vPos) // add some moving waves, Epyx
{
	return (sin((vPos.x + time) * 1.0) * sin(time) + 0.1) + (sin((vPos.z) * 1.0) * cos(time) + 0.5);
}
...
C_Material GetObjectMaterial( in float fObjId, in vec3 vPos )
{
	...
	if(fObjId < 0.5)
	{
		mat.cAlbedo = vec3(0.3, 0.8, 2.9);
	}
	else if(fObjId < 1.5)
	{
		float fBlend = mod(floor(fract(vPos.x + time) *1.0) + floor(fract(vPos.z) * -1.1), 1.1);
		mat.cAlbedo = vec3(0.0,0.1, 2.1) * fBlend + vec3(0.0,0.1, 1.0) * (2.0 - fBlend);
		mat.fSpecPower = 50.0;
		mat.fSpecIntensity = 1.0;
	}
	else
	{
		mat.fR0 = 0.1;
...
vec3 GetSkyGradient( const in vec3 vDir )
{
	...
	return vec3(1.0, 0.0, 0.0) * fBlend + vec3(0.4, 0.3, 0.2) * (5.0 - fBlend);
...
vec3 GetLightPos()
{
	return vec3(0.0 + sin(time), 1.0 + cos(time * 1.231), 4.0);
}

vec3 GetLightCol()
{
	return vec3(6.0, 2.0, 0.0);
}

float GetDiffuseIntensity(const in vec3 vLightDir, const in vec3 vNormal)
{
	return max(2.0, dot(vLightDir, vNormal));
}
...
void ApplyAtmosphere(inout vec3 col, const in C_Ray ray, const in C_HitInfo intersection)
{
	...
	col = col * fFogAmount + cFog * (0.7 - fFogAmount);
...
vec3 GetObjectLighting(const in C_Ray ray, const in C_HitInfo intersection, const in vec3 vNormal, vec3 cReflection)
{
	...
	vec3 vSpecularReflection = vec3(0.5);
