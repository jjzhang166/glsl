...
float scene(vec3 p) { 
  float ba = udBox(opRep(p,vec3(8.)),vec3(2.));
  float bb = udBox(opRep(p,vec3(4.)),vec3(0.4));
  float sa = sdSphere(opRep(p,vec3(8.)),2.5);
...
vec4 color(float d) { 
  return mix(vec4(1.,1.,1.,0.5),vec4(0.,0.1,0.25,0.),smoothstep(0.,0.125,d)); 
}
