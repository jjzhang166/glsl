...
const float 	SPEED = 9999.0; // This is how fast the fractal becomes less "bloomed". Smaller => faster.
const vec2 	START = vec2(0.32, 0.61); // Starting coordinates
float t = sin(time);
vec3	COLOR_WEIGHT = vec3((1.+t/5.), (1.+t/8.), (1.+t/7.))/MAXITERATIONS; // Relative weights of the colours used in the fractal.
//-------------------------
...
void main( void ) {
	...
	for (float i = 0.0; i < MAXITERATIONS; i += INCREMENT){
		...
		y = (2.0*x*y) + y0;
		xtemp *= 1.0 + 0.0002*cos(i*0.3+(0.23*time));
		x = xtemp + x0;
