...
void main( void ) {
	...
	vec4 lmd = texture2D(bb,vec2(0.0));
	...
	float bsize = 20. ;//clamp(surfaceSize.x,1.0,256.);
	...
	//if(p.y < 1.5 && p.x < 1.5)
	if(  all (lessThan( p, vec2(1.5))) )
	{
		//Attempting to lessen the inaccuracy a bit.
		gl_FragColor = vec4( 	clamp(mouse.x*2.,0.,1.), 
				    	clamp(mouse.y*2.,0.,1.), 
				    	clamp((mouse.x-0.5)*2.,0.,1.), 
				    	clamp((mouse.y-0.5)*2.,0.,1.));	
	}
