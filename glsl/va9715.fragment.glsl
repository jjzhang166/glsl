...
void main( void ) {
	...
	vec3 c =rotate( vec3(1.,0.,0.), vec3(cos(time*0.1),sin(time*0.1),cos(0.1*time)),time*0.5 );
	 p=rotate(p-c, c,time*0.05);
	vec3 color=tan(p.xyz*15.0*p.xzy*p.yzy) ;

	gl_FragColor = vec4(rotate(color,vec3(mouse.x,cos(time*0.75),mouse.y),mouse.x*6.28), 1.0 );
}