...
void main( void ) {
	...
	vec2 position = (( gl_FragCoord.xy / resolution.xy ) - .5) * sqrt(vec2(resolution.x/resolution.y,resolution.y/resolution.x)) + .5;
	...
	for (float i = 5.; i < 100.; i+=10.) {
		...
		float e = exp(-a*a*.04)*mix;
		csum += cos(av)*e;
		ssum += sin(av)*e;
		//particle = max(particle,exp(-30000.*dot(p,p))*mix);
	}
	
	gl_FragColor = vec4(sqrt(csum*csum+ssum*ssum)*.13*(1.-particle), 1.0 );
}