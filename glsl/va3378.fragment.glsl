...
float Krysler_311(vec2 p)
{
	p *= 2.0;
	float n = 4.0;
	float j = pow(p.x,4.0)+abs(p.x);
	j*=j;
	float xx = pow(p.x, n)-abs(p.x);
	xx += j*j;
	float yy = pow(p.y, n)-abs(p.y); 
	yy += j;
	float w = 1.0-sqrt(xx+yy) + sqrt(xx-yy);
	w+=-j;
...
float Krysler_312(vec2 p)
{
	p *= 1.0;
	float n = 4.0;
	float j = pow(p.x,4.0)+abs(p.y);
	j*=j;
	float xx = pow(p.x, n)-abs(p.x);
	xx += j*j;
	float yy = pow(p.y, n)-abs(p.y); 
...
float Krysler_313(vec2 p)
{
	...
	float xx = pow(p.x, n)-abs(p.x);
	xx += j*j;
	float yy = pow(p.y-p.x, n*2.0)-abs(p.y); 
	yy += j;
	float w = 1.0-sqrt(xx+yy) + sqrt(xx-yy);
	w+=-j;
	float ww = pow(p.x, -n)-abs(p.x);
	
	return min(w,ww*0.05);
	
}


float Krysler_314(vec2 p)
{
	float len = length(p);
	...
	float xx = pow(p.x, n)-abs(p.x);
	xx += j*invlen;
	float yy = pow(p.y-p.x*p.x, n*2.0)-abs(p.y); 
	yy += j*j;
	float w = 1.0-sqrt(xx+yy) + sqrt(xx-yy);
	w+=-j;
...
float Krysler_315(vec2 p)
{
	...
	p.x *= 0.5;
	float n = 4.0;
	float j = pow(p.x,2.0)+abs(p.y)-0.5;
	j*=j*len*len;
	float xx = pow(p.x-0.5, n)-0.25-abs(p.x-0.125);
	xx += j*invlen*invlen;
	float yy = pow(p.y-p.x*p.y, n*4.0)-abs(p.y-0.25)-p.x*1.5; 
	yy += j*j;
	float w = 1.0-sqrt(xx+yy) + sqrt(xx-yy);
	w+=abs(p.y)*0.25;
	w+=-j;
...
float Krysler_360(vec2 p)
{
	...
	float x = abs(p.x*PI*2.0);
	float ax = min(acos(cos(x)), cos(x))+1.0;
	float y = abs(p.y*PI*len*8.0);
	float ay = min(acos(cos(x)), cos(y))+1.0;
	
	float a = 1.0-sqrt(ax+ay)*2.0;
	float b = 1.0-sqrt(ax-ay)*2.0;
	
	return max(a,b)*( 1.0-abs(p.y));
	
}

float Krysler_361(vec2 p)
{
	...
	float x = abs(p.x*PI*len*2.0);
	float y = abs(p.y*PI*len*2.0);

	float g = 1.0-min(x,y)*len;
	g -=cos(x*PI*2.0 + g*y*PI*2.0) ;
	return g ;
