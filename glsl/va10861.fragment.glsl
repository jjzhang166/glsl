...
float f(vec3 o)
{
    float a=(sin(o.x)+o.y*.25)*sin(time * 0.14) * 3.35 + 14.;
    // o=vec3(cos(time+a)*sin(time+o.x)*o.x-sin(time+a)*o.y,sin(a)*sin(time+o.y)*o.x+cos(time+a)*o.y,o.z);
    o=vec3(sin(time+a)*sin(o.x+o.z/23.0)*o.x,sin(time+a)*cos(o.y+o.z/7.0)*o.y,o.z);
...
vec3 s(vec3 o,vec3 d)
{
    ...
    if( nh > 130.0 ) return vec3(1.0,1.0,9.0);

    t = t - dt*nh/(nh-lh);

    vec3 e=vec3(-.1,sin(time) *.3,0.0);
    vec3 p=o+d*t;
...
(length(p.xy)*15.,2.)<0.5?vec3(11.0,1.,1.):vec3(.0,.0,.0))
                           ,vec3(1.0,1.0,1.5), vec3(pow(t/10.,221.5)) ) );
}

void main()
{
    vec2 p = -1.0 + sin(time) * 0.2 + 2.0 * gl_FragCoord.xy / resolution.xy;
