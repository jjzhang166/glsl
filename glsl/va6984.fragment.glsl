...
//Fork of the terrain in order to change the water to lava
#ifdef GL_ES
...
vec3 getTerrain(vec3 scrCoord, vec3 rd, float dis) {
	...
	if (pos.y <= water) {
		...
		col.x = .6 + .40 * NoiseNonLinear(pos.x*74.0+offTime, pos.z*74.0+offTime*4.1);
		col.y = .1 +  .1 * NoiseNonLinear(pos.x*44.0+offTime, pos.z*44.0+offTime*4.1);
		
		mat = col;
		
		//no fog applied to lava
		dis = 0.0;
		...
		if (pos.y < .35 && nor.y > .1) {
			vec3 m = vec3(clamp(NoiseNonLinear(pos.x*98.0, pos.z*98.0)*.55+.0, 0.0, .5), 0.0, 0.0); 
			mat = mix(mat, m, clamp((nor.y-.1)*4.0, 0.0, 1.0));
		}
		mat = mix(mat, vec3(.3, .2, .1), NoiseNonLinear(pos.x*33.3,pos.z*33.3));
		mat = mix(mat, vec3(.8, .3, .2), NoiseNonLinear(pos.x*111.1,pos.z*111.1));
...
{
    //Camera animation
    vec2 vPos=(-1.0+2.0*(gl_FragCoord.xy/resolution.xy)) * 1.3;
    vec3 vuv=vec3(-.33+stime*0.35,1.4 ,stime * .23);     //view up vector
    vec3 vrp=vec3(sin(offTime*3.14)*35.0, .1, cos(offTime*2.14)*31.0); //view reference point
    ...
    {
        gl_FragColor = vec4(getTerrain(scrCoord, rd, dis), 0.5);
    }else
    {
        gl_FragColor= vec4(getSky(rd, light), 0.5);
    }
}
