// rotwang: @mod* clamping around for an alien landscape
// (was IQ's apple)
#ifdef GL_ES
...
float cubic_apple_terrain( vec3 p )
{
	
	float c = 1.5;
p.xz= mod(p.xz,c)-0.5*c;	
p.xz = min(vec2(0.5), abs(p.xz));

   p.y -= 1.25*pow(dot(p.xz,p.xz),0.75);
	float n = 16.0;
...
float displace( vec3 p )
{
	vec3 q = p;
		float c = 1.8;
q.xz= mod(q.xz,c)-0.5*c;	
q.xz = min(vec2(0.5), abs(q.xz));
	
    float d1 = cubic_apple_terrain(p);
    float d2 = fbm(q*10.0+(1.0*p.y))*1.33;
    return d1+d2;
}
...
vec3 appleColor( in vec3 pos, in vec3 nor, out vec2 spe )
{
    ...
    // green
    float f = smoothstep( 0.1, 1.0, fbm(pos*1.5) );
    col = mix( col, vec3(0.8,1.0,0.2), f );

    // dirty
    f = smoothstep( 0.0, 1.0, fbm(pos*8.0) );
    col *= 0.8+0.2*f;
    ...
    f = fbm( vec3(a*12.0 + pos.z,6.0*pos.y,pos.x)*2.0);
...
void main(void)
{
    ...
    // camera
    vec3 ro = 3.5*normalize(vec3(cos(0.2*time),1.5+0.45*cos(time*.31),sin(0.2*time)));
    ...
    if( tmat.y>0.5 )
    {
        ...
        vec3 lig = normalize(vec3(1.0,0.3,-3.9));
     
        float con = 1.0;
        float amb = 0.2 + 0.2*nor.y;
        float dif = max(dot(nor,lig),0.0)*4.0;
        float bac = max(0.1 + 0.1*dot(nor,vec3(-lig.x,lig.y,-lig.z)),0.0);
        ...
        col += 0.30*dif*vec3(1.00,0.7,0.5)*vec3(sh, (sh+sh*sh)*0.5, sh*sh );
        col += 0.15*bac*vec3(1.00,0.57,0.75);
