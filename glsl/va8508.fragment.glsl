...
{
	float delta = 0.5*sin(time*4.)+0.5;
	...
	float wires = pointToBoxVide(p, vec3(0.2,0.2,0.2), 0.05);
	return mix(wires,box,delta);
...
void main( void ) {
	...
	float shade = dot(normal,-raydir);
	gl_FragColor = vec4(vec3(shade, shade, shade*0.4+0.4),1);
