//MG
//Mod by MrOMGWTF :P
//Changed colors, added multi-sampled motion blur
#ifdef GL_ES
...
void main() {
	...
	vec4 col=texture2D(backbuffer,vec2(q.x - offset,q.y));
	col += texture2D(backbuffer,vec2(q.x + offset,q.y - offset));
	col += texture2D(backbuffer,vec2(q.x,q.y - offset));
	col += texture2D(backbuffer,vec2(q.x,q.y + offset));
	col += texture2D(backbuffer,vec2(q.x + offset,q.y + offset));
	col += texture2D(backbuffer,vec2(q.x - offset,q.y - offset));
	col += texture2D(backbuffer,vec2(q.x + offset,q.y - offset));
	...
	gl_FragColor+=vec4(pow(vec3(dist * 4.0), vec3(1.05, 1.1, 0.9)),1.0);
	gl_FragColor+=vec4(pow(vec3(fbm(p+mse)*0.025), vec3(1.1, 1.1, 0.9)), 1.0);
}
