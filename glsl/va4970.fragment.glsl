...
void main(void)
{
	...
	p[0] = vec2(resolution.x*.140,resolution.y*.4);
	p[1] = mouse*resolution;
	p[2] = vec2(resolution.x*.760,resolution.y*.6);
	vec2  oldcontrolpoint=p[1];
	// make the bez pass thru the control point
	p[1]+=(p[1]-((p[0]+p[2])*0.5));
	...
	// Curve Control point
	d = min(d, length(oldcontrolpoint - position)-4.0);
	
	d = clamp(d, 0.0, 1.0);
	d = mix(0.8, 0.0, d);
	gl_FragColor = vec4(d,d*0.3,d*15.0, 1.0);
}