...
// no mouse mod by mkjpboffi.tumblr.com
...
vec2 map( in vec3 pos )
{
	...
	float f;
	vec3 p = pos;
	float c = 1024.;//p.x+=time/c*256.;
	float sign1 = 1.;
	for(int i=0;i<5;i++)
	{sign1*=-1.;
		f+=c*(10.*sin(p.z*5.+sin(p.x*3.)-time*2.0)+sin(sin(p.z*5.-time*2.0)+p.x*10.)+cos(p.z*5.-p.x*10.));p*=2.0;c/=2.;
	}

    	vec2 res = vec2(pos.y + f/10240.0*(sin(time/2.0)+0.5)
			,1.0);//opU( vec2( sdPlane(     pos), 1.0 ), vec2(r, (abs(pos.y) * 200.0 + 700.0)));
