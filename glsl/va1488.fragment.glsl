...
vec4 c( vec3 p, float d )
{
	vec4 v = snoise(p*0.5);
	v += max(0.,min(1.,(6.25-d)*4.))* 2.*vec4(-2.0*p.x, -2.0*p.y, 0.0, 1.0)/(p.x*p.x+p.y*p.y+1.0);
...
void main()
{
	...
	for( int i = 0; i < iter; ++i )
	{
		...
		if (dot(v.xy,v.xy) < 1.7 && abs(atan(v.y,v.x)) > abs(fract(time*2.6)-0.5)*1.7 && b.z>5.6) {
			t += h*float(iter-i)*vec3(1,1,-2)*8./(.2+pow(b.z-6.,2.));
		}
	}
	t = t / float(iter*iter);
	gl_FragColor = vec4( sqrt(max(vec3(0),t)), 1. );
}
