...
// last tweak/remix by anony_gt :-) 9/12/11 or 12/9/11
#ifdef GL_ES
...
//IQs Torus (try other objects http://www.iquilezles.org/www/articles/distfunctions/distfunctions.htm)
vec2 obj1(in vec3 p){
  //obj deformation
  p.x=fract(p.x+0.5)-0.5;
  p.z=fract(p.z+0.5)-0.5;
  p.y=p.y-1.0+sin(time*.2+cos(time*.2))*(p.x*p.x+p.z*p.z);
  vec2 q = vec2(length(p.xz)-(0.3),p.y);
  return vec2(length(q)-0.1);
 // return vec2(length(p)-p.x);
}

//RoundBox with simple solid color
vec3 obj1_c(in vec3 p){
	return vec3(
		sin(p.z*p.z+p.y*p.y+1.0*time),
		0.,
		sin(p.y*p.y+p.x*p.x+1.0*time)
);
}
