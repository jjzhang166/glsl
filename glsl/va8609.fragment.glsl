...
void main (void) {
        vec2 pos = gl_FragCoord.xy + vec2(sin(time) * 0.0, 0.0);
	//Mario became Michael Jackson!
	//He can do the moon walk!
	vec2 mariomin = vec2(resolution.x / 2.0 - 8.0, 32.0);
	vec2 mariomax = vec2(resolution.x / 2.0 + 8.0, 48.0);
	if (within(vec2(pos), mariomin, mariomax)) {
		gl_FragColor = vec4(mario(vec2(pos.xy) - mariomin, time*0.75), 1.0);
	} else if (pos.y < 32.0) {
		gl_FragColor = vec4(block(vec2(pos.x - time * 50.0, pos.y)), 1.0);
	} else {
