...
void main( void ) {
	...
	vec2 sect = floor(clamp(p - resolution/2.,0.,1.)+0.5);

	vec3 c = vec3(0);
	
	vec2 qres = resolution/4.;
	
	if(sect == vec2(0,0))
	{
		c = ConicalGrad(qres,mouse*resolution,p);
	}
	if(sect == vec2(1,0))
	{
		c = RadialGrad(vec2(qres*vec2(3,1)),mouse*resolution,p);
	}
	if(sect == vec2(1,1))
	{
		c = SquareGrad(vec2(qres*vec2(3,3)),mouse*resolution,p);
	}
	if(sect == vec2(0,1))
	{
		c = LinearGrad(vec2(qres*vec2(1,3)),mouse*resolution,p);
	}
