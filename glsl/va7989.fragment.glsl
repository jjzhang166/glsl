...
void main(void) {
	// the gear
	vec3 camera = vec3(resolution.x * 0.5, resolution.y * 0.5, resolution.x*tan(90./360.*3.14159));
	vec3 screen = vec3(gl_FragCoord.xy, 0.0);
	vec3 light  = vec3(resolution.x * mouse.x, resolution.y * mouse.y, resolution.x*tan(90./360.*3.14159));
	
	// sphere 1 (blue)
	vec3 sphere_1_center = vec3(500.0, 500.0, -50.0);
