...
void powN1(inout vec3 z, float r, inout float dr, in int i) {
	...
	float theta = acos(z.z/r)+.5*float(i+0)*(time/4.0);
	float phi = atan(z.y,z.x)+.5*(float(i)+0.1)*(time/4.0);
...
void main(void){
  ...
  vec3 prp=vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*1.3; //Trackball style camera pos
