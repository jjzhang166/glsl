...
vec4 bg = vec4(0.0, 1.0, 1.0, 1.0); 
vec4 line = vec4(1.0, 1.0, 1.0, 1.0);
vec2 uv;

vec3 pointcolor(vec4 off) {
   vec2 point = vec2(0.5 + sin(time * off.x) / off.y, cos(time * off.z) / off.w);
   vec3 pulse = vec3(sin(time * 2.0) + .5, sin(time * 2.0) + .5, cos(time * 7.0) + .5) * 0.2;
...
void main(void)
{
   uv = (gl_FragCoord.xy / resolution.xy) - vec2(0.0, 0.1);
   ...
   color += pointcolor(vec4(0.9, 3.0, 0.6, 3.0));
   color += pointcolor(vec4(0.6, 6.0, 0.4, 3.0)); 
   color += pointcolor(vec4(0.6, 3.0, 0.5, 3.0));
   color += pointcolor(vec4(0.1, 6.0, 0.4, 4.0));
