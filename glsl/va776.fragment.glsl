// by @301z
//
// with some plane deformations by @paulofalcao
...
vec2 distortion(vec2 p){
  p.x=p.x+sin(p.y*2.0+time*3.0)*0.1;
  p.y=p.y+cos(p.x*2.0+time*5.0)*0.1;
  p=p-0.5;
  p=rot(p,sin(time*0.3));
  p=p+0.5;
  return p*(sin(time)+2.0);
}
