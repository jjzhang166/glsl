...
void main( void ) {
	...
	float bounds_sqrd = (4.0 + (2.*cos(2.*time)));
	bounds_sqrd *= bounds_sqrd;
	...
	for (float i = 0.0;i < maxIterations; i += 1.0)
	{
		...
		if (dot(z, z) > bounds_sqrd)
		{
			gl_FragColor = vec4(lookupCol(i / maxIterations), 1.0) + 0.5 * vec4(normalize(z), vec2(1., 1.));
			break;
		}
	}
}