...
float building(vec3 p) {
    ...
    vec3 b = vec3(box*0.5, sin(time*.6)*.8+1., 1.5);
...
float city(vec3 p) {
    ...
    float hv = -mod(ceil(p.x/40.0), 3.0)*2.0;
...
void main( void ) {
    ...
    vec3 cam = vec3(sin(time*0.1)*80.0, mouse.y*25.0+30.0, sin(time*0.15)*40.0);
