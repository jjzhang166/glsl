...
vec3 shader( vec2 p, vec2 resolution ) {
	...
	float wave = 1.-abs(pos.y - (sin((cos(pos.x + cos(time) +(.2*sin(time /2.))))*PI*2.+cos(time))*.25+0.99)); // 'just string together a bunch of random stuff' remix
	
	color = vec3(0.,pow(wave,20.),0.);
	color += vec3(pow(wave,30.));
	
	return vec3(color/0.1);
...
void main( void ) {
	...
	color *= abs(sin(rposition.y*2.0+(time*16.0))*.15+0.85);
	
	color *= vec3(clamp( abs(triwave(dposition.x/pixelsize))*2.0 , 1. , 1.4 ));
	color *= vec3(clamp( abs(triwave(dposition.y/pixelsize))*3.0 , 1.0 , 1.0 ));
