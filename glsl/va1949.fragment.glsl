...
void main(void)
{
	...
	for (float i = 1.0; i < stars; i++)
	{
		...
		float fade 	= (depth - z) / 0412.0;		
		gradient 	+= ((fade / depth) / pow(length(pos_center - blob_coord), 2.8));
	}
	float r = abs(1.13);
	//float g = abs(sin(time * 2.23));
	///float b = abs(sin(time * 3.33));

	gl_FragColor = vec4(gradient * r, 2, 3, 1.0);
}