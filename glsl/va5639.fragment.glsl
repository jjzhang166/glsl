...
const vec4 secondColor = vec4(1,0,0,100);
const float r1 = 200.0;
const float r2c = 100.0;

void main( void ) {
	highp float r2 = (sin(2.0 * time) + 1.0) * 30.0 + 20.0;
	highp float x1 = 300.0;
	highp float y1 = 900.0;
	highp float x2 = (cos(time) + 1.0) * 100.0 + 100.0;
	highp float y2 = (cos(time) + 1.0) * 100.0 + 100.0;
