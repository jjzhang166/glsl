...
vec2 obj1(in vec3 p){
  vec3 uv = floor(p);
  float m = (fract(sin(uv.x+uv.z*1e2)*1e6));
  p.y += m*15.-15.;
  //obj deformation
  p.x=fract(p.x)-0.5;
  p.z=fract(p.z)-0.5;
  p.y=clamp(p.y,-0.5, .5);
  return vec2(length(max(abs(p)-vec3(0.25,0.25,0.25),0.0))-0.1,1);
}
...
void main(void){
  ...
  for(int i=0;i<256;i++){
    ...
    gl_FragColor=mix(vec4(.0), vec4((b*c+pow(b,8.0))*(1.0-f*.02),1.0), 1.-clamp(pow(f,.2)-1.2, 0., 1.));//simple phong LightPosition=CameraPosition
