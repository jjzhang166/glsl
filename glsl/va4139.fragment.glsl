...
vec2 rot2(in vec2 p, float a) {
	mat2 m = mat2 (cos(a),-sin(a),sin(a),cos(a)
		);
	return p*m;
}

//(try other objects http://www.iquilezles.org/www/articles/distfunctions/distfunctions.htm)
vec2 obj1(in vec3 p){
	
	float d = p.z + 8.;
	
	vec2 n0 = cellular(p.xy/0.9)*0.15;
	vec2 n1 = cellular(p.yx/0.5)*0.2;
	vec2 n2 = cellular(p.xy/3.)*0.3;
	vec2 n3 = cellular(p.xy/19.)*3.;
	vec2 n4 = cellular(p.xy/70.)*10.;
...
void main(void){
  ...
  vec3 prp=vec3(-sin(mouse.x+2.6)*20.0,sin(mouse.y+2.6)*20.0,8.0); //Change camera path position here
