...
void main( void ) {
	...
	float filter = sigmoid(pow(2.,5.8)*(length((position/resolution-mouse + 0.5)*vec2(1.,resolution.y/resolution.x ))-0.125))*0.5 +0.5;
	position -= (mouse-0.5)*resolution;
	position *= c1;
	position = mix(position, position2, filter);
