...
float map( vec3 p )
{
    p = mod(p,vec3(1.0, 1.0, 1.0))-0.45;
    return abs(length(p.xy)-.1)*tan(time);
}
...
void main( void )
{
    ...
    float fog = 5.0;
    vec4 result = vec4( vec3(c*.2 , c*.1, c*1.) * (fog - total_d) / fog, 1.0 );

    ray.z -= 5.+time*.5;
    float r = rand(ray, 33.);
    //gl_FragColor = result*(step(r,.1)+r*.2+.1);
    gl_FragColor = result*tan(cos(sin(time/r)*2.));	
}
