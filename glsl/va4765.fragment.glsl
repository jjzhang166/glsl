...
//@fernozzle MOD! The bubbles inhale other bubbles.
...
void main( void ) {
	...
	for(int i=0; i<count; i++){
		...
		//color += 1.0/pow(dist-radius-0.006, 1.4)/10000.0;
		color *= 1.0/pow(dist-radius-0.006, 1.4)/800.0;
		
		color1 += vec3(color*(position(float(i*7)).x+1.0), color*(position(float(i*2)).x+1.0), color*(position(float(i*3)).x+1.0));
		uPos -= vec2((position(time+float(i)*10.0)- uPos) * 0.1) * pow(dist, -0.9);
	}
