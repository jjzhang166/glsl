// Raymarching a spherical heightfield
// Matthijs De Smedt
// matthijs@anji.nl

// Todo:
//  Improve shadowing by completing the sphere intersection code..
...
void main(void)
{
	...
	if(inter.t > 0.0)
	{
		...
		vec3 lightDir = normalize(light-inter.point);
		shadowInter.ro = inter.point;
		shadowInter.rd = lightDir;
		shadowInter.point = inter.point+lightDir*0.1;
		...
		// Some crazy colors
		vec3 diffuse = normalize(normalize(inter.point-sphere.xyz)+vec3(abs(sin(time/4.0)),abs(cos(time/5.0)),abs(sin(time/5.0)*2.0)));
		color.xyz = lighting(inter.point, inter.normal, light, diffuse) * shadowFactor;
