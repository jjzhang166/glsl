...
#define DELTA (1.0/max(resolution.x,resolution.y))
...
float fs = exp(log(1.0)*(1.0 - mod(time, 1.0)) + log(8.0)*mod(time, 1.0));

float polkadot(vec2 id, vec2 p) {
	float ofs = log(id.x) / log(2.0);
	p = abs(rotate(p, radians(ofs * 30.0))*fs);
	p -= id;
	
	float c = mix(0.1, 1.0, (sin(radians(ofs * 30.0 * 4.0)+time*120.0)+1.0)*0.5);
	
	return (length(p - 0.0618*id) - id.x*c*0.5) / fs;
}

// final evaluator
float position_to_distance(vec2 p) {
	vec2 id = vec2(logscale(length(p*fs)));
	return min(min(polkadot(id, p), polkadot(id * 2.0, p)), polkadot(id / 2.0, p));
}
