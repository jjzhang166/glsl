...
vec2 fblob(vec3 p) {
	return vec2(length(p)-1.0-0.8*length(pow(abs(cos(normalize(p)*7.)),vec3(0.1))),1);
	// just fiddling a bit with the shapes
}
...
void main(void){
  ...
  if (f<MAX_DEPTH){
    ...
    gl_FragColor=pow(vec4((b*c+pow(b,8.0))*(1.0-f*.01),1.0),vec4(1.0/1.8));//simple phong LightPosition=CameraPosition
  }
