...
{	
	...
	else
	{
		...
		if(abs(position.x) < 100.0 && position.y > 0.0 && position.y <60.0) color = vec3(1.2, 1.2, 1.2);
		else if(abs(position.x) < 100.0 && position.y < 0.0 && position.y> -60.0) color = vec3(1.2, 0.0, 0.0);
