...
const Material _matFloor = Material (vec3(1,.9,.7), vec3(0.5,0.5,0.5), 130.0);
const Plane _floor = Plane (vec3(1,1,0), vec3(0,0,1));
...
vec3 Phong (PointLight light, Material material, Ray normal, vec3 eye)
{
	...
	vec3 lightDir = lightVec * inversesqrt (lightDistance2);
	...
	vec3	f0	= 0.2 + 0.8*(1.0 - 0.2) * material.SpecularColor;
	float nh = 1.0 - max(0.0, dot(normal.Direction, -viewDir));
	vec3	fresnel	= f0 + (1.0 - f0) * pow(nh, 5.0);
	
	vec3 color =
		max (0.0, diffuse) *fresnel* light.DiffuseColor * light.DiffusePower * material.DiffuseColor +
		max (0.0, diffuse) *fresnel*specular * light.SpecularColor * light.SpecularPower * material.SpecularColor;
	float fog = 1.0 - min(distance(normal.Position, light.Position) / 15.0, 1.0);
	return fog * color * 110.0 / lightDistance2;
}
...
bool LightScene (inout Ray ray, inout vec3 color)
{
	...
	for (int iLight = 0; iLight < _cLights; ++iLight)
	{
		...
		{
			float fog = 1.0 - min(distance(bounce.Normal.Position, ray.Position) / 15.0, 1.0);
			color += fog * Phong (_rgLights[iLight], bounce.Material, bounce.Normal, ray.Position);
		}
	}
...
void main( void )
{
	...
	_rgLights [0] = PointLight (4.0*vec3(1,0.,2), vec3(.5,1,.5), .3, vec3(.5,1,.5), 2.0);
