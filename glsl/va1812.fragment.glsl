// The Spikeball
// Stolen from las's http://glsl.heroku.com/e#1802.0
// and stuffed into @paulofalcao's raymarcher http://glsl.heroku.com/e#299
...
vec2 spikeball(vec3 p) {
   ...
   return vec2(length(q)-2.5*pow(1.5,b.x*(1.-mix(.3, 1., sin(time*2.)*.5+.5)*b.x)), 1.0);
}
...
//Objects union
vec2 inObj(vec3 p){
  return ObjUnion(obj0(p),spikeball(p));
}
