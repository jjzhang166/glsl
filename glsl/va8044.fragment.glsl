...
{
	float rbl = 1.2;
	vec3 rbb = vec3(rbl,rbl,rbl);
	float rbr = 0.2;
	
	float box = length(max(abs(p)-rbb,0.0))-rbr;
	
	float a =0.;
	float b = 0.;
	float polyfoldOrder = 5.0 + sin(time)*2.0;
	float R1 = 2.;
	float R2 = 0.8;
	float R3 = 1.0;
...
void main(void)
{
	...
	cam.position = vec3(sin(t)*3.0, sin(t) * 7.0, cos(t)*7.0);						// The position of the camera
