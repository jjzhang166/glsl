...
void main(void)
{
	...
	vec3 lightcol = vec3(1.0,0.9,0.6)*2.0;
	...
	vec3 groundcol = (lightcol + ambcol) * 0.5;// * clamp(dot(vec3(0.0,1.0,0.0),light),0.0,1.0);
	...
	if (id > 0.5 && id < 1.5){
		...
		for(float i=0.0;i<5.0;i+=1.0){
			...
			totalspec += pow(abs(dot(light,refl)),200.0) * (clamp(dot(nor,light)+0.1,0.0,1.0)) * facetintensity;// * fre;
		}
