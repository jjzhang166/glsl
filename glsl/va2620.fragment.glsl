...
vec3 DomainRotateSymmetry( const in vec3 vPos, const in float fSteps )
{
    ...
    float steppedAngle = (floor(angle * fScale + 8749464.897)) / fScale;
...
{
	const float fRadius = 1.25;
	const float fThickness = 0.3;
...
vec3 WarpDomain( const in vec3 vPos )
{
    ...
    vResult.y += fDroopBlend * 0.3;
    vResult.z += sin(vResult.x * 21.0  + vResult.y * 61.0 + GetTime()*5.1) * fDroopBlend * 0.09;
    return vResult;
}
...
C_Material GetObjectMaterial( const in vec3 vObjId, const in vec3 vPos )
{
	...
	vBrickPos = fract(vBrickPos * vec2(1.0, 3.0));
        ...
        mat.cAlbedo = vec3(sin(time), cos(time), -sin(time)) * (0.3 + fFade * 0.6);
    }
    else
    if(vObjId.x < 2.5)
    {
    ...
    if(vObjId.x < 4.5)
    {
        ...
        mat.fR0 = 0.01;
        mat.fSmoothness = 0.9;
        mat.cAlbedo = vec3(1.0, 0.00, 0.00);
    }
    else
    if(vObjId.x < 5.5)
    {
...
#define kRaymarchEpsilon 0.20001
#define kRaymarchMatIter 50
#define kRaymarchStartDistance 1.0
