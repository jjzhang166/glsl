...
{
    float v = dtrefle(p,10.*b );
    vec2 g =(grad(10.*p,0.1*b ));
...
void main( void ) {
	...
	vec3 p=vec3(position,0.5-2.0*mouse.y+2.*mouse.x);
	vec3 cc=vec3(sin(time*0.4),cos(time*0.4),sin(time*0.2));
	 p=rotate(p*p-cc,cc,time*0.1)+cc;
	p=4.*p*p*cc+p;
	vec2 colr=color(p.xy,length(p));
	gl_FragColor = vec4(colr.x,colr.y,(colr.x+colr.y), 1.0 );
}
