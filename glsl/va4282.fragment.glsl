...
float a=a0*1.1415926535897932384626433832795/180.0;
  return mat3(
...
void main(){
  ...
  mat3 rot=genRotMat(sin(time/4.13)*54.0,1.0,0.0,0.0);
  rot=rot*genRotMat(sin(time/4.64)*54.0,0.0,1.0,0.0);
  rot=rot*genRotMat(sin(time/4.24)*54.0,0.0,0.0,1.0);
  vec2 p=vec2(0.0,0.0);
  ...
  for(int i=0;i<1000;i++){
    ...
    else tempc.z=1.0/tempc.z*sin(time*5.9);
    val=cubeDist(temp);
