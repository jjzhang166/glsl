...
void main( void ) {
	...
	vec2 position= (( gl_FragCoord.xy / resolution.xy ) * 2.0) - 1.0 + (mouse - 0.5);
	vec3 dir = normalize(vec3(position.x * (resolution.x / resolution.y), position.y, -1.0));
	
	float di = sin(time * 0.25) * 5.0;
	if(di < 0.0) di  *= -1.0; 
	di += 1.0;
	
	sphere[0].c 	= vec3(0.0, 0.0, -3.0);
	sphere[0].r	= 0.5;
	sphere[0].col	= vec3(1.0, 1.0, 1.0);
	
	sphere[1].c	= vec3(sin(time) * di, 0.0, cos(time) * di -3.0);
	sphere[1].r	= 0.5;
	sphere[1].col	= vec3(1.0, 0.3, 0.3);
	
	sphere[2].c	= vec3(sin(time + 1.05) * di, 0.0, cos(time + 1.05) * di -3.0);
	sphere[2].r	= 0.5;
	sphere[2].col	= vec3(1.0, 1.0, 0.3);
	
	sphere[3].c	= vec3(sin(time + 2.1) * di, 0.0, cos(time + 2.1) * di -3.0);
	sphere[3].r	= 0.5;
	sphere[3].col	= vec3(0.3, 1.0, 0.3);

	sphere[4].c	= vec3(sin(time + 3.15) * di, 0.0, cos(time + 3.15) * di-3.0);
	sphere[4].r	= 0.5;
	sphere[4].col	= vec3(0.3, 1.0, 1.0);
	
	sphere[5].c	= vec3(sin(time + 4.2) * di, 0.0, cos(time + 4.2) * di -3.0);
	sphere[5].r	= 0.5;
	sphere[5].col	= vec3(0.3, 0.3, 1.0);
	
	sphere[6].c	= vec3(sin(time + 5.25) * di, 0.0, cos(time + 5.25) * di -3.0);
	sphere[6].r	= 0.5;
	sphere[6].col	= vec3(1.0, 0.3, 1.0);
