...
float orbiter(float x,float y,float fx,float fy,float sx,float sy,float t,float offset){
	...
	float speedS=0.15;
   float xx=x+cos(t*fx+offset)*sx-(speedS*cos(t*standardSize));
   float yy=y+sin(t*fy+offset)*sy-(speedS*sin(t*standardSize));
...
void main( void ) {
   ...
   gl_FragColor = vec4(d.x,d.y,d.z,1.0)+vec4(e.x,e.y,e.z,1.0)+vec4(moons.x,moons.y,moons.z*1.2,1.0)+vec4(SUN.x*sin(time+90.0),SUN.y*cos(time),SUN.z*sin(time),1.0);
}