...
void main( void ) {
	...
	const float cursor_size = 5.0;//gtoledo3
	float anglebleed = prevColor.x*20.1415 + time*2.0;
	vec2  anglevec = vec2(mycos(anglebleed), sin(anglebleed))*2.0 ;
	...
	perlinval  += max(0., .99999 * (1. - (length((screenPosNorm - vec2(sin(time*.2)*.5+.5,mycos(time*.2)*.5+.5)) ) * cursor_size-.5)));//feedback write._gtoledo3

	vec3 colorLight = vec3(255.0, 255.0, 169 ) / 255.0;
	vec3 colorDark  = vec3(81.0, 47, 0. ) / 255.0;
	
	float vignette = 1.5-length(screenPosNorm*2.0-1.0);
	
	vec3 curr_color = 1.-vec3(mix(colorDark, colorLight, pow( perlinval,5.0) )) * vignette;//let's make a weird snow blackhole look._gtoledo3
