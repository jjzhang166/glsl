...
//'nother oldie-gt
float f(vec3 o){	
	float a=(cos(o.x)-o.y*1.)*(sin(time*.1)+1.);
	o=vec3(cos(a)-sin(a),sin(a)*cos(a),1.-o.z);
	return dot(sin(o)/cos(o)+1.5,vec3(1.))-.5;
}
...
void main(){
	...
	gl_FragColor=vec4(s(vec3(1.,1.,sin(time)*.1), normalize(vec3((gl_FragCoord.xy-vec2(resolution.x,resolution.y)/1.75)/vec2(resolution.x),.1))),1.);
}