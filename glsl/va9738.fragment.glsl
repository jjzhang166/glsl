...
float map(vec3 p) {
	...
	k = max(k, -(length(abs( mod(p.yz, 2.0) ) - 1.0) - 1.0));
	k = max(k, -(length(abs( mod(p.xz, 2.0) ) - 2.0) - 1.5));
	return k;
}
...
float tex(in vec3 x)
{
    ...
    vec4 b = hash(n + vec4(1.0, 58.0, 114.0, 171.0));
...
void main( void ) {
	...
	gl_FragColor = vec4(vec3(.1,.5,.3)-col + t * 0.02, 1.0 );
}