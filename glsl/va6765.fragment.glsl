...
void main()
{
    ...
    float fog = min(1.0, (1.0 / float(MAX_MARCH-4)) * float(march));
    glow *= min(1.0, 4.0-(4.0 / float(MAX_MARCH)) * float(march));
    gl_FragColor = vec4(vec3(0.2+glow*0.75, 0.2+glow*0.75, 0.3+glow)*fog, 1.0);
}
