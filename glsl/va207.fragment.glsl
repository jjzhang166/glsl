...
void main( void ) {
	...
	if ((me.b < 0.01) && (length(position-mouse) < 0.05)) {
		...
		if (me.g <= 0.9) {
			if (sum == 3) {
				...
				if (me.r > decay.r) { me.r -= 0.004; }
					...
					float rnd2 = mod(fract(sin(54.0*dot(position + time * 0.01, vec2(14.9898,78.233))) * 94949.5453), 1.0);
					me = (rnd2 > 0.999) ? live : decay;
				}
				gl_FragColor = me;
			}
		} else {
