...
float scene( vec3 p, out vec4 color ) {
	...
	float mindis = 800000.0;
	vec4 finalColor;
	for(int i = 0; i < 2; i ++)
	{
	
	p = p + vec3(2.0 * sin(float(i) * 3.0), 2.0 * cos(float(i) * 3.0), 2.0 * sin(float(i)* 3.0) * cos(float(i)* 3.0));
	float anim = clamp( pow( sin(time * 3.14159 / 3.0), 3.0 ) * 1.2, -1.0, 1.0 );
	//
	p.x = p.x + (float(i)+ 1.0) * sin(0.5 * time );
	p.y = p.y + (float(i)+ 1.0) *cos(0.5 * time );
	p.z = p.z + (float(i)+ 1.0) *2.0 * sin(time ) * cos(time);
	//p.x = 0.;
	//p.y = cos(time);
	...
	p = rotY( p, time * 20.0 + mousepos.x * -180.0 );
	p -= vec3(0.0, -0.1, 0.0);
	...
	float d0 = 0.0;
	//bottom
	d0 += max(meltmax, cube( p - vec3(0.0, -0.4, 0.0), vec3(0.2, 0.2, 0.2) ) - 0.04);
	
	//bottom inside
	float d1 = 0.0;
	d1 += max(meltmax,sphere(p - vec3(0.0, -0.3, 0.0),  0.2));
	...
	d2 += max(meltmax,ballon(p - vec3(0.0, 0.5, 0.0),  0.5));
        float d3 = max(meltmax,cylinder(p - vec3(0.0, 0.2, 0.0), vec2(0.2, 0.05)));
	vec4 color2;
	if(d3 < d2)
	{
		color2 = vec4(0.9, 0.9, 0.5, 0.4);
		d2 = d3;
	}
	else
		color2 = vec4(textureSphere(p - vec3(0.0, 0.5, 0.0),  0.5), 0.4);
