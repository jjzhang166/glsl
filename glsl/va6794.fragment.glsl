...
// nonsensical modifications by anonymous. i'm so sorry
...
float ellipse(vec3 p, vec3 r) {
	return length(p / r + 0.5*sin(time * 0.5));
}
...
float facedisplace(vec3 p, out vec4 mask) {
	...
	d1 = ellipse(tp, vec3(2.0)) - 1.0; //top line
	...
	// mouth
	tp = p - vec3(0.0, -0.78 + (p.x * p.x) * 1.5, 0.0);
	dd = capsule(tp, vec2(0.2, 0.118));
	mask.y = dd; //mask:mouth distance
	k = 3.0 * dd + .5*sin(time * 30.) - 3.;
	dd = k * exp(2.0 - k);
	k = exp(-p.x * p.x * 80.0); // side fade
	dd *= k;
	mask.z = k; //mask:mouth side fade
	dd *= 0.8 + cos(time * 4.) + smoothstep(-0.03, 0.03, tp.y) * 0.2; // top and bottom
	d += dd * 0.6;
...
vec3 eyecolor(vec2 p) {
	...
	// iris
	vec3 pcol;
	float pr = r * 2.0*(0.5*sin(time*3.)+1.);
	// outline
	pcol = mix(vec3(sin(time * 13.), cos(time * 19.), 0.0), vec3(0.1, 0.0, 0.0), smoothstep(0.7, 0.95, pr));
	// center
...
Surf facecolor(vec3 p) {
	...
	// eye
	vec2 ec = vec2(cos(time) * 0.2 + 0.396, sin(time) * 0.1 + -0.254);
	vec2 ep = p.xy - ec;
	if(p.x < 0.0) {
		ep.x += ec.x * 2.0;
	}
...
void main() {
	...
	dir = normalize(dir);
	vec3 p = vec3(0.004 * tan(time*75.), -4.5, 0.0 + sin(time) * 0.05);
	...
	// bg
	vec3 color = mix(vec3(0.0), vec3(0.9, 0.0, 0.0), pow(1.0 - length(scrn) * 0.5, 2.0));
