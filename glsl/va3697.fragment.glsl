...
float nPrism( vec2 p, float h, float n)
{
	float c = 3.1415926 / n;
	float t = abs(mod(atan(p.y,p.x),c+c)-c);
	float d = length(p);
	vec2 r = vec2(cos(t)*d-h*cos(c),sin(t)*d-h*sin(c));
	return max(length(r)*sign(r.y),r.x);
}

float nPrism2( vec2 p, float h, float n)
{
	float c = 3.1415926 / n;
	float t = abs(mod(atan(p.y,p.x),c+c)-c);
	float d = length(p);
	vec2 r = vec2(cos(t)*d-h,sin(t)*d-h*tan(c));
	return max(length(r)*sign(r.y),r.x);
}
...
void main(void)
{
    ...
    float d8 = nPrism(p, 1.0, 6.0);  // SIGNED - EUCLID DIST - JIX
    float d9 = nPrism2(p, 1.0, 6.0); // SIGNED - EUCLID DIST - JIX

    gl_FragColor = distance2Color(d9);
}