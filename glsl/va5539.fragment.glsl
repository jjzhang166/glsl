// test bottle, not proper working
...
{
return (p.y>5.0 || p.y<-1.0) ? 10.0 : sqrt(pow(p.x*0.1,2.0)+pow(p.z*0.1,2.0))/1.0;
}
...
return (p.y>5.0 || p.y<-1.0) ? 10.0 : envelope*sqrt(pow(p.x*0.1,2.0)+pow(p.z*0.1,2.0))/1.0;
}
...
float scene( vec3 p )
{
	...
	float acylinder = cylinder(p);
	float abottle = bottle(p);
	
    	return abottle; // both_shapes_with_displacement;
}
