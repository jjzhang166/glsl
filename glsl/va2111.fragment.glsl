...
const float GRADIENT_DELTA = 0.0001;
const float SOFT_SHADOWS_FACTOR = 6.;
...
{
	return sin(20.*p.x)*sin(p.y*2.)*sin(20.*p.z)*0.1;
}
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
	...
	float sphere2 = sdSphere(p + vec3(cos(time * 0.5 + PI) * 0.45,0,-.1), 0.25);
	sphere1 = blend(sphere1, sphere2);
...
bool raymarch_to_light(vec3 ray_start, vec3 ray_dir, float maxDist, out float dist, out vec3 p, out int iterations, out float light_intensity) {
    ...
    for (int i = 1; i <= MAX_RAYMARCH_ITER; i++) {
        ...
        float ifloat = float(i);
        minStep += 0.0000011 * ifloat * ifloat;
    }
    return false;
}
...
void main( void ) {
    ...
    if (raymarch(ray_start, ray_dir, dist, p, iterations)) {
        ...
        if (!raymarch_to_light(p + light1_dir * 0.1, light1_dir, light1_dist, d2, p2, i2, light_intensity)) {
            diffuse1 = vec3(0.8, 0.1, 1.0) * max(0.0, dot(normalize(light1_pos - p), n) * light_intensity * 3.0 / (dist * dist));
        }
        vec3 diffuse2 = vec3(0,0,0);
        if (!raymarch_to_light(p + light2_dir * 0.1, light2_dir, light2_dist, d2, p2, i2, light_intensity)) {
            diffuse2 = vec3(0.8,0.6, 1.0) * max(0.0, dot(normalize(light2_pos - p), n) * light_intensity * 3.0 / (dist * dist));
        }
        color = vec4(vec3(0.4,0.4,0.7) * max(diffuse1 + diffuse2, ambient), 1);
    } else {
        color = vec4(0.4,0.7,0,0);
    }
    gl_FragColor = color;
}