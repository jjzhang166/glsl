...
{
   // slightly modified code from http://glsl.heroku.com/e#5564.0
   p *= 0.02;
   float color = 0.0;
   float distance = sqrt(p.x*p.x + p.y*p.y);
   float angle = atan(p.y, p.x);
   color = abs(sin(distance*10.0 - angle*10.0 + time*0.1));
   color = color*color*3.0 * (2.0 - color);
   float d = min(1.0, distance*distance*10.0);
   float fColor = sin(angle-time*1.0+distance*100.0)*5.5 + sin(angle+1.2+time*1.0+distance*300.0)*0.3 + color*0.3*d;
   return fColor;	
}
...
void main( void ) {
	...
	float color = 0.5;
	vec4 light = vec4(color, color, color, 0.0) * max(0.0, dotNL); // diffuse light color
