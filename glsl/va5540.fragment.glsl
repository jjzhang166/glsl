//
// Original by Inigo Quilez , http://www.iquilezles.org
...
vec2 cylinder(vec3 p)
{
	float d = (p.y>5.0 || p.y<-1.0) ? 10.0 : sqrt(pow(p.x*0.1,2.0)+pow(p.z*0.1,2.0))/1.0;
	vec2 v = vec2(d, 1.0 );
	return v;
}
...
vec2 bottle(vec3 p)
{
	...
	float envelope = (p.y>2.0 && p.y<4.0) ? cos(2.0*3.141592*p.y/(2.0)) : 1.0;

	float d = (p.y>5.0 || p.y<-1.0) ? 10.0 : envelope*sqrt(pow(p.x*0.1,2.0)+pow(p.z*0.1,2.0))/1.0;
	vec2 v = vec2(d, 1.0 );
	return v;
}


vec2 apple_shape( vec3 p )
{
...
vec3 calcNormal( in vec3 pos )
{
    ...
    vec3 nor;
    nor.x = shape(pos+eps.xyy).x - shape(pos-eps.xyy).x;
    nor.y = shape(pos+eps.yxy).x - shape(pos-eps.yxy).x;
    nor.z = shape(pos+eps.yyx).x - shape(pos-eps.yyx).x;
