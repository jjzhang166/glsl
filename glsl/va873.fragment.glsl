...
//
// by @paulofalcao
// colored with Newtow Fractal 1-z^3 with abs() by @hintz
// http://glsl.heroku.com/e#824.1
...
float deltax=12.0+sin(time*0.41)*10.0;
...
vec3 hsv2rgb(vec3 col)
{
  ...
  if (iH==0.0)
  {
    return vec3(col.z, t, p);
  }
  if (iH==1.0)
  {
    return vec3(q, col.z, p);
  }
  if (iH==2.0)
  {
    return vec3(p, col.z, t);
  }
  if (iH==3.0)
  {
    return vec3(p, q, col.z);
  }
  if (iH==4.0)
  {
    return vec3(t, p, col.z);
  }
  
  return vec3(col.z, p, q); 
}
...
vec2 newtonfractal(vec3 z)
{
  for (int n=0;n<100;n++)
  {
    ...
    if (d.x+d.y < 0.3)
    {
      return vec2(z.x+z.y+z.z*0.1, float(n)*0.06);
    }
  }
...
vec3 m(vec3 p)
{
  vec3 q = vec3(p.x*cosThetaPi - p.z*sinThetaPi, p.x*sinThetaPi + p.z*cosThetaPi, 0.37*(p.y+p.x+p.z));

  vec2 results = newtonfractal(q);
...
{ 
  ...
  vec3 backc=m(vec3(real,0.0,imag));
