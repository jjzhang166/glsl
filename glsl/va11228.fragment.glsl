...
void main( void ) {
	...
	vec3 zcol;
	int iters;
	float minDistPointGeo = 1e10;
	for (int i=0; i<ITER_MAX; i++) {
		z = vec3(z.x*z.x-z.y*z.y, z.x*z.y*2.0, 0)+c;
		float spr = orbitTrap(z);
		iters=i;
		minDistPointGeo = min(minDistPointGeo, length(z-vec3(0)));
		if (spr >= 0.0) {
	...
	//color = zcol;
	//color.x = orbitTrap(z);
	//color += vec3(sin(zcol.x),sin(zcol.y*zcol.y),float(iters)/float(ITER_MAX));
	color += vec3(minDistPointGeo);
