...
void main( void ) {
	...
	float count = 0.0;
	float pulse = sin(mod(time * 0.7, pi));
	float limit = (1.0 - pulse) * 100.0 + 2.1;
	pulse = sin(mod(time * 0.7 * 2.0 + pi * 0.85, pi * 2.0));
	float surge = time * 1.6 + pulse*1.0;
	...
	for (float i = 0.0; i < MAXITERATIONS; i += INCREMENT){
		...
		y = (2.0*x*y) + y0;
		xtemp *= 1.0 + 0.0002*surge*cos(i*0.3+(0.23*surge));
		//xtemp *= 1.0 - 0.0006*surge;
		x = xtemp + x0;
		x *= 1.0 + 0.00008*time*cos(MAXITERATIONS-i*0.26+(0.33*surge));
		y *= 1.0 + 0.00009*time*sin(MAXITERATIONS-i*0.29+(0.43*surge));
		count += INCREMENT;
