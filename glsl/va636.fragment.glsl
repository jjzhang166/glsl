...
float h(vec3 q)
{
    ...
    f*=cos(q.y)*cos(q.x)-.1*(cos(time*.5)+.5)-cos(q.z*7.+time*2.5)*cos(sin(time*1.5)*(q.x*3.))*cos(q.y*4.)*.1;
    return f;
}
...
void main()
{
    ...
    for(int i=0;i<75;i++)
    {
        if(h(o+d*t)<.4)
        {
            ...
            n=normalize(n);
            c+=max(dot(vec3(0.,.25*(cos(time*2.)),-.5),n),.0)+max(dot(vec3(.0,-.5,.5),n),.0)*.5;
            break;
        }
        t+=5.;
    }
    gl_FragColor=c+vec4(1.,.23,.1,1.)*(t*.025);
}