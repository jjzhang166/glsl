...
float shadow ( vec3 ro, vec3 rd, float tMin, float tMax, float k ) {
	...
	for (int i=0; i<48; i++) {
		if (t<tMax) {
			float d = map(ro+rd*t).x; // get nearest intersection
			res = min(res, k*d/t); 
...
vec3 render( vec3 ro, vec3 rd ) {
	...
	if (res.y > -0.5 && res.y < 0.5) {
		...
		color += 1.2 * lDif * vec3(0.5, 0.6, 0.5);
	}
	
	// mat1 = primitive
	if (res.y > 0.5 ) { 
		#if 0
		vec4 quat = quaternion(vec3(0.0, 1.0, 0.0), mod(0.5 * time, TWOPI));
		float n = fbm( rotate(vec3(1.0*pos.x, 20.*pos.y, 1.0*pos.z), quat) );
		color *= 0.2 + 0.8 * smoothstep(0.0, 0.8, n);
		n = fbm( rotate(10.0*pos, quat) );
		color *= 0.2 + 0.8 * smoothstep(0.0, 0.8, n);
		#endif
