...
// Cheap Noise
float rand(vec2 co){ return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.0); }
...
float tile_surface( vec2 p ) {
	float f = 0.0;
	f += 0.3000*noise(p); p *= 1.22+cos( -p.x);
	f += 0.2500*noise(p); p *= 2.03+cos( -p.x);
	f += 0.1250*noise(p); p *= 3.01+sin(f);
	f += 0.0625*noise(p); p *= 4.01+cos(f);
