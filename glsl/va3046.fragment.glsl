...
// Fix for shaders compilers that don't allow div by zer0
...
void main( void ) {
	...
	p.x = tan( clamp((p.x-r/2.0*cos(0.0)+p.y/r), -0.4, 0.4)); // Fix for shaders compilers that don't allow div by zer0
