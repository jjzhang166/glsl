...
vec4 tex(vec2 pos)
{
	...
	vec4 blahc = vec4(c,c*0.8,c*0.8,1.0);
	blahc = floor(blahc*2.0)/2.0;
	vec4 blah = vec4(a*1.2,a*0.7,a,1.0);
	blah = floor(blah);
	vec4 noise = vec4(b*0.5,b,b*0.8,1.1);	
	vec4 fac=vec4(sin(pos.y/102.0)/2.0+1.0, sin(pos.y/506.0)/2.0+1.0, sin(pos.y/812.0)/2.0+1.0, 1.0);
	return blah*fac.x+noise*fac.y+blahc*fac.z;
}

void main( void ) {

	vec2 pos = gl_FragCoord.xy+vec2(sin(time/4.0)*501.0,time*78.0+sin(time)*50.0);
	vec4 t = tex(pos);
	t *= vec4(cos(time/180.2)/2.0+1.0,(sin(time/80.53)/2.0+1.0),cos(time/169.0+100.0)/2.0+1.0,1.0);
