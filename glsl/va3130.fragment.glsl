...
float scene(vec3 p)
{
   ...
   // body
   d = sphere(p*vec3(4.0 * (p.z + 2.), (4.0 + sin(p.z * 5.) * (p.z+2.) * 0.4) * (p.z - 1.5), 1.0), 1.0)*0.25;

   // fins
   float f;
   f = sphere(p * vec3(1., 10., 3.) + vec3(0., 0.5, 0.4), 0.5);
   f = difference(f, sphere(p + vec3(0., 0.5, -1.8), 2.));
	
   f = _union(f, sphere(p * vec3(1., 10., 3.) + vec3(0., 0.5, -1.3), 0.3));
	   //(p, vec3(0.6, 0.02, 0.2));
   //f = intersect(f, sphere(p - vec3(0., 0., 1.8), 2.)); 
