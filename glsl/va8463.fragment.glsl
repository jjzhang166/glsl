...
float pcount(float x,float period){return floor(x/period);}
...
float distfunc(vec3 pos){
	...
	return min(max(max(periodic(r,3.0,0.2),periodic(pos.z,1.0,0.7+0.3*cos(time/3.0))),periodic(a*r,3.141592*2.0/6.0*r,0.7+0.3*cos(time/3.0))),0.25);
}
