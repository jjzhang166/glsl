...
const float ANTIALIAS = 0.01; // .5 = smoother but wider
...
float point(vec2 s) {
	...
	for(int iter = 0; iter < MAX_ITER; iter++) {	
		...
		a = a.x*a*vec4(1,1,sin(time * 2.0) * 10.0,2)+a.y*a.yxwz*vec4(-1,1,-2,2)+c;
		a = a.x*a*vec4(1,1,sin(time * 3.35) * 6.0,2)+a.y*a.yxwz*vec4(-1,1,-2,2)+c;
		a = a.x*a*vec4(1,1,2,sin(time * 2.155) * 5.0)+a.y*a.yxwz*vec4(-1,1,-2,2)+c;
...
void main( void ) {
	...
	vec2 p = (( gl_FragCoord.xy / resolution.xy - .5))*vec2(resolution.x/resolution.y, 1.) * 2. * zoom + vec2(-.7476,.115);
