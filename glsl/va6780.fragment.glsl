...
float sph0(vec3 p)
{
	p -= vec3(0,sin(time)*0.3-0.0,0);
	float d = length(p) - 0.5 + sin(p.x*40.0+time*2.0)*1.1* sin(p.y*30.0+time)*0.32*sin(p.z*01.0)*0.5; 
	return d; 
}
...
void main( void ) {
	...
	refl = smoothstep(refl,0.1,1.0)*0.1;
	
	col = vec3(dot,0.5,1)*0.7*shade + 0.05*vec3(1,1,1) + refl*vec3(1,1,1)*0.1;
	col = col*(1.0/dist) + 0.0*vec3(0.1,0.1,0.5)*dist*0.1; 
