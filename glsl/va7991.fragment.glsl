...
void main(void)
{
	...
	// Ray marching
	const float MIN_DISTANCE = 0.00001;								// Distance to scene that we will count as a hit
	const float MAX_DEPTH=400.0;								// Distance from camera that we will count as a miss
	const int MAX_STEPS = 500;								// Maxmimum amount of ray marching steps before counting the ray as a miss
