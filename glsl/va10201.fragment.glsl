...
const vec3 green = vec3( 0x00, 0xff, 0xee ) / ff;
const vec3 red   = vec3( 0xff, 0x4d, 0xee ) / ff;
...
void main( void ) {
	...
	bool sel = mod(0.2*time-0.25, 2.0) > 1.0;
	vec4 r = vec4( sel ? red : green, circle( 0.5, 0.003, p) );
	vec4 g = vec4( sel ? green : red, radar );
