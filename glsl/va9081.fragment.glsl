...
float terrain( in vec2 x )
{
    ...
    float tempF = clamp(sin(time), 0.0, 1.0);
	tempF = smoothstep(0.5, 10.0, tempF);

    return 140.0*a*tempF;
}
...
vec3 camPath( float time )
{
	...
	return vec3( p.x, 0.0, p.y);//*mouse.x, 0.0, p.y*mouse.y );
}
...
void main(void)
{
	...
	vec3 light1 = normalize( vec3(  .66, 0.22,  0.2 ) );
	vec3 light2 = vec3( -0.707, clamp(cos(time), 0.0, 1.0), -0.707 );
	...
	col *= vec3(clamp(sin(mouse.x * 0.7),0.0, 0.75), 0.35, 0.1);
