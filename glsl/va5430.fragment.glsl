...
void main( void ) {
	...
	if(p.x * p.x + p.y * p.y < abs(sin(time/5. - 2.))){ //inside of circle

		colorR = 1. - ((p.x)  * (p.x) + p.y * p.y) * 1./abs(sin(time/5. - 2.));
	
	}else if((p.x - 200.) * (p.x - 200.) + p.y * p.y < .2){ //inside of circle
		
		colorB = 1. - ((p.x+.5) * (p.x+.5) + p.y * p.y);// * 1./abs(sin(time));
	
	}else{ //outside of circle
	
		colorG =1. - (p.x * p.x + p.y * p.y) * .2;
