//  Thematica :distance d'un point a une grille de l'espace
//  Quadrillage de l'espace en abime
...
for(int  i=0; i<100; i++){	
	...
	if(abs(lamb)<1.0e-4) break;
		...
		return clamp(couleur*1.8,0.0,1.0);
}
...
void main( void )
{
	...
	vec3 ecran=vec3(0.0,0.0,0.0);		
					// rotation de la camera
	
	 oeil=rot(oeil);
	vec3 pe=vec3(p.x,p.y,1.60);
	vec3 dirRayon=normalize(rot(pe));
					//calcul de la couleur
	float  coul = render( oeil+12.0*dirRayon, dirRayon );
	vec3 color=vec3(coul,0.8-0.8*coul,0.5*coul);
