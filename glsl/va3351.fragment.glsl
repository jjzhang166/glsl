...
Added lots of comments as demanded by @emackey... and removed the nasty sqrt of negative numbers. Fixed raymarch code and changed crater code a bit (previous version made some inside-out craters).

I'm coding on Windows Vista / Radeon by the way.
...
float meteor(vec3 pos) {
	...
	for (int i = 0; i < 15; i++) {
		...
		// Don't suppress all small craters within bigger ones.
		float random = step(fract(dot(signv,vec3(.3414,.4351,.2723))),.5);

		// Depending on randomness I don't suppress the crater
		float newcraters = sqrt(craters*craters+max(0.,dots*signf)*abs(dots));
		craters2 += (max(0.,dots*signf)-(newcraters-craters))*random;
		craters = newcraters;
	...
	return (craters2+craters-sqrt(blobs))*55.1+.6;
}
...
void main( void ) {
	...
	for (int i = 0; i < maxiter; i++) {
		...
		pos += dir*h;
		dist += h;
		if (abs(h) < .0001 || dist > maxdist) break;
	}
	...
	vec3 color = vec3(pow(c,2.),pow(c,1.5),pow(c,1.));
