...
vec2 hash(vec2 p)
{
    p = vec2(dot(p, vec2(127.2, 311.7)), dot(p, vec2(169.5, 13.3)));
	return fract(sin(p) * 0.3);
}
...
float layerA(vec2 p, float seed)
{
  ...
  //left side control of cut out tunnel
  //w = length(sp - vec2(-2.0, 0.0)) - 0.7;
  //v = min(v, 1.0 - step(w, 0.0));
  
  
  a = st;
  float fanSpeed = 8.0;
  float fanPosition = 2.7;
  w = fan(sp, vec2(fanPosition, 0.5), a * fanSpeed);
  v = min(v, 1.0 - w);
...
void main(void)
{
	...
	uv = uv * 5.0;
	vec2 divs = vec2(resolution.xy / (1.0 + (sin(time) * 0.0)));
	...
	//controls left/right or up/down camera
	vec2 cam = vec2(sin(t) * 0.0, sin(t) * 1.0);
	...
	for(int i = 0; i < 5; i ++)
	{
		...
		if(i == 3 || i == 1)
			w = layerA(vec2(p.x, p.y) * f, float(i));
		else
