...
void main( void ) {
	...
	float mradius = 1.;//0.07 * (-0.03 + length(zoomcenter - mouse));
	if (length(position-mouse) < .0001) {
		float rnum = (position.x+position.y)/2.1;
		if(rnum < 0.33)
		...
		vec4 source = texture2D(backbuffer, position + vec2((rand(vec2(time+position*30.0))-.5)/resolution.x,(rand(vec2(time+position*40.0))-.5)/resolution.y)*30.9);
	...
	if(mouse.x > 0.9)
		me = vec4(rand(position*0.1+time*0.1+0.5),rand(position*0.1+time+0.123),rand(position*0.1+time*0.01+0.872),1.0);
