...
void main()
{
	...
	vec2 q = vec2( (gl_FragCoord.x - resolution.x*0.5)/resolution.y, gl_FragCoord.y / resolution.y - 0.5 ) * 5.0;
	if (dot(q,q) < 1.7 && abs(atan(q.y,q.x)) > abs(fract(time*2.6)-0.5)*1.7 ) {
