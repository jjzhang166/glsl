// ray traced androdi by @simesgreen
...
float scene(vec3 p)
{
    ...
    // legs
    d = _union(d, capsuleY(rotateX(p, sin(time)*0.5) - vec3(-0.4,-1.8, 0.0), 0.2, 0.5));
    d = _union(d, capsuleY(rotateX(p, sin(time+3.142)*0.5) - vec3(0.4, -1.8, 0.0), 0.2, 0.5));


    // floor
    p += vec3(0.0, 1.0 + abs(sin(time)), 0.0);
...
void main(void)
{
    ...
    vec3 rd = normalize(vec3(asp*pixel.x, pixel.y, -2.0 + cos(time *.5)*.5));
    vec3 ro = vec3(0.0, 0.5, 4.5);


    float a = sin(time *.25)*.5;
    rd = rotateY(rd, a);
    ro = rotateY(ro, a);
    ...
    if(hit)
    {
        ...
        rgb = mix(vec3(0.8, 0.9, 1.0), vec3(0.0, 0.0, 0.0), rd.y);
     }
