...
void main( void ) {
	...
	if (gl_FragCoord.y < 1.) {
		...
		if (gl_FragCoord.x < 1.) {
			...
			gl_FragColor = mix(mix(vec4(t1.xy+move*step(fract(time*1.752654)+.0001,s3), t1.zw), fract(vec4(time*1.34312532,time*31.634541,fract((s+1.5)/256.),0)), eaten), mix(vec4(t1.xyz,t1.w+.01),vec4(.2,.2,0,0),step(.9,t1.w)), eatself);
