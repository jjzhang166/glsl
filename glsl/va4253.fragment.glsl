...
void main() {
	...
	for (float i = 1.0; i <= 14.0; i++) {
		...
		float fx = exp(phi * i * abs(pos.x) * 120.0) * tan(i * time/4.0 + abs(pos.x) * 5.0) / 20.0 * tan(i);
	...
	color -= dis * 0.0035 + (cos(time)/1.0);
	gl_FragColor += vec4(color, 1.0);
	/*
	float fx=cotan(pos.x*5.0+time*BLU_SPEED+mse.x*BLU_SPEED*5.0)/8.0;
