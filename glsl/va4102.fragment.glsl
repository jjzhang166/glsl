...
/*
 * Metaballs 101.00001, same as last one, but colored.
...
void main( void ) {
	...
	float centerContribution = ( 1.0 + 0.5 * sin( time ) ) / distanceToCenter;
	float mouseContribution = 1.0 / distanceToMouse;
	float totalContribution = centerContribution + mouseContribution;
	
	// Out!
	gl_FragColor = vec4( totalContribution * centerContribution / 20.0, 0.0, totalContribution * mouseContribution / 10.0, 1.0 );
