...
// applie shape and shading from by Inigo Quilez
...
vec2 appleShapeX( const in vec3 vPos , const in vec3 vSphereOrigin)
{
    vec3 p = vPos - vSphereOrigin;
	
    p.y -= 0.75*pow(dot(p.xz,p.xz),0.2) - 0.5;
	
   vec2 d1 = vec2( length(p) - 0.75, 2.0 );
   vec2 d2 = vec2( p.y+0.55,2.0 );
   //if( d2.x<d1.x) d1=d2;
   return d1;
}

vec2 appleShape( vec3 p , const in vec3 vSphereOrigin) {
	
	vec2 d1 = appleShapeX(p, vSphereOrigin);
...
vec2 GetDistanceScene( const in vec3 vPos )
{
	...
	//vec2 vDistSphere2 = appleShape(vPos, vSphere2Pos);
	vDistSphere2 = vDistSphere1;
	
	
	
	//vec2 vResult = DistCombineUnion(vDistSphere1, vDistSphere2);
	vec2 vResult = appleShape(vPos, vSphere1Pos);;
...
C_Material GetObjectMaterial( in float fObjId, in vec3 vPos )
{
	...
	else
	{
		...
		//vec2 spe;
		//mat.cAlbedo = appleColor(  vPos, GetSceneNormal(vPos), spe );
	}
