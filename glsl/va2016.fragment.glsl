...
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
    ...
    if (ray_dir.z <= 0.0 || p.z < 1.0) { // Optimization: try not to compute blobby object distance when possible
        float sphere1 = sdSphere(p + vec3(cos(time * 1000.2 + PI) * 0.45,0,0), 0.25);
        float sphere2 = sdSphere(p + vec3(cos(time * 1000.2) * 0.45,0,0), 0.25);
        float torus = sdTorus(vec3(p.y + sin(time * 1000.0) *0.1, p.z + cos(time * 1000.0) * 0.1, p.x), vec2(0.2, 0.08));
