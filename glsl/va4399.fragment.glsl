...
-0.10673931989767256,-0.9242528744643419,0.3665560582387368,
0.9874363040574016,-0.14173963578072568,-0.06985285304270773
)*scale;

// Maximum tracing distance
const float maxdist = 30.; // originally 100, but very heavy on my macbook air 

// Maximum number of raymarch steps
const int maxiter = 25;// originally 100, but very heavy on my macbook air 
...
float meteor(vec3 pos) {
	...
	for (int i = 0; i < 10; i++) {
		...
		float dots = max(0.,.18-dot(v,v));	// simple blob around the unit cell center
...
void main( void ) {
	...
	float c = (dot(vec3(1,0,2),normalize(vec3(hd)))+1.0)*.8;
	
	float distAttenuate = max(0.,1.-dist/maxdist);
	float attenuate = distAttenuate*distAttenuate*distAttenuate*max(0.,3.-h);
