...
// On my machine (win7, Firefox, AMD5850) the function and variable
// name must be different
vec2 rosefn(float radian, float r) {
...
void main( void ) {
	...
	for (int i = 0; i < num; ++i) {
		...
		a = (sin(time*.09)*3.5)*(sin(time*.01)*3.);
		// t += (atan(position.y,position.x)*.1);
		vec2 rose = rosefn(t, a);
	...
	vec3 rgb = vec3(
		sum * .01, 
		sum * (tan(time*1.05)*1.1) * .006,
		sum * (tan(time*1.01)*1.1) * .006 * 1.6
	);
