...
void main( void ) {
  ...
  //Camera animation
  vec3 vuv=vec3(0,1,0.5);//Change camere up vector here
  vec3 vrp=vec3(0.6,5,0.8); //Change camere view here 
  vec3 prp=vec3(sin(time)*16.0*mouse.x,4,cos(time)*16.0*mouse.y);
  ...
  else gl_FragColor=vec4(0.8,0,0,1); //background color
}