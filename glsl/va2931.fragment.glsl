...
float waveLength = 10000.100;

void main( void ) {
	vec2 uv = gl_FragCoord.xy/resolution;

	vec2 p1 = (vec2(sin(time), cos(time))*orbitDistance)+0.4;
	vec2 p2 = (vec2(sin(time+3.142), cos(time+3.142))*orbitDistance)+0.5;

	float d1 = 1.-length(uv +p1);
	float d2 = 1.-length(uv -p2);

	float wave1 = sin(d1*waveLength+(time*5.))*0.5 + 0.5 * (((d1 - 0.7) * 2.) + 0.6);
	float wave2 = sin(d2*waveLength+(time*5.))*0.5 + 0.5 * (((d1 - 0.3) * 2.) + 0.4);
