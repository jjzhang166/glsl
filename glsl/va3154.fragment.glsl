...
// Adding some of the ashima noise to the noise kabuto has going because I heard you like some noise with your noise, dog. Other small tweaks maybe. -gtoledo
...
void main( void ) {
	...
	float n = surface3(vec3(position, time * 0.3));//*mat3(1,0,0,0,.8,.6,0,-.6,.8));
	float lum = length(n);	
	...
	l = min(l*1.01,.5);
	//float a = atan(cPrev.x,-cPrev.z)+rnd*.05*((n*2.3)*sin(time*2.)+1.);
	float a = atan(cPrev.x,-cPrev.z)+rnd*.1*(lum*.5);
	a = cPrev.w < .5 ? -max(fract(dot(m,m)*4.)-.5,0.)*12.*min(max(abs(m.x*10.-2.),0.),1.) : dot(m,m) < .0002 ? 1. :  min(a, a*a*8.);
