...
void main( void ) { 
	...
	vec2 c = vec2(0.5);
	gl_FragColor.xyz = vec3(line_segment(uv, pos1, d1, pos2, d2));
	...
	gl_FragColor.rgb = max( gl_FragColor.xyz, max(stem_feedback, max(left_arm_feedback, right_arm_feedback)));
	
	gl_FragColor.rgb = mix( gl_FragColor.xyz, texture2D(backbuffer, uv).xyz, vec3(0.125)); // sort of a motion blur
