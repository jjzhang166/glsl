...
float sdl(vec2 a, vec2 b, vec2 p){
	float d;
	vec2 n, l;
...
float sline(vec2 a, vec2 b, vec2 p, float w){
	return smoothstep(w, 0., sdl(a, b, p));
}
...
vec4 map( vec3 p )
{
	...
	p = normalize(p+sline(1.2*p.xy,vec2(0.5),1.3*p.xy,.015))*sin(time*.05)*8.;
    // pattern
	float f;
	...
	col = mix( col, col, clamp( (r.y+0.1)/1.5, 0.0, 1.0 ));
...
void main(void)
{
	...
	// contrast
	col = col*col*(3.0-2.0*col)*1.5 - 0.4;
