...
float ringInstance(vec2 p, float t, float xden, float yden)
{
	...
	return ringShape(p - vec2(mod(th, xden) / xden, mod(th, yden) / yden) * 1.0 + 9.0, fract(t));
}
...
void main( void ) {
	...
	float t = time * 5.0 + 8.0;
	int a;
	gl_FragColor.a = 1.0;
	gl_FragColor.rgb = 	ringInstance(p, t - 5.0, 7.0,  13.0) * vec3(5.0, 0.7, 0.6) +
