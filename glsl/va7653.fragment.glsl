// Lumpy displacement creature by @thismatt based on @PauloFalcao's 704 on @P_Malin's
...
void CalcPixelRandom()
{
	...
	vec4 s1 = sin(time * 10.3422 + gl_FragCoord.xxxx * vec4(324.324234, 563.324234, 657.324234, 764.324234)) * 543.3423;
...
{          
	...
	float tt=time*0.5+10.0;
	vSphereDomain = vSphereDomain + sin(1.0*vPos.y+tt)*sin(1.0*vPos.z+tt);
//	vSphereDomain.x=sin(vSphereDomain.x)+sin(tt);
//	vSphereDomain.z=cos(vSphereDomain.z)+cos(tt);
//	vSphereDomain = RotateY(vSphereDomain,time*1.1);
	#ifdef FLOATING
	vSphereDomain.y=log(vSphereDomain.y)+sin(tt);
	#endif

	//float dist = length(length(vSphereDomain))-mix(0.0,2.0,mouse.x)-sin(oP-tt*4.0);
	float dist = length(vSphereDomain) - 6.0;
	vec4 vDistSphere = vec4( dist, 2.0, vSphereDomain.xy);
...
//	mat.cAlbedo = vec3((sin(d*.25-tt*4.0)+1.0)/2.0,(sin(tt)+1.0)/2.0,(sin(d-tt*4.0)+1.0)/2.0);
	...
	mat.cAlbedo = vec3(1.0,1.0,1.0) - sin(vPos.y *8.0)/4.0 + cos(vPos.z *5.0)/3.0;
	return mat;
}
...
vec3 GetLightCol()
{
	return vec3(52.0, mouse.x*10.0, mouse.y*10.0) * 10.0;
}
