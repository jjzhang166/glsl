...
{	
	...
	float t = time * 0.000002;
  	vec2 center1 = vec2(cos(t), cos(t*0.535));
    	...
    	float x = (x0*cosr - y0*sinr) * sin(time);
    	float y = (x0*sinr + y0*cosr) * sin(time);
	...
	float speed = sin(time * sin(time) * 5000.0) * 10.0;
	gl_FragColor = vec4(mod(red,sin(time*sin(time)*speed)*01.805), 
			    mod(red,sin(time*cos(time)*speed)*01.805), 
			    mod(red,sin(time*speed)*01.805), 
			    1.0);
