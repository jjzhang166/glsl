...
void main( void ) {
	...
	position = transform_rotate(position, vec2(0.0), time*0.2 + length(position)*sin(time*0.2)*twoPI/4.0);
	position = transform_N_polygon(position, vec2(0.0), 6, 1.7);
	
	float r = length(position) + sin(angle*10.0 + time)*0.02;
	gl_FragColor = vec4( fract( ( ( sin(vec3(0.2,1.4,2.3) * 0.00001 * time) + vec3(1.2,1.8,2.2) * 0.00001 * time  ) * 200.0 * vec3(3.0,11.0,15.0) )*( r + time * 0.05 + sin(time*0.1) ) ), 1.0 );
}