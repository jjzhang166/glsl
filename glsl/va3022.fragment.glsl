...
vec4 DistCombineBlend( const in vec4 v1, const in vec4 v2 )
{
...
{          
	...
	vec4 vDistBox = vec4( GetDistanceRoundedBox( vSphereDomain, vec3(1.0, 0.2, 1.0), 0.1 ), 2.0, vSphereDomain.xy);
        
	vec4 blend = DistCombineSubtract(vDistBox, vDistSphere);
	vResult = DistCombineUnion(vResult, blend);
...
#define kRaymarchEpsilon 0.0
#define kRaymarchMatIter 5048
