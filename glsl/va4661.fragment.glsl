...
void put_pixel(float src_x, float src_y, float end_x, float end_y, vec3 color_s, float calc)
{
 if(position.x > src_x*resolution.x && position.x-src_x*resolution.x < end_x &&
    position.y > src_y*resolution.y && position.y-src_y*resolution.y < end_y)
 {
   color = 0.6;
