...
void main( void ) {
	...
	{
		alpha = fract(length(pixel) * radius - length(centered_position) + sin(time));
	}
	else
	{
		float rnd = mod(cos(time * time * (position.x * position.y)) * cos(time * time * time* (position.x * position.y)), 1.0);
