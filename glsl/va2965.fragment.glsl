...
vec2 inObj(in vec3 p){
    vec2 f = obj_floor(p);
    float b1 = torus(p+vec3(sin(time2*0.77)*1.4,sin(time2)*2.,cos(time2)*5.),vec2(2.0,1.0));
    float b2 = sphere(p+vec3(cos(time2)*4.4,cos(time2)*2.,cos(time2*1.2)*3.5),2.0);
    float b3 = sphere(p+vec3(sin(time2)*3.6,sin(time2*0.7)*2.,sin(time2)*2.6),2.0);
    float b4 = sphere(p+vec3(cos(time2*0.7)*4.4,sin(time2*1.1)*2.,cos(time2)*3.5),2.0);
    float b5 = sphere(p+vec3(sin(time2*1.3)*3.6,cos(time2*1.33)*2.,sin(time2*0.94)*2.6),2.0);
    const float e = 0.1;
    const float r = 2.0;
...
n=normalize(
  ...
  if (f<MAX_DEPTH){
    ...
    if(s.y==1.0)gl_FragColor -= max(0.5 - ao(p, n, 0.03),0.0);
    //if(s.y==1.0) gl_FragColor *= vec4(ao(p, n, -0.4));
    if(s.y==0.0)gl_FragColor.xz -= max(0.25 - softshadow(p, n, 4.0),0.0);
  }
