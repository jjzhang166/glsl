...
p.xz = clamp(vec2(-2.0),vec2(2.0), p.xz);
	*/
   vec2 d2 = vec2( p.y+0.55, 2.0 );

   p.y -= 0.5*pow(dot(p.xz,p.xz),0.5);
	float n = 16.0;
	...
	float zz = pow(p.z,n);
	float k = 1.0 * (-p.y*2.0);
  vec2 d1 = vec2( sqrt(xx+yy+zz)+k, 1.0 );
...
void main(void)
{
    ...
    // camera
    vec3 ro = 3.5*normalize(vec3(cos(0.2*time),0.5+0.45*cos(time*.31),sin(0.2*time)));
    ...
    if( tmat.y>0.5 )
    {
        ...
        float spe = pow(clamp(dot(lig,ref),0.0,1.0),64.0);
        ...
        col += 0.70*dif*vec3(1.00,0.7,0.5)*vec3(sh, (sh+sh*sh)*0.5, sh*sh );
    ...
    gl_FragColor = vec4(1,0,time,10.0);
}