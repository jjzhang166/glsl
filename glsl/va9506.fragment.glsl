...
float map(vec3 p)
{
    float h = 1.0 + sin ( time ) * 0.5;
    float rh = 0.0;
    float grid = 0.4;
    float grid_half = grid *0.45 + sin ( time  * 0.2 ) * 0.02;
    float cube = 0.1;
    vec3 orig = p;
...
void main()
{
    ...
    vec3 camPos = vec3(-0.9,0.0,3.0);
    vec3 camDir = normalize(vec3(0.3, 0.0, -1.0));
    //camPos -=  vec3(0.0,0.0,time*0.1);
	...
	vec3 p = ray;
        ...
        if(gp2.y<(0.31 + cos ( time * 0.65 ) * 0.1) && gp2.z<0.2) {
            grid2 = 0.0;
        }
        glow += grid1+grid2;
    }
    ...
    vec3  fog2 = (sin ( time)+1.0) * 0.02 * vec3(1, 1, 1.5) * total_d;
    glow *= min(1.0, 4.0-(4.0 / float(MAX_MARCH-1)) * float(march));
    float scanline = mod(gl_FragCoord.y, 4.0) < 2.0 ? sin(time) * 0.5 +0.9 : 1.0;
    gl_FragColor = min(vec4(vec3(0.05+glow*0.75, 0.55+glow*0.75, 0.55+glow)*fog + fog2, 1.0), 1.0) * scanline;
}
