...
void main( void ) {
	...
	float wave1 = mod(sin(d1*waveLength+(time*5.))*0.5 + 0.5 * (((d1 - 0.5) * 2.) + 0.5),sin(time));
	...
	gl_FragColor.z = mod(pow(clamp(1.-1.5*length(uv  - mouse + d),0.,1.),4.0),0.50);
	gl_FragColor.y = mod(gl_FragColor.z*0.5,0.5) + mod(gl_FragColor.x*0.4,0.5);
