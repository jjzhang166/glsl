...
//bizarrefish - Haha, I understand raymarching too! To prove it, i've made it all glowy.
...
void main( void )
{
	...
	float glow = max(closest - 0.1, 0.);
	
	light += vec3(glow,glow,glow);

	vec3 col = vec3(1.0*r.y,0.8-0.2*r.y,.3);
	
	result = vec4(light*col,0.0);
	
	gl_FragColor = mix(result, vec4(1.,1.,1.,0.0),min(d,0.));
}