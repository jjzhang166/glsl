...
void main( void ) 
{
	...
	vec3 groundColor = vec3(1.0, 0.3, 0.2);
	vec3 skyColor = vec3(0.6);
	...
	for(int i = 0; i < 20; i++) {
		...
		if(dist < epsilon) {
			...
			// costheta for hemisphere lighning
			float costheta = dot(n, l);
			float a = 0.5 + 0.5 * costheta;
			
			// hemisphere lightning
			color += vec4(mix(groundColor, skyColor, a), 1.0);
