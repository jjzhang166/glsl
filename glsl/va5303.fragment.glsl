...
float scene(vec3 p) { 
  p = opRep(p,vec3(8.,8.,8.));
  float ub = udBox(p,vec3(.25,1.,.25));
  float ss = sdSphere(p+vec3(0.,1.,0.),1.);
  return min(ub,ss);
}

vec4 color(float d) { 
  return mix(vec4(1.,1.,1.,0.025),vec4(0.,0.,0.,0.0),smoothstep(0.,1.,abs(d))); 
}
...
vec4 ray(vec3 pos, vec3 step) {
    ...
    vec4 col;
    float d;
#define RAY1  d = scene(pos); col = color(d); col.rgb *= (pos.y+.5); col.rgb *= col.a; sum += col*(1.0 - sum.a); pos += step*max(d,_MinStep);
