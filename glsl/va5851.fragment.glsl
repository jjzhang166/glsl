...
void main( void ) {
	...
	vec2 p = vec2(sin(time*0.1),-cos(time*0.1)) + (3.*gl_FragCoord.xy)/resolution.y;
	
	float color = voronoi(p);
	
	float light_intensity = 0.75/tile_height;
	vec3 light = normalize(vec3(sin(time),cos(time),1.0)) * light_intensity;
	
	
	vec3 n = normal(p);
	float s = 2., inv = 1.;
	vec2 os = vec2(100., 100.);
	for(int i=0; i<3; i++){
		n = n + normal(p*s + os)*inv;
		color += voronoi(p*s+os);
		s *= s;
		os += os;
		inv*=-1.;
	}
