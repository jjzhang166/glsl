...
void main( void ) {
	...
	float as = (cos(time*.01)*.99*m);	// there was originally an outer sinh for as and bs but difference is just about 1 percent which doesn't really matter for the camera path
	...
	for (int i = 0; i < 8; i++) {
		...
		pos += pos*kDist*+0.25 + dir*kDist*1.55;	// compute actual distance (as we're in the klein metric we can't simply do length(a-b) - we have to use
