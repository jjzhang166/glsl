...
// weyland retweaked these parameters
...
void main()
{
	...
	for( float i = 0.0; i < 15.0; ++i )
	{
		...
		t += (pow(max(0.0,dot(normalize(reflect(light-v, normalize(hv.xyz))), normalize(b))), 34.0) + abs(dot(normalize(hv.xyz), light-v))*0.1 * (normalize(hv.xyz)+1.0)) * pow(h, -1.2) * 0.1 * float(15.0-i);
	}
	t = t / float(15.0*15.0);
