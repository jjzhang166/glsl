...
vec2 R = resolution;
vec2 Offset;
vec2 Scale=vec2(0.032,0.052);
float Saturation = 1.8; // 0 - 1;
...
void main( void ) {
	...
	float th = atan(position.y, position.x) / (2.3 * 3.5);
	float dd = length(position) + 0.055;
	float d = 0.8 / dd + time;
	
    	vec2 x = gl_FragCoord.xy;
    	vec3 c2=vec3(0,0,0);
   	x=x*Scale*R/R.x;
    	x+=sin(x.yx*sqrt(vec2(1,15)))/8.;
    	c2=lungth(sin(x*sqrt(vec2(3,59))),vec3(1,2,9)*Saturation * d);
