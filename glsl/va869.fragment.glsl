...
float f(vec3 o)
{
    float a=(sin(o.x)+o.y*.25)*.35+time;
    o=vec3(cos(a)*o.x-sin(a)*o.y,sin(a)*o.x+cos(a)*o.y,o.z);
    return dot(cos(o)*cos(o),vec3(3.-sin(a)))-2.5;
}
...
vec3 s(vec3 o,vec3 d)
{
    ...
    vec3 p=o+d*t;
    vec3 n=-normalize(vec3(f(p+e),f(p+e.yxy),f(p+e.yyx))+vec3((sin(p*75.)))*.05);

    return vec3( mix( ((max(-dot(n,vec3(.577)),0.) + 0.125*max(-dot(n,vec3(-.707,-.707,0)),0.)))*(mod(length(p.xy)*20.,2.)<1.0?vec3(.3,.65,.85):vec3(.35,.7,.95)),vec3(.93,.94,.85), vec3(pow(t/9.,5.)) ) );
}
#else
//
// original marching
