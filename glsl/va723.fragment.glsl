...
vec2 map( vec3 p )
{
   vec2 d2 = vec2( p.y+1., 2.0 );

   p.y += 0.75*(dot(p.xz,p.xz));
...
vec3 appleColor( in vec3 pos, in vec3 nor, out vec2 spe )
{
    ...
    // red
    vec3 col = vec3(1.4,0.89,0.83);
     float f;
    // green
     f = smoothstep( 0.1, 1.0, fbm(pos*1.0) );
    col = mix( col, vec3(1.3,1.,0.7), f );
  ...
  f = smoothstep( 0.0, 1.0, fbm(pos*1.0) );
    f = smoothstep( 0.7,0.9,f);
...
void main(void)
{
    ...
    // camera
    vec3 ro = 2.5*normalize(vec3(1.0,1.0,0.2));
    vec3 ww = normalize(vec3(0.0,0.,0.0) - ro);
    ...
    if( tmat.y>0.5 )
    {
        ...
        vec3 lig = normalize(vec3(1.0*sin(time/2.),0.8,-0.6*cos(time)));
      vec3 lig2 = normalize(vec3(-1.0*sin(time),0.8,0.6*cos(time)));
     
        float con = 1.0;
        float amb = 0.5 + 0.5*nor.y;
        float dif = max(dot(nor,lig),0.0);
      float dif2 = max(dot(nor,lig),0.0);
        float bac = max(0.2 + 0.8*dot(nor,vec3(-lig.x,lig.y,-lig.z)),0.0);
        float rim = pow(1.0+dot(nor,rd),3.0);
        float spe = pow(clamp(dot(lig,ref),0.0,1.0),16.0)+pow(clamp(dot(lig2,ref),0.0,1.0),16.0);
        ...
        col += 0.70*dif*vec3(0.5,0.4,0.4)*vec3(sh, (sh+sh*sh)*0.5, sh*sh );
