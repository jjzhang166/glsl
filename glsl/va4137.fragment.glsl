...
void main( void ) {
	...
	float scale = ( 0.5 + 0.45 * sin( time ) );
	float sphereContribution1 = metaballContribution( position, sphereCenter1 ) * scale;
	...
	float r = 0.6;
	vec3 n1 = sphericalNormal( r * scale, position * scale, vec2( 0.0 ) );
