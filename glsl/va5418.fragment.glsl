...
float map(vec3 p) {
	return p.y + (0.5 + waveHeight1 + waveHeight2 + waveHeight3) 
		+ snoise(vec2(p.x , p.z + 0.6)) * waveHeight1
		+ snoise(vec2(p.x * 1.6 - 0.4, p.z * 1.7 - 0.6)) * waveHeight2
	  	+ snoise(vec2(p.x * 6.6 - 1.0, p.z * 2.7 + 1.176)) * waveHeight3;
}
...
void main( void ) {
	float waveHeight = USE_MOUSE ? mouse.x * 100.0 : 30.0;
	...
	vec3 ray_start = vec3(0, 0.35, -2);
	vec3 ray_dir = normalize(vec3(position,0) - ray_start);
	ray_start.z = cos(time*0.5);
	
	const float dayspeed = 0.1;
	float subtime = abs(sin(time * dayspeed) * 0.2);
	...
	const vec3 brown = vec3(0.48, 0.25, 0.0);
	const vec3 green = vec3(0.2, 0.5, 0.0);
	
	vec3 diffusecol = green;
	...
	if (dist > 0.0) {
		...
		vec3 ambient = skycolor_now * 0.1;
        	...
        	vec3 diffuse1 = diffusecol * max(0.0, dot(light1_dir, n)  * 2.8);
		vec3 r = reflect(light1_dir, n);
		vec3 specular1 = vec3(1.5, 1.2, 0.6) * (shininess * pow(max(0.0, dot(r, ray_dir)), 200.0));	    
