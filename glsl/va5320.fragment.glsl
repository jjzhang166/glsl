...
/*
    for(int x = 0; x < SLIME_FIELD_WIDTH; x += 1)
    {
        result = obj_union(result, vec2(columnZoneDistance(p, float(x)), float(x) + 1.0));
    }
*/

    result = obj_union(result, vec2(columnZoneDistance(p, 0.0), 0.0 + 1.0));
    result = obj_union(result, vec2(columnZoneDistance(p, 1.0), 1.0 + 1.0));
    result = obj_union(result, vec2(columnZoneDistance(p, 2.0), 2.0 + 1.0));
    result = obj_union(result, vec2(columnZoneDistance(p, 3.0), 3.0 + 1.0));
    result = obj_union(result, vec2(columnZoneDistance(p, 4.0), 4.0 + 1.0));
    result = obj_union(result, vec2(columnZoneDistance(p, 5.0), 5.0 + 1.0));
...
void pumpCountBank(vec2 bank_org, float counted_type)
{
    ...
    if(xofs < float(SLIME_FIELD_WIDTH))
    {
        ...
        for(int y = 0; y < SLIME_FIELD_HEIGHT; y += 1)
        {
...
void doColumnZoneDisplay(vec2 p, float i)
{
    float dist = columnZoneDistance(p, i);
    
    if(dist < 0.0)
    {
        gl_FragColor.rgb = (columnZoneDistance(MOUSE_COORD, i) < 0.0) ? vec3(0.7) : vec3(0.4);
    }
    else if(dist < 0.005)
    {
        gl_FragColor.rgb = vec3(0.0);
...
{
    doColumnZoneDisplay(p, 0.0);
    doColumnZoneDisplay(p, 1.0);
    doColumnZoneDisplay(p, 2.0);
    doColumnZoneDisplay(p, 3.0);
    doColumnZoneDisplay(p, 4.0);
    doColumnZoneDisplay(p, 5.0);

    float dist = rotateZoneDistance(p);
