 ...
 * MOD: COMBINED WITH http://glsl.heroku.com/e#4567.0 !!! @fernozzle
 */
...
void main( void ) {
	...
	p = (gl_FragCoord.xy / resolution.xy - vec2(0.5,0.5)) * vec2(1.0, resolution.y/resolution.x);
	vec3 color;
	vec2 center = vec2(0.0);
	vec2 flare = position.xy;
	color = vec3(0.0);
	//sun
	vec3 sun = vec3(0.0);
	sun += pow(vec3(1.0, 0.9, 0.8) * bokeh(flare ,0.05, 0.1) * 1.0, vec3(2.0));
	sun += vec3(1.0, 0.86, 0.8) * point(vec2(0.), flare);
	//ghosts
	vec3 ghosts = vec3(0.0);
	ghosts += adjust(vec3(1.0, 0.9, 0.8) * bokeh(p + flare * 0.1,0.05, 0.004)) * 0.2;
	ghosts += adjust(vec3(1.0, 0.8, 0.7) * bokeh(p + flare * - 0.1,0.03, 0.003)) * 0.3;
	ghosts += adjust(vec3(1.0, 1.0, 0.9) * bokeh(p + flare * 0.6,0.06, 0.010)) * 0.2;
	ghosts += adjust(vec3(1.0, 1.0, 0.9) * bokeh(p + flare * 1.0,0.09, 0.015)) * 0.1;
	...
	gl_FragColor = vec4( c / r * vec3(0.7, 0.93, 1.0) *0.5 + sqrt(color) + vec3(0.02, 0.0, 0.04), 1.0 );
