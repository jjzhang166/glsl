...
void main( void ) {
	...
	// move sphere
	sph1.x = 0.6 * cos( time );
	sph1.y = 1.0 * sin( time ) + tan(time);
	sph1.z = 0.5 * cos( time );
	
	// generate a ray with 
	vec3 ro = vec3( 0.5, 0.5, 3.0 );
	...
	vec3 col = vec3(0.5); // wow, pretty nice!! :)
	...
	if ( id>0.5 && id<1.5 ) {
		...
		col = vec3( 0.9, 0.1, 0.6) * dif * ao + vec3(0.1, 0.2, 0.4) * ao;
	} else if ( id>1.5 ) {
		// plane
		vec3 pos = ro + t * rd;
		vec3 nor = nPlane( pos );
		float dif = clamp( dot(nor, light), 0.0, 1.0 );
		float amb = smoothstep( 0.2, sph1.w * 1.0, length(pos.xz - sph1.xz) ); // shadows under the sphere
		col = vec3 (amb * 0.3);
	}
