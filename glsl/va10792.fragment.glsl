...
float map(vec3 p, vec3 ray_dir) { //  ray_dir is used only for some optimizations
		...
		vec2 np = vec2(p.x+time*.15, p.z) * .35;
...
void main(){
	float r = hash(uv+time+hash(gl_FragCoord.yx));
	int stipple = int(mod(floor(r*3.), 0.));
	float pixel = mouse.y/resolution.y;
	
	vec4 buffer = gl_FragColor = texture2D(backbuffer, uv + vec2(pixel, 0.));
	
	if(gl_FragCoord.x > resolution.x * .75){
		gl_FragColor = render(); //fully rendered
	}
	else if(gl_FragCoord.x > resolution.x * .5){
		gl_FragColor = buffer; //buffered from 1 pixel to the right
	}
	else if(gl_FragCoord.x > resolution.x * .25 && stipple == 0)
	{
		buffer = vec4(0.);
		vec2 kernel[4];
		kernel[0] = vec2(0., pixel);
		kernel[1] = vec2(-pixel,0.);
		kernel[2] = vec2(0., 0.);
		kernel[3] = vec2(0.,-pixel);
		
		float hpixel = pixel*.15;
		for(int i = 0; i < 4; i++){
			float ri = .001*hash(uv+time*32.)*hpixel;
			buffer += texture2D(backbuffer,uv - kernel[i]);
		}
		buffer *= 0.25;
		
		gl_FragColor = buffer; //stochiastic update
	}else{
		gl_FragColor = render() * .25 + buffer * .75; //smoothed
	}
	
		
	if(mouse.x > .972 && mouse.y < .05 || uv.x > .972 && uv.y < .05){
		gl_FragColor = vec4(0.);//reset
	}
}//sphinx
