...
// dildo dancing by tater, shouts to #anti
...
vec2 inObj(in vec3 p){
	...
	vec2 r = obj0(p);
	vec3 q = p + vec3(0., -.25-3.*abs(sin(time * 4.0)), 0.);
	q.x += 5.0 * sin(time * 2.);
	q.z += 5.0 * cos(time * 2.);
	
	r = ObjUnion(r, capsule(q, .5, 1.5));
	//r = ObjUnion(r, vec2(length(q+vec3(-.5, 1.5, 0.)) - 0.95, 1.));
	//r = ObjUnion(r, vec2(length(q+vec3(.5, 1.5, 0.)) - 0.95, 1.));
	//r = ObjUnion(r, vec2(length(q+vec3(0, -1.5, 0.)) - .65, 1.));
...
void main(void){
  ...
  vec3 prp=vec3(sin(time) * 5.0, 5.0, cos(time) * 5.0); //Trackball style camera pos - Change camera path position here
