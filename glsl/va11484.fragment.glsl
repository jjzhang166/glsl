...
// Trying to port this from shader toy, guess we can made up a texture2D from mouse or backbuffer?¿?
...
float noise( in vec2 x )
{
	...
	return sin(uv.x);//texture2D( iChannel0, (uv+118.4)/256.0, -100.0 ).x;
}
...
void main(void)
{
	...
	col *= pow( 64.0*qq.x*qq.y*(1.0-qq.x)*(1.0-qq.y), 0.05 );
