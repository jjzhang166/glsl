...
void main( void ) {
	vec2 uv = 0.5 + (gl_FragCoord.xy * vec2(1./resolution.x,1./resolution.y) - 0.5)*vec2(1.,resolution.y/resolution.x);
	float mouseW = atan(mouse.y - 0.5, mouse.x - 0.5);
	vec2 mousePolar = vec2(sin(mouseW), cos(mouseW));
	vec2 offset = (mouse - 0.5)*4.;

	gl_FragColor = vec4(circle(uv, 8.)* 0.5); // seed

	float filter = smoothcircle(uv, 0.1, 30.);
	gl_FragColor += texture2D(backbuffer, torus_mirror(0.5 + complex_mul(((uv - 0.5)*mix(1.4, 16., filter)), mousePolar) - complex_mul(offset, mousePolar) +time*0.02))*mix(1., 0.5, filter); // add complex feedback (:
}