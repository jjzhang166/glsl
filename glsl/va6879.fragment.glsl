...
#define DIST_BOX(p) (box(p, vec3(2, .01, 2), 0.01))// + snoise(p.xz * 30.0) * 0.01)
#define DIST_SPHERE(p) (sphere(p - vec3(0.0, 1.0, 0.0), .8) + snoise((p).zy) * 0.1 * snoise(vec2(time, time)))
...
float shade(in vec3 o, in vec3 p, in vec3 rayDir, in vec3 LightPos)
{
	#define SHADE_STEPS 6.0
	
	float rayLength = length(p - o);
	float d = 0.0;
	float step = rayLength / SHADE_STEPS;
	float accumShade = 0.0;
	
	for (int i = 0; i < int(SHADE_STEPS); ++i)
	{
		d += step;
		...
		float s = DIST_SPHERE(P);
		
		// Dist to nearest object after stepping toward light
		float sl = DIST_SPHERE(P + (lightDir * s));
		
	
		// If distance shrunk, we should be in penumbra
		accumShade += max(0.0,  (s - sl));
		
	}
	return accumShade / SHADE_STEPS;
}
...
void main( void )
{
	...
	float r = 4.0;
	vec3 campos = vec3(r * cos(mouse.x * 10.0), /*mouse.y * 4.0*/ 1.0, r * sin(mouse.x * 10.0));
