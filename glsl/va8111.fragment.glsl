...
float NLogo(vec2 p) {
	...
	float y = floor((p.y)*-32.)+8.;
	if(y < 0. || y > 512.) return 0.;

	float x = floor((1.-p.x)*48.)-32.;

	float v = 0.;
	v = mix(v, 620069., step(y, 4.5)); // 011001000101110
	v = mix(v, 861525., step(y, 3.5)); // 100101101101001
	v = mix(v, 730450., step(y, 2.5)); // 111101010101001
	v = mix(v, 599378., step(y, 1.5)); // 100101000101001
	v = mix(v, 599586., step(y, 0.5)); // 100101000101110
...
vec3 layer(float i, vec2 pos, float dist, vec2 coord) {
	...
	return  clamp(vec3(NLogo(uv*.5))*m*dist, 0.0, 1.0);
}
...
vec3 Chart( vec2 p ) {
	...
	vec2 p0 = vec2(x-.5, NValue(x+0., s));
	vec2 p1 = vec2(x+.5, NValue(x+1., s));
	d = min(d, segment(p+vec2(0,0), p0, p1));
	
	p0 = vec2(x+1.5, NValue(x+2., s));
