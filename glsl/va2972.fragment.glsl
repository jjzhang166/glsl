// @rotwang: @mod* checker, @mod* vertical camera
...
void main(void){
	...
	float timed8 = time/4.0;
	float usint = sin(timed8)*0.5+0.5;
	
  //Camera animation
  vec3 U=vec3(1,0,0);//Camera Up Vector
  vec3 viewDir=vec3(0,-1,0); //Change camere view vector here
	
  float zoom = 	2.0 + usint*32.0;	
	
  vec3 E = vec3(0, zoom,0);	
	
 // vec3 E=vec3(sin(timed8)*8.0,16,sin(timed8)*8.0); //Camera location; Change camera path position here
