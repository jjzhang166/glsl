...
void main( void ) {
	...
	vec2 position = gl_FragCoord.xy / resolution.xy - (mouse.xy)+0.5;

	float r=dist(position,vec2(cosScr(1.0,1.6),cosScr(0.6,0.2)));
	float g=dist(position,vec2(cosScr(1.3,0.8),cosScr(0.7,0.7)))-r;
	float b=dist(position,vec2(cosScr(0.6,0.3),cosScr(0.2,0.5)))-g;
