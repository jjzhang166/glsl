...
void main( void ) {
    ...
    float f = 1.2-(it/float(MAX_RAY_STEPS));

    vec3 bg = vec3(pow(length(tx*0.5),3.5));

    float lights = max(0.0, pow(max(abs(tx.y), 1.0), 1.0))*0.9;
    float sky    = max(0.0, pow(max(tx.y+min(300.0,l)*0.01, 0.0), 0.9))*0.1;
