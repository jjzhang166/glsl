...
float GetDistanceToScene2( vec3 P)
{
	...
	float s;
	vec3 O = P;
	float f = 0.7; // remember the scaling applied to space so far
	...
	for (int i = 0; i < 5; i++)
	{
		...
		d = min(d, sdeCylinderY(O, vec3(0.0), sin(time), 0.045)/f); // Tweak: /f added. (If you scale space you should scale distances as well. This works much better than the previously used approximation.)
	}
...
float GetDistanceToScene(vec3 P)  {
	...
	float planeDist = sdPlane(P, vec4(0,1,0,0));
...
void main(void){
  ...
  //Raymarching
	...
	const int MAX_STEPS = 30;								// Maxmimum amount of ray marching steps before counting the ray as a miss
	...
	float distanceFromCamera = 0.0;								// The ray starts at the camera, so 0
  	...
  	for(int i=0;i<MAX_STEPS;i++){
		...
		distanceToScene = GetDistanceToScene(raymarchPosition)*.4;				// Check how far the new position is away from the scene
  	}
  ...
  //Lighting
	...
	const vec3 e=vec3(0.02,0,0);								// Epsilon value, used to calculate the hit normal
  	...
  	if (distanceFromCamera<MAX_DEPTH){							// If this is true, it means we hit something
		...
		float lambert = pow(dot(normal, lightDir)+1.9,3.0);					// This is equivalent to having a diffuse light at the camera position
