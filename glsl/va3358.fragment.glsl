...
// @CBS: + Stars
// @CBS: + Water
// @CBS: + Clouds (work in progress...)
// @CBS: * Flying above clouds will show stars and cloud surface
// rotwang an CBS: well done
#ifdef GL_ES
...
vec2 water(in vec3 p){
    	...
    	float da = p.y;
  	vec2 vd = vec2(p.y+12.5)+waternoise(p.xz*0.2+(time*0.25));
	vd.x += da;
 	vd.y = ID_WATER;
	return vd;
}

vec2 clouds(in vec3 p){
	
    	float da = p.y;
	vec2 vd;
  	if (p.y<= 0.0){
		vd = vec2(p.y)+0.5*(cloudnoise(p.xz-0.05+(time*1.5)));
	}else
	{
		vd = vec2(p.y)-0.5*(cloudnoise(p.xz-0.05+(time*1.5)));
	}
 ...
 */
//smooth is a keyword in GLSL, so don't use it as a variable name!
//fixed compile error on AMD/ATI
//by Yours3lf
vec3 smoothmix(vec3 a, vec3 b, float base, float smoot, float source)
{
	float f = smoothstep(base-smoot, base+smoot, source );
	vec3 vec = mix(a, b, f);
	return vec;
}
...
vec2 scene(in vec3 p){
    ...
    vec2 hit = mindist( mindist(clouds(p),terrain(p)), mindist(water(p),ufo(p)) );
   	//hit -= water(p);
	return hit;
}
...
void main(void){
    ...
    if (f<MAX_DEPTH){
	    ...
	    //CLOUDS
            vec3 cc = vec3(0.6,0.8,1.0); 
            //clr *= vec3(b*clr+pow(b,0.5));
	    clr += mix(clr,cc, p.y+1.0);
