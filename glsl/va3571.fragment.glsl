...
const float 	SPEED = 90000.1; // This is how fast the fractal becomes less "bloomed". Smaller => faster.
const vec2 	START = vec2(0.22, 0.61); // Starting coordinates

const vec3	COLOR_WEIGHT = vec3(2.0, 5.0, 4.5)/MAXITERATIONS; // Relative weights of the colours used in the fractal.

//-------------------------

void main( void ) {
	
	float y0 = surfacePosition.x / START_ZOOM + START.x;
	float x0 = surfacePosition.y / START_ZOOM + START.y;
	float y = 0.0;
	float x = 0.0;
	...
	for (float i = 0.0; i < MAXITERATIONS; i += INCREMENT){
		...
		float xtemp = pow(x, .2) - pow(y, 2.0);
		y = (2.0*x*y) + y0;
		xtemp *= 1.0 + 0.0002*time*cos(i*0.2+(0.23*time));
		x = xtemp + x0;
		//x *= 1.0 + 0.00008*time*cos(MAXITERATIONS-i*0.26+(0.1*time));
		//y *= 1.0 + 0.00009*time*sin(MAXITERATIONS-i*0.29+(0.1*time));
		count += INCREMENT;
