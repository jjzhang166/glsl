...
// derived from Baldwin. for CGE2013. experimentation. mirror cube.

//glossary, for my (philip dickinson's) benefit:
// ro = ray origin
// rd = ray direction
...
void main(void)
{
	float rotspeed = time*1.+mouse.x;
	vec3 light = vec3(5.,4.+3.*sin(-rotspeed*.4),2.);
	float radius = sin(rotspeed*.1)*2.+4.;
	vec3 boxPos = vec3(0.3,1.5+sin(rotspeed),0.2);
	vec3 eye = vec3(radius*sin(rotspeed),2.*sin(.1*rotspeed)+2.5+2.*mouse.y,radius*cos(rotspeed*1.));
	//fixed mouse control
	vec3 screen = vec3((radius-1.)*sin(rotspeed),1.5*sin(.1*rotspeed)+2.+2.*mouse.y,(radius-1.)*cos(rotspeed*1.));
	...
	vec3 ca=vec3(0.);
	float lra=1.;
	for (int reflections=0;reflections<10;reflections++) { //try with just 1 or 2 reflections...
		...
		// Colouring
		global = .3; //affects cube strongly
