...
{
	return voxel.y < random( vec4( voxel.x, voxel.z, 4.4235, 0.3481 ) ) * 4.0;
}
...
vec4 GetRayColor(Ray ray)
{
	...
	for(int i=0;i<100/*CAREFUL WITH THIS!!!*/;i++)
	{
		if(ShouldDraw(voxel))
		{
			return vec4(0.4, 0.6, 0.3, 1.0) * abs( dot( hitPoint, ray.Direction ) );
		}
...
void main( void ) 
{
	...
	GetCameraRay(vec3(10.1*sin(time*0.1), 15.1, 10.1*cos(time*0.1)), vec3(0.0, 0.0, 0.0), currentRay);
