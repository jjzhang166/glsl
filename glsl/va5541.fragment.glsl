...
{
	float d = (p.y>3.0 || p.y<-1.0) ? 10.0 : sqrt(pow(p.x*0.01,2.0)+pow(p.z*0.01,2.0))/1.0;
	vec2 v = vec2(d, 1.0 );
	return v;
}
...
vec2 bottle(vec3 p)
{
	...
	float d = (p.y>5.0 || p.y<-1.0) ? 10.0 : envelope*sqrt(pow(p.x*0.01,2.0)+pow(p.z*0.01,2.0))/1.0;
	vec2 v = vec2(d, 1.0 );
	return v;
}
...
// main shape function
vec2 shape(in vec3 p){
  return ObjUnion(bottle(p),floor_shape(p));
	//return cylinder(p);
}
