...
// I was expecting something different, lol. Mod by MrOMGWTF
#ifdef GL_ES
...
vec4 heightmap(vec2 position)
{
	...
	height = sin(position.x * position.y * 0.2) * 0.5 + 1.0;
...
void main( void ) {
	...
	gl_FragColor = vec4( col+((colorvec*brightness*0.75)-0.5)*2.0, 1.0 );
}