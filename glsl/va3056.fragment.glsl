...
float pn(vec3 p) {
   vec3 i = floor(p);
   vec4 a = dot(i, vec3(1., 57., 21.)) + vec4(123.3, 57., 21., 78.);
...
{  
   ...
   vec3 p = vec3(0.3,0.1,1.0);
   vec3 d = vec3((gl_FragCoord.xy/(0.5*resolution)-1.)*vec2(resolution.x/resolution.y,1.0), 0.) - vec3(1,2,3);
   d = normalize(d); 
