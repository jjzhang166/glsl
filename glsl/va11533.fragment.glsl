...
vec3 lazer(vec2 pos, vec3 clr, float mult)
{
	...
	//fixes for intel 4000 GPU
	vec3 color = vec3(0.0);
	//for(int i =0; i<N; ++i){
		float order = (1.-float(1)/float(N));
		float w = fract(time*0.6);
		
		w = 5.+sin(pi2*(w+order));
		w *= 1.*order; //ampli intensite
		w += 0.3;
...
void main()
{
	...
	vec3 color = max(vec3(0.), lazer(pos, vec3(3.00, 0.0, 3.4), 0.25));
	gl_FragColor = vec4(color * 0.025, 0.75);
}
