...
float noise2(vec2 x,float t) { return frac(sin(dot(x,vec2(1.38984*sin(t),1.13233*cos(t))))*653758.5453); }
...
void main( void ) {
	...
	col+=PlotFunc(fbm(position.xx-time*1.5)*3.5,vec3(0,1,0),0.0125);	
	col+=PlotFunc(fbm(position.xx-time*2.0)*2.5,vec3(0,0,1),0.0125);		
//	col+=PlotFunc(acos(gX*8.0),vec3(1,1,0),0.0125);			
//	col+=PlotFunc(tan(gX*8.0),vec3(1,0,1),0.0125);	

	float y=gl_FragCoord.y/resolution.y;
	float c=noise2(gl_FragCoord.xy,time);
	c*=0.2+0.05*clamp(sin(y*2.0*3.141592+time*9.0)-0.4,0.0,1.0);
	vec3 c2=vec3(c);

	gl_FragColor = vec4( (col*0.4)+c2,1.0); //mix( col,c2,0.8), 1.0 );
