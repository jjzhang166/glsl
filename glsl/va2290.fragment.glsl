...
vec3 colorMix(in vec2 position, in vec3 color, in float n) {
	...
	return color * val + (1.0 - val) * vec3(0.9, 0.05, 0.01);
}
...
void main()
{
	vec2 position = -1.0 + 10.0 * (gl_FragCoord.xy / resolution.xy);
	...
	vec2 centerpos = vec2(0.3 + cos(time) * 0.1, 0.2 + sin(time * 0.27) * 1.1);
	float d = (1.0 - distance(position, centerpos)) * 0.04;
	
	centerpos -= position;
	for (float j = 0.0; j < 0.28; j+= 0.011) {
		color = mix(color, texture2D(backbuffer, position + centerpos * j).rgb, 0.21 - j); // 0.2
	}
	color += d * d + d ;
