...
void main(void)
{
	...
	*/
	...
	nd =sin(3.14*2.0*pos.y + i*90.5 + time)*90.3*(pos.y+80.3)+0.5;
	...
	vec2 drop = 0.5 + complex_mul((uv_orig - 0.5)*aspect, rot)*baz/aspect;
	drop = wrap_flip(drop);
	drop = clamp(drop, 0., 1.);
	
	vec4 dbuff =  bilinear(backbuffer,complexSquaredPlusC, resolution)*foo - bar;
 
	gl_FragColor = mix(cbuff, vec4(1),  dbuff * (1. - border(complexSquaredPlusC, 0.25)));
