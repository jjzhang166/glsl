...
void main( void ) {
	vec2 position = (gl_FragCoord.y - resolution * 0.5) / resolution.yy;
	...
	color += mix(vec3(0.2, 0.2, 1.0), vec3(0.1, 0.1, 0.2),  pow(b, 0.5)) * 0.15;
	color += mix(vec3(1.2, 1.2, 10.0), vec3(0.1, 0.2, 0.2),  pow(c, 0.2)) * 0.15;
	
	float scale = sin(0.1 * time) * 2.5 + 5.0;
	float distortion = resolution.y / resolution.x;

	vec2 position2 = (((gl_FragCoord.xy * 1.8 / resolution) ) * scale);
