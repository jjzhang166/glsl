...
void main()
{
	...
	vec3 color = floor(mod(position.x,2.0)) * floor(mod(position.y, 2.0)) *  vec3(0.5,0,0)
		+ floor(mod(position.y, 2.0)) * floor(mod(position.x,2.0)) * vec3(0,0,0.5);

	gl_FragColor = vec4(color,1.0)+0.95*texture2D(backbuffer, gl_FragCoord.xy/resolution.xy);
}