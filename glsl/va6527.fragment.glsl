...
void main(void) {
	...
	vec2 p = gl_FragCoord.xy / resolution.xy * vec2(-0.2, 2.5) - 1.0;
	p.x /= resolution.y / resolution.x;
	
	vec3 c1 = mix( vec3(0., 0., 0.), vec3(1.2), gl_FragCoord.y/resolution.y * 0.8);
	float c2 = fbm(p - time/10.) + fbm(p - time/10.) + fbm(p - time/50.) + 3.0;
	
	//float v = (((c2 * 0.1) - .5) * .5) + 0.4;
	float v = (((c2 * 0.24) - .9) * .65) * 0.8;
	
	//v*= .8;
	
	//vec4 col = vec4( mix( c1, vec3(v), v ), 1.0 );	
	vec4 col = vec4( mix( c1, vec3(1.), v ), 1.0 );
