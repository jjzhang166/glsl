...
// TEST BUILDING
vec2 obj_testBuilding(vec3 p, vec3 b){ // b= height deformation
  vec3  di = abs(p) - b;
  float mc = maxcomp(di);
  return vec2(min(mc,length(max(di,0.0))), 1);
}
...
vec2 obj_manyBoxes(in vec3 p){
	...
	return obj_testBuilding(q,b);	
}

// SIMPLE BUILDING (white)
vec2 obj_simpleBuilding(in vec3 p, in float height){
	vec3 offset = vec3(0.0, (-height), 0.0);
	return vec2(length(max(abs(p+offset)-vec3(1,height,2), 0.0)), 1);
}
...
void main(void){
	...
	const float MAX_DEPTH=200.0; //Max depth use 500
	...
	if (totalDist<MAX_DEPTH){
		...
		if(SHADING_MODE==PHONG_SHADING){
			...
			if (dist.y==0.0) // floor color
				finalColor=vec4(COLOR_DARKGREY,1.0);
