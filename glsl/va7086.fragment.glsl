...
void main(void) {
	...
	float mind = -1.0;
	for (int i = 0; i < 128; i++) {
		float a = 2.0 * PI / 16.0 * float(i) + time * 0.3;
		hit sh = sphere(orig, dir, vec3(float(i) / 16.0 - 4.0, cos(a), 2.0 + sin(a)), 0.15);
	...
	if (h.dist >= 0.0) {
		...
		float isqr = 10.0 / (4.0 * PI * pow(distance(h.pos, lightPos), 2.0));
		...
		float radiance = max(ambient, diffuse) * isqr;
		float highlight = specular * isqr;
		
		color = mix(vec3(radiance, 0.0, 0.0), vec3(1.0), highlight);
	}
