...
void main( void ) {
	...
	float st = floor(fract(speed)*12.0)/12.0;
	vec2 p1 = rotated( st,0.75);
	float shade_b = circle( p1-pos, 0.125, 0.09);
	
	
	vec3 clr_a = vec3( shade_a*0.8, shade_a*0.4, shade_a*0.1 );
	vec3 clr_b = vec3( shade_b*0.5, shade_b*0.6, shade_b*0.2 );
	vec3 clr = max(clr_a, clr_b)* 2.0-length(pos);
