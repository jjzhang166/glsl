...
void main(){
	...
	gl_FragColor=vec4(s(vec3(cos(time*1.1)*.9,sin(time)*.10,time), normalize(vec3((gl_FragCoord.xy-vec2(resolution/2.))/vec2(resolution.x),.5))),1);
}