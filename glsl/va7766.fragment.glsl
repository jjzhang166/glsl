...
void main( void ) {
	...
	float d = resolution.x / 0.7;

	for(int n=0; n<150; n++) {
		float x = sin(time*float(n+0)/10.0)*d/float(n+1)*time/(time/6.0)+resolution.x/2.0;
		float y = cos(time*float(n+0)/10.0)*d/float(n+1)*time/(time/6.0)+resolution.y/2.0;
