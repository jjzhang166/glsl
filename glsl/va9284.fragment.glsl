...
void main(void) 
{
	...
	position2 *= resolution.xy/(sin(time*0.0)*5.0+290.0);

	float angle1 = time*sin(time*0.0001) + 0.00;
	float angle2 = time*cos(time*0.005)*0.0 + 0.04;
	float angle3 = time*0.005*0.0 + 0.08;
	vec2 d1 = vec2(cos(angle1), sin(angle1+sin(time)*0.5));
	vec2 d2 = vec2(cos(angle2), sin(angle2));
	vec2 d3 = vec2(cos(angle3), sin(angle3-sin(time)*0.5));
	...
	for(int i = 2; i <= 5; i++)
	{
		...
		vec3 uv = coord + vec3(0.,-time*.02, time*.005);
