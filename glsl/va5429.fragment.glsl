...
vec4 two(float x, float y)
{
	...
	float realX = aspect * (x - 0.5);
        float realY = y - 0.5;
	float radius = 0.2;
	
	float dist = radius - length(vec2(realX, realY));
	float color;
	if (dist > 0.0)
		color = 0.35;
	else
	...
	float theta = 2.0 * PI * realX;
	float phi = PI * realY;
	
	float nx = cos(theta) * sin(phi) * radius;
	float ny = sin(theta) * sin(phi) * radius;
	float nz = -cos(phi) * radius;
	
	vec3 eye = vec3(0.0, 0.0, 10.0);
	eye = normalize(eye);
	
	vec3 lightVector = vec3(sin(time * 5.), cos(time * 2.), sin(time * 4.) * cos(time * 3.));
	vec3 normal = vec3(nx, ny, nz);
	normal = normalize(normal);
	float diffuseFactor = dot(lightVector, normal);
	
	vec3 ambientColor = vec3(color, color, color);
	
	vec3 diffuseColor = vec3(color * sin(time * 2.), color * cos(time * 2.), color * cos(time * 10.));
	diffuseColor *=  diffuseFactor;
	
  	vec3 reflectVec = normalize(-reflect(lightVector, normal));
  	vec3 specular = diffuseColor * pow(max(dot(reflectVec, eye), 0.0), 2.75);
	
	return vec4(ambientColor + diffuseColor + specular, 1.0);
}
...
vec4 three(float x, float y)
{
        ...
        float result = mod(floor(sqrt(realX * realX + realY * realY) / DR) -
time * 31.0,  69.0);
        return vec4(mod(1.0 - result * 0.12 * sin(time), 1.0),
(mod(result * 0.017 * cos(time), 1.0)), sin(result* time * 0.0043),
1.0);
}
