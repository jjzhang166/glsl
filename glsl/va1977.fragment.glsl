...
#define SCALE (0.7 + sin(time * 2.5) * 0.00)
#define COLOR_BACK vec4(.0, .0, 0.0, 1.0)
...
void main( void ) {
	...
	float st = resolution.x * 0.5 * SCALE + tan(time * 900.0) * 40.0;
	...
	{
		gl_FragColor = PSAMPLE(pixel.x, pixel.y + 1.0);
	}
